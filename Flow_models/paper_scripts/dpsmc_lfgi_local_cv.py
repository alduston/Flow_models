...
# -*- coding: utf-8 -*-
"""
dpsmc_lfgi_local_cv.py

Batched Diffusion Path Sequential Monte Carlo (DPSMC) comparison for targets
from the attached iterative_lfgi script.

This script compares:
  * DPSMC-GlobalMatrixCV: DPSMC with the paper-style global matrix control
    variate schedule based on a global target score covariance / Fisher matrix.
  * DPSMC-LFGI-LocalMatrixCV: DPSMC with a state-dependent LFGI Hessian-
    resolvent matrix gate estimated from the current auxiliary posterior.

It reuses the attached target families and diagnostics: misaligned GMMs,
Neal funnel, and the molecular LJ/DW-style examples with target score and
Hessian access.  Outputs include config.json, metrics_by_method.csv,
dpsmc_diagnostics.csv, heatmaps_final.png, and metric_bars.png.

Example smoke run:
python dpsmc_lfgi_local_cv.py \
  --outdir results/dpsmc_gmm3d \
  --device cuda --dtype float64 \
  --target gmm_3d \
  --methods global,lfgi \
  --n_samples 5000 --n_truth 5000 \
  --n_ref 512 --gate_n 512 --sampler_batch 16 \
  --n_steps 300 --t_min 0.001 --t_max 3.0 \
  --time_schedule linear --aux_init target \
  --bank_coupling independent

Note: exact per-trajectory DPSMC scales as O(n_samples * n_ref * n_steps).
Large n_ref=2000, n_samples=5000, n_steps=300 runs are intentionally expensive;
use --sampler_batch to control memory and start with smaller smoke settings.
"""
...
# -----------------------------------------------------------------------------
# DPSMC local/global matrix-CV experiment driver
# -----------------------------------------------------------------------------

@dataclass
class DPSMCDiagnostics:
    method: str
    n_generated: int
    sampler_batch: int
    n_ref: int
    ...
    min_mean_ess_frac: float = float("nan")
    final_mean_ess_frac: float = float("nan")
    resample_events: int = 0
    failed: bool = False
    fail_reason: str = ""
    elapsed_sec: float = 0.0


def _dpsmc_method_key(method: str) -> str:
    key = str(method).strip().lower().replace("-", "_")
    aliases = {
        "global": "global",
        "dpsmc_global": "global",
        ...
        "dpsmc_global_dpsmc_global": "global",
        "matrix_blend": "global",
        "matrix_matrix": "global",
        "blend_blend": "global",
        "lfgi": "lfgi",
        "local": "lfgi",
        "dpsmc_lfgi": "lfgi",
        ...
        "tsi": "tsi",
        "scalar": "scalar",
        "scalar_global": "scalar",
        "msi": "msi",
    }
    if key not in aliases:
        raise ValueError(f"Unknown DPSMC method {method!r}; use global, lfgi, tweedie, tsi, scalar, or msi")
    return aliases[key]


def selected_dpsmc_methods(methods: str) -> List[Tuple[str, str]]:
    raw = str(methods or "global,lfgi").strip().lower()
    if raw in {"all", "default", "*"}:
        keys = ["global", "lfgi", "tweedie", "tsi", "scalar", "msi"]
    else:
        keys = [k.strip() for k in raw.replace(";", ",").split(",") if k.strip()]
    out: List[Tuple[str, str]] = []
    seen = set()
    pretty = {
        "global": "DPSMC-GlobalMatrixCV",
        "lfgi": "DPSMC-LFGI-LocalMatrixCV",
        "tweedie": "DPSMC-Tweedie",
        "tsi": "DPSMC-TSI",
        "scalar": "DPSMC-GlobalScalarCV",
        "msi": "DPSMC-MSI",
    }
    for k in keys:
        kk = _dpsmc_method_key(k)
        if kk not in seen:
            out.append((pretty[kk], kk))
            seen.add(kk)
    return out


def make_dpsmc_target(cfg: Config, device: torch.device, dtype: torch.dtype):
    key = str(cfg.target).strip().lower().replace("-", "_")
    if key in {"gmm_8d", "gmm8d"}:
        cfg.target = "misaligned_gmm"
        cfg.d = 8
        if int(cfg.rank) >= int(cfg.d):
            cfg.rank = 3
        return make_target(cfg, device, dtype)
    if key in {"gmm_3d", "gmm3d", "misaligned_gmm_3d", "misaligned3d"}:
        cfg.target = "misaligned_gmm"
        cfg.d = 3
        cfg.rank = min(max(int(getattr(cfg, "rank", 1)), 1), 2)
        if int(getattr(cfg, "n_components", 0)) == int(Config().n_components):
            cfg.n_components = 4
        if float(getattr(cfg, "radius", 3.0)) == float(Config().radius):
            cfg.radius = 2.5
        if float(getattr(cfg, "sigma_perp", 0.035)) == float(Config().sigma_perp):
            cfg.sigma_perp = 0.06
        target = make_target(cfg, device, dtype)
        target.name = "misaligned_gmm_3d"
        target.target_info = lambda target=target: {
            "target_name": "misaligned_gmm_3d",
            "target_type": "gmm",
            "target_dim": int(target.d),
            "gmm_rank": int(target.rank),
            "gmm_n_components": int(target.K),
...
def _chunked_target_score(target, x_flat: torch.Tensor, chunk: int) -> torch.Tensor:
    outs: List[torch.Tensor] = []
    chunk = max(1, int(chunk))
    for s in range(0, int(x_flat.shape[0]), chunk):
        outs.append(target.score(x_flat[s:s + chunk], t=0.0).detach())
    return torch.cat(outs, dim=0)


def _chunked_target_hessian(target, x_flat: torch.Tensor, chunk: int) -> torch.Tensor:
    outs: List[torch.Tensor] = []
    chunk = max(1, int(chunk))
    for s in range(0, int(x_flat.shape[0]), chunk):
        outs.append(target.observed_information(x_flat[s:s + chunk], t=0.0).detach())
    return torch.cat(outs, dim=0)


def _log_kernel_ou(x: torch.Tensor, y: torch.Tensor, t: float) -> torch.Tensor:
    """Unnormalized log K_t(x | y) for OU corruption, shape [B,N]."""
    alpha, gamma = alpha_gamma(float(t), device=x.device, dtype=x.dtype)
    gamma = torch.clamp(gamma, min=torch.as_tensor(1.0e-12, device=x.device, dtype=x.dtype))
    diff = x[:, None, :] - alpha * y
    return -0.5 * torch.sum(diff * diff, dim=-1) / gamma


class BatchedAuxiliarySMC:
    """Per-trajectory auxiliary SMC bank for DPSMC score identities.

    The particles live in clean target coordinates Y.  For each noisy path state
    x_b at time t, weights approximate p(Y | X_t=x_b).  We use identity forward
    kernels between adjacent times, optional resampling, and optional ULA
    rejuvenation against the current OU posterior.
    """
    ...
    def __init__(
        ...
        self.device = target.device
        self.dtype = target.dtype
        self.prev_x = x0.detach().clone()
        self.prev_t = float(t0)
        aux_init = str(aux_init).strip().lower().replace("-", "_")
        self.aux_init = aux_init
        if aux_init in {"target", "oracle", "p0"}:
            y = target.sample(self.B * self.N, generator=generator).reshape(self.B, self.N, self.d)
            base_logw = torch.zeros((self.B, self.N), device=self.device, dtype=self.dtype)
        elif aux_init in {"prior", "gaussian", "normal", "n0i"}:
            y = torch.randn((self.B, self.N, self.d), device=self.device, dtype=self.dtype, generator=generator)
            flat = y.reshape(-1, self.d)
            base = target.log_prob(flat, t=0.0) - standard_normal_logprob(flat)
            base_logw = base.reshape(self.B, self.N)
        else:
            raise ValueError("aux_init must be target or prior")
        self.y = y.detach()
        self.logw = base_logw + _log_kernel_ou(self.prev_x, self.y, self.prev_t)
        self.logw = self._center_logw(self.logw)
        self.resample_events = 0
        self.min_mean_ess_frac = 1.0
        self.final_mean_ess_frac = float("nan")

    def _center_logw(self, logw: torch.Tensor) -> torch.Tensor:
        logw = torch.nan_to_num(logw, nan=0.0, posinf=0.0, neginf=0.0)
        return logw - torch.logsumexp(logw, dim=1, keepdim=True)

    def normalized_weights(self, sl: Optional[slice] = None) -> torch.Tensor:
        lw = self.logw if sl is None else self.logw[:, sl]
        return torch.softmax(lw, dim=1)

    def ess_frac(self, sl: Optional[slice] = None) -> torch.Tensor:
        w = self.normalized_weights(sl)
        n = w.shape[1]
        return 1.0 / torch.clamp(torch.sum(w * w, dim=1) * float(n), min=1.0e-30)

    def maybe_resample(self, generator: torch.Generator, threshold: float) -> int:
        if threshold <= 0.0:
            ef = self.ess_frac()
            self.final_mean_ess_frac = safe_float(ef.mean())
            self.min_mean_ess_frac = min(float(self.min_mean_ess_frac), self.final_mean_ess_frac)
            return 0
        ef = self.ess_frac()
        self.final_mean_ess_frac = safe_float(ef.mean())
        self.min_mean_ess_frac = min(float(self.min_mean_ess_frac), self.final_mean_ess_frac)
        rows = torch.nonzero(ef < float(threshold), as_tuple=False).reshape(-1)
        if rows.numel() == 0:
            return 0
        probs = torch.softmax(self.logw[rows], dim=1)
        idx = torch.multinomial(probs, self.N, replacement=True, generator=generator)
        gather_idx = idx[:, :, None].expand(-1, -1, self.d)
        self.y[rows] = torch.gather(self.y[rows], dim=1, index=gather_idx).detach()
        self.logw[rows] = 0.0
        self.resample_events += int(rows.numel())
        return int(rows.numel())

    def ula_rejuvenate(self, x: torch.Tensor, t: float, generator: torch.Generator, steps: int, step_size: float, score_chunk: int) -> None:
        steps = int(steps)
        if steps <= 0 or float(step_size) <= 0.0:
            return
        alpha, gamma = alpha_gamma(float(t), device=self.device, dtype=self.dtype)
        gamma = torch.clamp(gamma, min=torch.as_tensor(1.0e-12, device=self.device, dtype=self.dtype))
        dt = float(step_size)
        ns = math.sqrt(max(2.0 * dt, 0.0))
        for _ in range(steps):
            flat_y = self.y.reshape(-1, self.d)
            s0 = _chunked_target_score(self.target, flat_y, score_chunk).reshape(self.B, self.N, self.d)
            grad_log_post = s0 + (alpha / gamma) * (x[:, None, :] - alpha * self.y)
            grad_log_post = clamp_norm(grad_log_post.reshape(-1, self.d), getattr(self.cfg, "aux_grad_clip", 1.0e4)).reshape(self.B, self.N, self.d)
            self.y = self.y + dt * grad_log_post + ns * torch.randn(self.y.shape, device=self.device, dtype=self.dtype, generator=generator)
            if self.cfg.sample_clip and self.cfg.sample_clip > 0:
                self.y = torch.clamp(self.y, min=-float(self.cfg.sample_clip), max=float(self.cfg.sample_clip))
            self.y = torch.nan_to_num(self.y, nan=0.0, posinf=float(self.cfg.sample_clip), neginf=-float(self.cfg.sample_clip)).detach()
        # Treat the move as an approximate rejuvenation kernel and restart weights.
        self.logw.zero_()

    def advance(self, x_new: torch.Tensor, t_new: float, generator: torch.Generator, threshold: float, score_chunk: int) -> int:
        old_lp = _log_kernel_ou(self.prev_x, self.y, self.prev_t)
        new_lp = _log_kernel_ou(x_new, self.y, float(t_new))
        self.logw = self._center_logw(self.logw + new_lp - old_lp)
        self.prev_x = x_new.detach().clone()
        self.prev_t = float(t_new)
        n_resampled = self.maybe_resample(generator, threshold)
        self.ula_rejuvenate(
            self.prev_x,
            self.prev_t,
            generator,
            int(getattr(self.cfg, "aux_move_steps", 0)),
            float(getattr(self.cfg, "aux_move_step_size", 0.0)),
            score_chunk,
        )
        return n_resampled


def matrix_resolvent_from_curvature(H: torch.Tensor, lam: torch.Tensor, gamma: torch.Tensor, eps: float, clip: float) -> torch.Tensor:
    d = H.shape[-1]
    ...
    signs = torch.where(evals >= 0, torch.ones_like(evals), -torch.ones_like(evals))
    evals_safe = torch.where(torch.abs(evals) < float(eps), signs * float(eps), evals)
    gvals = lam / evals_safe
    if clip and float(clip) > 0.0:
        gvals = torch.clamp(gvals, min=-float(clip), max=float(clip))
    return sym(evecs @ torch.diag_embed(gvals) @ evecs.transpose(-1, -2))


def compute_global_fisher(target, cfg: Config, generator: torch.Generator) -> torch.Tensor:
    n = max(16, int(getattr(cfg, "global_fisher_n", 4096)))
    batch = max(1, int(getattr(cfg, "global_fisher_batch", 2048)))
    acc = torch.zeros((target.d, target.d), device=target.device, dtype=target.dtype)
    total = 0
    for start in range(0, n, batch):
        b = min(batch, n - start)
        x = target.sample(b, generator=generator)
        s = target.score(x, t=0.0).detach()
        acc = acc + s.T @ s
        total += b
    F = acc / max(total, 1)
    F = process_curvature(F.unsqueeze(0), cfg.curvature_mode, cfg.curvature_floor, cfg.curvature_cap)[0]
    return sym(F.detach())


def dpsmc_score_estimate(
    method: str,
    x: torch.Tensor,
    t: float,
    score_state: BatchedAuxiliarySMC,
    gate_state: Optional[BatchedAuxiliarySMC],
    cfg: Config,
    global_fisher: Optional[torch.Tensor],
) -> torch.Tensor:
    key = _dpsmc_method_key(method)
    B, d = x.shape
    alpha, gamma = alpha_gamma(float(t), device=x.device, dtype=x.dtype)
    gamma = torch.clamp(gamma, min=torch.as_tensor(1.0e-12, device=x.device, dtype=x.dtype))
    lam = alpha * alpha
    # Score bank atoms.
    ys = score_state.y
    ws = score_state.normalized_weights()
    flat_y = ys.reshape(-1, d)
    s0 = _chunked_target_score(score_state.target, flat_y, int(getattr(cfg, "aux_score_chunk", 65536))).reshape(B, score_state.N, d)
    b = (alpha * ys - x[:, None, :]) / gamma
    c = s0 / alpha
    bbar = torch.sum(ws[:, :, None] * b, dim=1)
    cbar = torch.sum(ws[:, :, None] * c, dim=1)
    if key == "tweedie":
        return bbar
    if key == "tsi":
        return cbar
    if key == "msi":
        # Heuristic mixed score identity: A=lambda?  In b+G(c-b) form, G=lambda.
        return bbar + lam * (cbar - bbar)
    if key == "scalar":
        # Global scalar CV analogue of DPSMC Proposition 2.  With base I_nu=I,
        # score-variance traces define A on DSI; G=1-A on the TSI correction.
        F = global_fisher
        trF = torch.trace(F).clamp_min(1.0e-30)
        trI = torch.as_tensor(float(d), device=x.device, dtype=x.dtype)
        A_scalar = (trF / lam) / (trI / gamma + trF / lam)
        G_scalar = 1.0 - A_scalar
        return bbar + G_scalar * (cbar - bbar)
    if key == "global":
        if global_fisher is None:
            raise ValueError("global_fisher is required for global matrix CV")
        G = matrix_resolvent_from_curvature(global_fisher.unsqueeze(0), lam, gamma, cfg.resolvent_eps, cfg.gate_clip)[0]
        return bbar + (cbar - bbar) @ G.T
    if key == "lfgi":
        gs = score_state if gate_state is None else gate_state
        yg = gs.y
        wg = gs.normalized_weights()
        flat_g = yg.reshape(-1, d)
        H = _chunked_target_hessian(gs.target, flat_g, int(getattr(cfg, "aux_hessian_chunk", 2048))).reshape(B, gs.N, d, d)
        H = process_curvature(H.reshape(-1, d, d), cfg.curvature_mode, cfg.curvature_floor, cfg.curvature_cap).reshape(B, gs.N, d, d)
        Hbar = torch.sum(wg[:, :, None, None] * H, dim=1)
        G = matrix_resolvent_from_curvature(Hbar, lam, gamma, cfg.resolvent_eps, cfg.gate_clip)
        return bbar + torch.einsum("bij,bj->bi", G, cbar - bbar)
    raise RuntimeError(f"Unhandled method key {key}")


@torch.no_grad()
def run_dpsmc_sampler(target, method: str, cfg: Config, generator: torch.Generator, global_fisher: Optional[torch.Tensor]) -> Tuple[torch.Tensor, Dict[str, object]]:
    t0 = time.time()
    n_total = int(cfg.n_samples)
    d = int(target.d)
    batch_size = max(1, int(getattr(cfg, "sampler_batch", 16)))
    n_ref = max(1, int(cfg.n_ref))
    gate_n = int(cfg.gate_n) if int(cfg.gate_n) > 0 else n_ref
    gate_n = max(1, gate_n)
    aux_init = str(getattr(cfg, "aux_init", getattr(cfg, "initial_reference_mode", "target")))
    if aux_init == "oracle":
        aux_init = "target"
    ts = make_time_grid(cfg, int(cfg.n_steps), direction="reverse", device=target.device, dtype=target.dtype)
    ess_thresh = float(getattr(cfg, "smc_ess_threshold", 0.5))
    method_key = _dpsmc_method_key(method)
    out_batches: List[torch.Tensor] = []
    max_abs_score = 0.0
    min_mean_ess = 1.0
    final_ess_vals: List[float] = []
    resample_events = 0
    failed = False
    fail_reason = ""
    for start in range(0, n_total, batch_size):
        B = min(batch_size, n_total - start)
        x = torch.randn((B, d), device=target.device, dtype=target.dtype, generator=generator)
        bank_coupling = canonical_bank_coupling(getattr(cfg, "bank_coupling", "shared"))
        if method_key == "lfgi" and bank_coupling == "independent":
            score_state = BatchedAuxiliarySMC(target, B, n_ref, d, x, float(ts[0].item()), cfg, generator, aux_init=aux_init)
            gate_state = BatchedAuxiliarySMC(target, B, gate_n, d, x, float(ts[0].item()), cfg, generator, aux_init=aux_init)
            states = [score_state, gate_state]
        elif method_key == "lfgi" and bank_coupling == "prefix":
            pool_n = max(n_ref, gate_n)
            pool_state = BatchedAuxiliarySMC(target, B, pool_n, d, x, float(ts[0].item()), cfg, generator, aux_init=aux_init)
            # Prefix mode is implemented by using the same pool for both and the
            # first gate_n particles as the gate.  To keep the object simple, if
            # gate_n != pool_n we just use the whole pool for both estimates.  This
            # is conservative and avoids maintaining inconsistent per-prefix ESS.
            score_state = pool_state
            gate_state = pool_state
            states = [pool_state]
        else:
            score_state = BatchedAuxiliarySMC(target, B, n_ref, d, x, float(ts[0].item()), cfg, generator, aux_init=aux_init)
            gate_state = score_state
            states = [score_state]
        for j in range(int(cfg.n_steps)):
            t = float(ts[j].item())
            tn = float(ts[j + 1].item())
            h = float((ts[j] - ts[j + 1]).item())
            s = dpsmc_score_estimate(method_key, x, t, score_state, gate_state, cfg, global_fisher)
            s = clamp_norm(s, cfg.score_clip)
            max_abs_score = max(max_abs_score, safe_float(s.abs().max()))
            if not torch.isfinite(s).all():
                failed = True
                fail_reason = f"nonfinite score at step {j}"
                break
            drift = x + 2.0 * s
            noise = torch.sqrt(torch.as_tensor(2.0 * h, device=target.device, dtype=target.dtype)) * torch.randn(x.shape, device=target.device, dtype=target.dtype, generator=generator)
            x = x + h * drift + noise
            if cfg.sample_clip and cfg.sample_clip > 0:
                x = torch.clamp(x, min=-float(cfg.sample_clip), max=float(cfg.sample_clip))
            x = torch.nan_to_num(x, nan=0.0, posinf=float(cfg.sample_clip), neginf=-float(cfg.sample_clip)).detach()
            for st in states:
                resample_events += st.advance(x, tn, generator, ess_thresh, int(getattr(cfg, "aux_score_chunk", 65536)))
                min_mean_ess = min(min_mean_ess, float(st.min_mean_ess_frac))
            if not torch.isfinite(x).all():
                failed = True
                fail_reason = f"nonfinite state at step {j}"
                break
        if bool(getattr(cfg, "final_denoise", False)) and not failed:
            t_min = float(ts[-1].item())
            s = dpsmc_score_estimate(method_key, x, t_min, score_state, gate_state, cfg, global_fisher)
            alpha, gamma = alpha_gamma(t_min, device=target.device, dtype=target.dtype)
            x = (x + gamma * s) / alpha
            if cfg.sample_clip and cfg.sample_clip > 0:
                x = torch.clamp(x, min=-float(cfg.sample_clip), max=float(cfg.sample_clip))
        for st in states:
            final_ess_vals.append(float(st.final_mean_ess_frac if math.isfinite(st.final_mean_ess_frac) else safe_float(st.ess_frac().mean())))
        out_batches.append(x.detach())
        if failed:
            # Keep going would usually just repeat the same instability; return
            # whatever was generated and mark failure.
            break
    samples = torch.cat(out_batches, dim=0) if out_batches else torch.empty((0, d), device=target.device, dtype=target.dtype)
    if samples.shape[0] < n_total and samples.shape[0] > 0:
        # Pad with resampled generated states so downstream metrics/plots stay alive.
        idx = torch.randint(0, samples.shape[0], (n_total - samples.shape[0],), device=target.device, generator=generator)
        samples = torch.cat([samples, samples[idx]], dim=0)
    diag = asdict(DPSMCDiagnostics(
        method=str(method_key),
        n_generated=int(samples.shape[0]),
        sampler_batch=int(batch_size),
        n_ref=int(n_ref),
        gate_n=int(gate_n),
        ess_threshold=float(ess_thresh),
        aux_init=str(aux_init),
        max_abs_score=float(max_abs_score),
        min_mean_ess_frac=float(min_mean_ess),
        final_mean_ess_frac=float(np.nanmean(final_ess_vals)) if final_ess_vals else float("nan"),
        resample_events=int(resample_events),
        failed=bool(failed),
        fail_reason=str(fail_reason),
        elapsed_sec=float(time.time() - t0),
    ))
    return samples.detach(), diag


def save_dpsmc_heatmaps(outdir: str, target, truth: torch.Tensor, samples_by_method: Dict[str, torch.Tensor], cfg: Config) -> None:
    # Reuse the plotting convention from the attached script, but with one final
    # panel per DPSMC method rather than alternating rounds.
    if _is_funnel_target(target):
        def to_plot(x):
            return as_numpy(target.plot_projection(x)) if hasattr(target, "plot_projection") else as_numpy(x[:, :2])
        arrays = [to_plot(truth)] + [to_plot(v) for v in samples_by_method.values()]
        lims = make_funnel_projection_limits(arrays)
        xlabel, ylabel = "funnel x_1", "funnel x_2"
        show_ticks = True
    else:
        mean, basis = fit_pca_projection(truth)
        arrays = [project_np(truth, mean, basis)] + [project_np(v, mean, basis) for v in samples_by_method.values()]
        lims = make_projection_limits(arrays)
        xlabel = ylabel = None
        show_ticks = False
        np.savez(os.path.join(outdir, "projection_basis.npz"), mean=mean, basis=basis)
    vals_for_vmax = arrays
    vmax = shared_heatmap_vmax(vals_for_vmax, lims, cfg)
    names = ["Truth"] + list(samples_by_method.keys())
    n = len(names)
    ...
    for ax, name, arr in zip(axes, names, arrays):
        plot_heatmap_panel(
            ax, arr, name, lims, cfg, vmax=vmax,
            xlabel=xlabel, ylabel=ylabel if name == "Truth" else None,
            show_ticks=show_ticks,
        )
        if show_ticks and name != "Truth":
            ax.set_yticklabels([])
    fig.suptitle("DPSMC endpoint samples", fontsize=12)
    fig.savefig(os.path.join(outdir, "heatmaps_final.png"), dpi=220, bbox_inches="tight")
    plt.close(fig)


def save_dpsmc_metric_bars(outdir: str, rows: List[Dict[str, object]]) -> None:
    try:
        import pandas as pd
    except Exception:
        return
    ...
    if df.empty:
        return
    metrics = ["nll", "mmd", "ksd", "sw2", "sliced_ks", "mean_norm", "cov_frob_err"]
    ...
    for ax, metric in zip(axes, metrics):
        if metric not in df.columns:
            ax.axis("off")
            continue
        ax.bar(df["family"].astype(str), df[metric].astype(float))
        ax.set_title(metric + " ↓")
        ax.tick_params(axis="x", rotation=35, labelsize=8)
        ax.grid(True, axis="y", alpha=0.25)
    for ax in axes[len(metrics):]:
        ax.axis("off")
    fig.suptitle("DPSMC metric comparison", fontsize=12)
    fig.savefig(os.path.join(outdir, "metric_bars.png"), dpi=220)
    plt.close(fig)


def run_dpsmc_experiment(cfg: Config) -> None:
    ...
    cfg.bank_coupling = canonical_bank_coupling(getattr(cfg, "bank_coupling", "shared"))
    target = make_dpsmc_target(cfg, device, dtype)
    cfg.d = int(target.d)
    print(f"Target {getattr(target, 'name', type(target).__name__)}: d={target.d}, device={device}, dtype={dtype}", flush=True)
    print(f"  moment check ||E[X]||={target.moment_mean_norm:.3e}, ||Cov-I||_F={target.moment_cov_frob_err:.3e}", flush=True)
    print(f"  DPSMC n_samples={cfg.n_samples}, n_ref={cfg.n_ref}, gate_n={cfg.gate_n if cfg.gate_n > 0 else cfg.n_ref}, batch={getattr(cfg, 'sampler_batch', 16)}, steps={cfg.n_steps}", flush=True)

    truth_gen = make_generator(int(cfg.seed + 202), device)
    truth = target.sample(int(cfg.n_truth), generator=truth_gen).detach()
    fisher_gen = make_generator(int(cfg.seed + 303), device)
    print("Estimating global Fisher/score covariance for DPSMC global matrix CV...", flush=True)
    global_fisher = compute_global_fisher(target, cfg, fisher_gen)
    gf_eigs = torch.linalg.eigvalsh(sym(global_fisher))

    config_dict = asdict(cfg)
    for extra in [
        "sampler_batch", "smc_ess_threshold", "aux_init", "aux_move_steps", "aux_move_step_size",
        "aux_score_chunk", "aux_hessian_chunk", "global_fisher_n", "global_fisher_batch", "aux_grad_clip",
    ]:
        if hasattr(cfg, extra):
            config_dict[extra] = getattr(cfg, extra)
    ...
    config_dict.update({
        ...
        "global_fisher_eig_min": safe_float(gf_eigs.min()),
        "global_fisher_eig_max": safe_float(gf_eigs.max()),
        "global_fisher_trace": safe_float(torch.trace(global_fisher)),
        **target_info,
    })
    ...
    baseline_gen = make_generator(int(cfg.seed + 404), device)
    truth2 = target.sample(min(int(cfg.n_truth), int(cfg.metrics_max_n)), generator=baseline_gen).detach()
    rows.append({"kind": "baseline", "family": "TARGET_FLOOR", "method": "TARGET_FLOOR", **compute_metrics(target, truth2, truth, None, cfg, baseline_gen)})
    prior = torch.randn((min(int(cfg.n_samples), int(cfg.metrics_max_n)), target.d), device=device, dtype=dtype, generator=baseline_gen)
    rows.append({"kind": "baseline", "family": "PRIOR", "method": "PRIOR", **compute_metrics(target, prior, truth, None, cfg, baseline_gen)})

    samples_by_method: Dict[str, torch.Tensor] = {}
    for display, method in selected_dpsmc_methods(cfg.methods):
        print(f"\n=== Running {display} ({method}) ===", flush=True)
        gen = make_generator(int(cfg.seed + 10_000 + family_seed_offset(display)), device)
        samples, diag = run_dpsmc_sampler(target, method, cfg, gen, global_fisher)
        samples_by_method[display] = samples
        metric_gen = make_generator(int(cfg.seed + 20_000 + family_seed_offset(display)), device)
        metrics = compute_metrics(target, samples, truth, None, cfg, metric_gen)
        rows.append({"kind": "sample", "family": display, "method": method, **metrics, **diag})
        diag_rows.append({"family": display, "method": method, **diag})
        print(
            f"{display}: NLL={metrics['nll']:.4g}, MMD={metrics['mmd']:.4g}, KSD={metrics['ksd']:.4g}, "
            f"SW2={metrics['sw2']:.4g}, minESS={diag['min_mean_ess_frac']:.3f}, "
            f"resamples={diag['resample_events']}, elapsed={diag['elapsed_sec']:.1f}s",
            flush=True,
        )

    write_csv(os.path.join(cfg.outdir, "metrics_by_method.csv"), rows)
    write_csv(os.path.join(cfg.outdir, "dpsmc_diagnostics.csv"), diag_rows)
    save_dpsmc_heatmaps(cfg.outdir, target, truth, samples_by_method, cfg)
    save_dpsmc_metric_bars(cfg.outdir, rows)
    # Backward-compatible filename for notebooks that expect the alternating script output.
    write_csv(os.path.join(cfg.outdir, "metrics_by_round.csv"), rows)
    print("\nDone. Wrote:", flush=True)
    for name in ["config.json", "metrics_by_method.csv", "dpsmc_diagnostics.csv", "heatmaps_final.png", "metric_bars.png"]:
        print("  " + os.path.join(cfg.outdir, name), flush=True)


def parse_args() -> Config:
    p = argparse.ArgumentParser(description="DPSMC global matrix CV vs LFGI local matrix CV on the attached LFGI targets.")
    defaults = Config()
    # Core/run flags.
    p.add_argument("--outdir", type=str, default="results/dpsmc_lfgi")
    p.add_argument("--seed", type=int, default=defaults.seed)
    p.add_argument("--device", type=str, default=defaults.device)
    p.add_argument("--dtype", type=str, default=defaults.dtype)
    p.add_argument("--target", type=str, default="gmm_3d", help="gmm_3d, gmm_8d/misaligned_gmm, funnel_d10, lj13_2d, dw4_16d")
    p.add_argument("--methods", type=str, default="global,lfgi", help="Comma list: global,lfgi,tweedie,tsi,scalar,msi or all")
    p.add_argument("--n_samples", type=int, default=1000)
    p.add_argument("--n_truth", type=int, default=5000)
    p.add_argument("--metrics_max_n", type=int, default=defaults.metrics_max_n)
    p.add_argument("--n_ref", type=int, default=512, help="Auxiliary SMC particles per generated sample trajectory")
    p.add_argument("--gate_n", type=int, default=0, help="Gate particles for independent LFGI; <=0 defaults to n_ref")
    p.add_argument("--sampler_batch", type=int, default=16, help="Number of generated trajectories processed together")
    p.add_argument("--bank_coupling", type=str, default="shared", choices=["shared", "prefix", "independent"])
    p.add_argument("--aux_init", type=str, default="target", choices=["target", "prior"], help="Initial auxiliary proposal q0")
    p.add_argument("--initial_reference_mode", type=str, default=None, help="Alias for --aux_init, accepted for compatibility")
    ...
    p.add_argument("--time_schedule", type=str, default=defaults.time_schedule, choices=["linear", "log_linear"])
    p.add_argument("--final_denoise", action="store_true")
    p.add_argument("--sample_clip", type=float, default=defaults.sample_clip)
    p.add_argument("--score_clip", type=float, default=defaults.score_clip)
    # DPSMC-specific SMC controls.
    p.add_argument("--smc_ess_threshold", type=float, default=0.5)
    p.add_argument("--aux_move_steps", type=int, default=0)
    p.add_argument("--aux_move_step_size", type=float, default=1.0e-4)
    p.add_argument("--aux_grad_clip", type=float, default=1.0e4)
    p.add_argument("--aux_score_chunk", type=int, default=65536)
    p.add_argument("--aux_hessian_chunk", type=int, default=2048)
    p.add_argument("--global_fisher_n", type=int, default=4096)
    p.add_argument("--global_fisher_batch", type=int, default=2048)
    # Target params copied from the attached script.
    p.add_argument("--d", type=int, default=defaults.d)
    p.add_argument("--rank", type=int, default=defaults.rank)
    p.add_argument("--n_components", type=int, default=defaults.n_components)
    p.add_argument("--target_seed", type=int, default=defaults.target_seed)
    p.add_argument("--radius", type=float, default=defaults.radius)
    p.add_argument("--sigma_perp", type=float, default=defaults.sigma_perp)
    p.add_argument("--jitter", type=float, default=defaults.jitter)
    p.add_argument("--normalize_target", dest="normalize_target", action="store_true", default=True)
    p.add_argument("--no_normalize_target", dest="normalize_target", action="store_false")
    p.add_argument("--funnel_d", type=int, default=defaults.funnel_d)
    p.add_argument("--funnel_eta2", type=float, default=defaults.funnel_eta2)
    p.add_argument("--funnel_score_bank", type=int, default=defaults.funnel_score_bank)
    p.add_argument("--funnel_score_chunk", type=int, default=defaults.funnel_score_chunk)
    p.add_argument("--mol_n_particles", type=int, default=defaults.mol_n_particles)
    p.add_argument("--mol_particle_dim", type=int, default=defaults.mol_particle_dim)
    p.add_argument("--mol_beta", type=float, default=defaults.mol_beta)
    p.add_argument("--mol_lj_eps", type=float, default=defaults.mol_lj_eps)
    p.add_argument("--mol_lj_sigma", type=float, default=defaults.mol_lj_sigma)
    p.add_argument("--mol_lj_soft_core", type=float, default=defaults.mol_lj_soft_core)
    p.add_argument("--mol_bond_k", type=float, default=defaults.mol_bond_k)
    p.add_argument("--mol_confinement_k", type=float, default=defaults.mol_confinement_k)
    p.add_argument("--mol_com_k", type=float, default=defaults.mol_com_k)
    p.add_argument("--mol_init_noise", type=float, default=defaults.mol_init_noise)
    p.add_argument("--mol_sample_steps", type=int, default=defaults.mol_sample_steps)
    p.add_argument("--mol_sample_step_size", type=float, default=defaults.mol_sample_step_size)
    p.add_argument("--mol_sample_batch", type=int, default=defaults.mol_sample_batch)
    p.add_argument("--mol_norm_samples", type=int, default=defaults.mol_norm_samples)
    p.add_argument("--mol_norm_eig_floor", type=float, default=defaults.mol_norm_eig_floor)
    p.add_argument("--mol_score_bank", type=int, default=defaults.mol_score_bank)
    p.add_argument("--mol_score_chunk", type=int, default=defaults.mol_score_chunk)
    p.add_argument("--mol_hessian_chunk", type=int, default=defaults.mol_hessian_chunk)
    # Gate/curvature controls.
    p.add_argument("--curvature_mode", type=str, default=defaults.curvature_mode, choices=["raw", "psd", "abs"])
    p.add_argument("--curvature_floor", type=float, default=defaults.curvature_floor)
    p.add_argument("--curvature_cap", type=float, default=defaults.curvature_cap)
    p.add_argument("--resolvent_eps", type=float, default=defaults.resolvent_eps)
    p.add_argument("--gate_clip", type=float, default=defaults.gate_clip)
    p.add_argument("--mp_leaf_floor", type=float, default=defaults.mp_leaf_floor)
    p.add_argument("--mp_leaf_tol", type=float, default=defaults.mp_leaf_tol)
    # Plot/metric controls.
    p.add_argument("--sw2_projections", type=int, default=defaults.sw2_projections)
    p.add_argument("--hist_bins", type=int, default=defaults.hist_bins)
    p.add_argument("--hist_gamma", type=float, default=defaults.hist_gamma)
    p.add_argument("--hist_vmax_quantile", type=float, default=defaults.hist_vmax_quantile)
    # Compatibility no-ops for command lines based on iterative_lfgi_route1b.
    p.add_argument("--n_rounds", type=int, default=1)
    p.add_argument("--pf_steps", type=int, default=defaults.pf_steps)
    p.add_argument("--feedback_mode", type=str, default="none")
    p.add_argument("--kde_n_eval", type=int, default=defaults.kde_n_eval)
    p.add_argument("--kde_n_fit", type=int, default=defaults.kde_n_fit)
    p.add_argument("--kde_bandwidth", type=float, default=defaults.kde_bandwidth)
    p.add_argument("--no_likelihood_calibration", dest="likelihood_calibration", action="store_false", default=False)
    args = p.parse_args()
    cfg = Config()
    for k, v in vars(args).items():
        if v is not None:
            setattr(cfg, k, v)
    if args.initial_reference_mode is not None:
        cfg.aux_init = canonical_initial_reference_mode(args.initial_reference_mode)
    cfg.t_start = cfg.t_max
    cfg.t_end = cfg.t_min
...
if __name__ == "__main__":
    run_dpsmc_experiment(parse_args())
