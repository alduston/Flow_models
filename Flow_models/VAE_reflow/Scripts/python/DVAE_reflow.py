...
class EncoderX1OnlyPro(nn.Module):
    def __init__(self, D=2, width=512, depth=8, dropout=0.05,
                 ...
                 residual_mu=True, ortho_mix=False, n_householder=4,
                 logvar_min=-10.0, logvar_max=+6.0):
        super().__init__()
        self.D = int(D)
...
class AVRC:
    def __init__(self, cfg=AVRCConfig()):
        ...
        self.Enc = EncoderX1OnlyPro(
            ...
            depth=cfg.enc_depth,
            residual_mu=bool(cfg.enc_residual_mu)
        ).to(device)
    ...
    @torch.no_grad()
    def _save_alignment_over_t_frame(self, round_idx: int):
        """
        Plot directional misalignment vs t for the current model snapshot:
        E[ 1 - cos(u_teacher(x_t,t), n_chord) ] over t in [0,1].
        ...
        for t in t_vals:
            ...
            v = self.V(xt, tt)                                           # (n,D)
            # alignment: 1 - cos between teacher direction and chord direction
            eps = 1e-12
            u_teacher = v / (v.norm(dim=1, keepdim=True) + eps)
            n_chord   = label / (label.norm(dim=1, keepdim=True) + eps)
            cos_sim   = (u_teacher * n_chord).sum(dim=1).clamp(-1.0, 1.0)
            a = (1.0 - cos_sim).mean()
            align_vals.append(a.item())

        # Convert once for plotting
        t_np   = t_vals.detach().cpu().numpy()
        disp_np= np.array(align_vals, dtype=np.float64)
        ...
        out = os.path.join(out_dir, f"alignment_{round_idx:05d}.png")

        plt.figure(figsize=(6, 4), dpi=150)
        plt.plot(t_np, disp_np, linewidth=2, label="alignment (1 - cos)")
        ...
        plt.xlabel("t")
        plt.ylabel(r"$\mathbb{E}\,[1 - \cos(u_t, n)]$")
        plt.title(f"Directional alignment vs t — round {round_idx:05d}")
        ...
        print(f"[alignment] r={round_idx}  a(0)={disp_np[0]:.4f}  a(1)={disp_np[-1]:.4f}  E||ℓ||={lab_norm:.4f}")
    ...
    def organizer_step(self, B=None):
        ...
        # Direction-only organizer: align teacher direction with chord direction
        with torch.no_grad():
            v_t = self.V_teacher(zt, t)                 # teacher velocity label at (x_t, t)
        eps = 1e-12
        u_teacher = v_t / (v_t.norm(dim=1, keepdim=True) + eps)     # unit teacher direction
        n_chord   = ell / (ell.norm(dim=1, keepdim=True) + eps)     # unit mean-field (chord) direction

        # Scalar control gamma ≈ stopgrad(s_theta) * E[||c||] (per-batch mean as a simple proxy)
        s_stop = v_t.norm(dim=1, keepdim=True).detach()
        c_norm_hat = ell.norm(dim=1, keepdim=True).detach().mean()
        gamma = s_stop * c_norm_hat

        cos_sim = (u_teacher * n_chord).sum(dim=1, keepdim=True).clamp(-1.0, 1.0)
        dir_piece = (w * gamma * (1.0 - cos_sim)).mean() / (w.mean() + 1e-12)

        # Unweighted alignment (angle-only, no gamma or w), for logging/diagnostics
        align_unw = (1.0 - cos_sim).mean()

        disp_piece  = torch.tensor(0.0, device=zt.device, dtype=zt.dtype)
        align_piece = dir_piece

        loss_kl = kl_normal_diag(mu0_enc, logv0)

        # scalar speed of teacher along the chord direction
        speed_teacher = (v_t * n_chord).sum(dim=1, keepdim=True)             # ⟨v_t, n_chord⟩
        chord_len     = ell.norm(dim=1, keepdim=True)                        # ||ℓ||

        # dispersion (magnitude) match; unbiased when t are midpoints
        disp_piece = F.mse_loss(speed_teacher, chord_len)

        # Direction-only objective uses align weight as λ_dir; dispersion term is disabled
        loss = (self._lam_align * align_piece
                + self._lam_disp  * disp_piece      # ← NEW
                + self._lam_kl    * loss_kl)

        self.opt_enc.zero_grad(set_to_none=True); loss.backward()
        # Encoder grad-norm (pre-clip) for diagnostics
        g2 = torch.tensor(0.0, device=zt.device, dtype=zt.dtype)
        with torch.no_grad():
            for p in self.Enc.parameters():
                if p.grad is not None:
                    g2 += (p.grad * p.grad).sum()
        enc_grad_norm = float(torch.sqrt(g2 + 1e-20).detach().cpu())
        ...
        return {'disp': float(disp_piece.item()),
                ...
                'align_unw': float(align_unw.item()),
    ...
    def train(self, rounds=None, progress=True, seed=None):
        ...
        try:
            ...
            self._save_alignment_over_t_frame(round_idx=0)
        ...
        for r in range(1, rounds+1):
            ...
            if (self.cfg.k_plot is not None) and (self.cfg.k_plot > 0) and (r % self.cfg.k_plot == 0):
                try:
                    ...
                    self._save_alignment_over_t_frame(round_idx=r)
                    if self.cfg.viz_latent:
            ...
            if (r % self.cfg.log_every) == 0 and progress:
                ...
                print(
                    ...
                    f"align {stats['align']:.4f} align_unw {stats.get('align_unw', float('nan')):.4f} kl {stats['kl']:.4f} | "
                    f"enc∥∇∥ {stats.get('enc_grad_norm', float('nan')):.3e} | "
                    f"lam_align {self._lam_align:.2f} lam_kl {self._lam_kl:.2f} | "
...
def _plot_heat_grid(
    ...
    Ks=(1,2,3,4,8), bins=160, xlim=None, ylim=None, cmap="magma",
...
def main1(target="checker", embedding="identity", K=2, lam_align: float = 1.0):
    """
    ...
    avrc = AVRC(AVRCConfig(
        ...
        lam_align_start=1.0, lam_align_end=1.0,
        enc_lr_decay=.975,
        viz_latent=True,
        viz_proj_mode="pca",
...
def parse_periodic_round_lines(lines: List[str]) -> List[Dict[str, Any]]:
    ...
    for ln in lines:
        ...
        # disp removed from logs; keep placeholder parse off
        d["align"] = grab(r"\s+align\s+(" + FLOAT + r")")
        d["align_unw"] = grab(r"\s+align_unw\s+(" + FLOAT + r")")
        d["kl"]    = grab(r"\s+kl\s+(" + FLOAT + r")")

        # lam_disp removed from logs
...
def parse_alignment_endpoints(lines: List[str]) -> List[Dict[str, Any]]:
    out = []
    rx_r   = re.compile(r"\[alignment\]\s+r=(\d+)")
    rx_d0  = re.compile(r"a\(0\)=(" + FLOAT + r")")
    rx_d1  = re.compile(r"a\(1\)=(" + FLOAT + r")")
    ...
    for ln in lines:
        ...
        m1 = rx_d1.search(ln)
        me = rx_ell.search(ln)
        if m0: rd["a0"] = float(m0.group(1))
        if m1: rd["a1"] = float(m1.group(1))
...
def make_all_plots(log_path: str, out_dir: str = "training_plots") -> Path:
    ...
    disper   = parse_alignment_endpoints(lines)
    ...
    if rounds:
        ...
        simple_curve(xs, ys, "round", "value",
                     "Alignment / KL vs round",
                     outdir/"training_align_kl.png", legend=True)
        ys = {
    ...
    # Alignment endpoints & E||ℓ||
    if disper:
        xs = [d["round"] for d in disper]
        d0 = [d.get("a0") for d in disper]
        d1 = [d.get("a1") for d in disper]
        y_max = None
        if d0 and d0[0] is not None:
            y_max = d0[0] * 1.05
        plt.figure(figsize=(7.0, 4.2), dpi=140)
        plt.plot(xs, d0, label="a(0)", linewidth=2)
        plt.plot(xs, d1, label="a(1)", linewidth=2)
        if y_max is not None:
            plt.ylim(0, y_max)
        plt.xlabel("round"); plt.ylabel("alignment endpoint (1 - cos)")
        plt.title("Alignment endpoints vs round")
        plt.legend(); plt.grid(alpha=0.3); plt.tight_layout()
        plt.savefig(outdir/"alignment_endpoints_over_rounds.png"); plt.close()
...
def meta_run_examples(examples: list[tuple[str, str, int]]):
    ...
    for ex in examples:
        ...
        with open(log_path, "w", encoding="utf-8") as lf, \
            ...
            # Alignment (no projection)
            make_movie("Alignment movie",
                       f"{cross_dir}/alignment_*.png",
                       f"{cross_dir}/alignment_evolution.mp4", 5)
...
if __name__ == "__main__":
    main1('8g', embedding = 'identity', K = 2)
    ...
    parser.add_argument(
        "--examples",
        type=str,
        default="[('rings','identity', 2)],[('checker','identity', 2)], [('moons','identity', 2)], [('spiral','identity', 2)]",
