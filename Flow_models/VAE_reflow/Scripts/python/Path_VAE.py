...
class AVRC:
    ...
    def __init__(self, cfg: AVRCConfig = AVRCConfig()):
        ...
        # ---------------------- viz / rf state ----------------------
        self.posthoc_rf_sampler = None              # decoded RF sampler (you already had this)
        self.posthoc_rf_latent_sampler = None       # <--- add
        self.posthoc_rf_v_model = None              # <--- add
    ...
    def _time_warp(self, t: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Sinusoidal per-sample time jitter, α(t;φ), α̇(t;φ) as in the time-warped
        rectified VAE note. If cfg.jitter_lambda == 0 → identity. :contentReference[oaicite:0]{index=0}
        """
        import math
        ...
        two_pi = 2.0 * math.pi
        # one random phase per sample
        phi = two_pi * torch.rand_like(t)

        # α(t) = t + (ε / 2π)[sin(2πt + φ) − sin φ]
    ...
    def export_latent_path_animation(
        self,
        out_mp4: str = "latent_latent_evolution.mp4",
        frames: int = 120,
        hist_bins: int = 160,
        fps: int = 30,
        ...
        """
        Make an MP4 of the latent path z_t = (1 - t) z_0 + t z_1, t ∈ [0,1],
        where z_0 is encoder-sampled and z_1 = μ(x). Each frame is a 2D
        histogram of the first two latent dims at that t.

        Additionally: pick `trace_n` points from the batch and, on top of the
        histogram, draw their trajectory from z_0 to current z_t in cyan.
        At t=0: only cyan dots at z_0.
        At t=1: full straight segments z_0 → z_1 are shown.
        """
        import os, shutil
        import numpy as np
        ...
        device  = self.device
        tmp_dir = tmp_dir or getattr(self.cfg, "viz_anim_dir", "viz_latent_anim")
        os.makedirs(tmp_dir, exist_ok=True)

        # ----- get a consistent latent batch -----
        N = int(batch_n or getattr(self.cfg, "viz_hist_n", 8192))
        batch = self._get_viz_logging_batch(N)
        z0 = batch["model_z0"]            # (N, D)
        x_for_mu = batch["x_tgt_enc"]     # (N, D)

        mu, _, _ = unpack_encoder_output(self.Enc(x_for_mu))  # z1 = μ(x)
        D = z0.size(1)
        if D < 2:
            raise ValueError("latent dim D<2, cannot make 2D histogram animation")

        # ----- pick persistent subset to trace -----
        trace_n = min(trace_n, N)
        perm = torch.randperm(N, device=device)
        trace_idx = perm[:trace_n]
        z0_tr = z0[trace_idx, :2].detach().cpu().numpy()   # (trace_n, 2)
        z1_tr = mu[trace_idx, :2].detach().cpu().numpy()   # (trace_n, 2)

        # ----- global 2D window so frames don't jump -----
        both = torch.cat([z0[:, :2], mu[:, :2]], dim=0)  # (2N, 2)
        q_lo = both.quantile(0.001, dim=0)
        q_hi = both.quantile(0.999, dim=0)
        pad  = 0.05 * (q_hi - q_lo)
        xlim = (float(q_lo[0] - pad[0]), float(q_hi[0] + pad[0]))
        ylim = (float(q_lo[1] - pad[1]), float(q_hi[1] + pad[1]))

        # ----- prepare figure ONCE (fixed size) -----
        plt.ioff()
        fig, ax = plt.subplots(figsize=(5.0, 5.0))
        H0 = np.zeros((hist_bins, hist_bins), dtype=np.float32)
        im = ax.imshow(
            H0,
            origin="lower",
            extent=[xlim[0], xlim[1], ylim[0], ylim[1]],
        ...
        # create line artists for each traced point
        trace_lines = []
        for k in range(trace_n):
            # start with zero-length line at z0
            ln, = ax.plot(
                [z0_tr[k, 0], z0_tr[k, 0]],
                [z0_tr[k, 1], z0_tr[k, 1]],
                color="cyan",
        ...
        # moving scatter for current positions
        scat = ax.scatter(
            z0_tr[:, 0],
            z0_tr[:, 1],
            c="cyan",
        ...
        ts = torch.linspace(0.0, 1.0, steps=frames, device=device, dtype=z0.dtype)
        frame_paths: list[str] = []

        for i, t in enumerate(ts.tolist()):
            # full batch for histogram
            zt = (1.0 - t) * z0 + t * mu
            zt2 = zt[:, :2].detach().cpu().numpy()
            ...
            im.set_data(H.T)
            im.set_clim(vmin=H.min(), vmax=H.max() if H.max() > 0 else 1.0)
            ax.set_title(f"latent z_t  (t = {t:0.3f})")

            # update traced paths
            cur_tr = (1.0 - t) * z0_tr + t * z1_tr   # (trace_n, 2)
            for k in range(trace_n):
                # line from start to current
                trace_lines[k].set_data(
                    [z0_tr[k, 0], cur_tr[k, 0]],
                    [z0_tr[k, 1], cur_tr[k, 1]],
                )
            scat.set_offsets(cur_tr)

            frame_path = os.path.join(tmp_dir, f"frame_{i:04d}.png")
            fig.savefig(frame_path, dpi=120)   # fixed canvas
        ...
        with imageio.get_writer(out_mp4, fps=fps, codec="libx264") as writer:
            ...
            print(f"[anim] latent path animation saved → {out_mp4} and temp dir wiped.")
    ...
    @torch.no_grad()
    def export_decoder_path_benchmark(
        self,
        ...
        Post-training benchmark:
          line: MMD(D(z_t,t), x1) for t in (0,1] on a fine grid (n_line points)
          hists: same 3 paths but only for user-specified t_list
        """
        import os, math
        import numpy as np
        import matplotlib.pyplot as plt
        ...
        # master times we must transport to
        ...
        for tr in range(trials):
            ...
            for i, t in enumerate(t_line):
                t_tensor = torch.full((N, 1), float(t), device=device, dtype=TDTYPE)

                x_oracle = self.Dec(oracle_latents[t], t_tensor)
                mmd_val = mmd_rbf_nd(x_oracle, x1, max_n=self.cfg.log_mmd_max_n)
                mmd_oracle_line[i] += mmd_val

                x_rf_enc = self.Dec(rf_enc_latents[t], t_tensor)
                mmd_val = mmd_rbf_nd(x_rf_enc, x1, max_n=self.cfg.log_mmd_max_n)
                mmd_rf_enc_line[i] += mmd_val

                x_rf_gauss = self.Dec(rf_gauss_latents[t], t_tensor)
            ...
            for t in t_hist:
                t_tensor = torch.full((N, 1), float(t), device=device, dtype=TDTYPE)
                last_oracle_dec_hist.append(self.Dec(oracle_latents[t], t_tensor).detach().cpu())
                last_rf_enc_dec_hist.append(self.Dec(rf_enc_latents[t], t_tensor).detach().cpu())
                last_rf_gauss_dec_hist.append(self.Dec(rf_gauss_latents[t], t_tensor).detach().cpu())
        ...
        plt.plot(t_line, mmd_oracle_line, color="green", label="oracle D(z_t,t)")
        plt.plot(t_line, mmd_rf_enc_line, color="red", label="RF(enc)→D")
        plt.plot(t_line, mmd_rf_gauss_line, color="blue", label="RF(gauss)→D")
        plt.xlabel("t")
        plt.ylabel("MMD to x₁")
        plt.title("Decoder fidelity vs t (fine)")
        plt.grid(True, alpha=0.25)
        plt.legend()
        line_path = os.path.join(out_dir, f"decoder_path_mmd{tag}.png")
        plt.tight_layout()
        ...
        q = 0.997
        xlim = (np.quantile(x1_proj[:, 0], 1 - q), np.quantile(x1_proj[:, 0], q))
        ylim = (np.quantile(x1_proj[:, 1], 1 - q), np.quantile(x1_proj[:, 1], q))
        xlim = [v * 1.2 for v in xlim]
        ylim = [v * 1.2 for v in ylim]
        ...
        for j, t in enumerate(t_hist):
            ...
            _panel(axes[1, j], H_oracle[j], "oracle" if j == 0 else "")
            _panel(axes[2, j], H_rf_enc[j], "RF(enc)" if j == 0 else "")
            _panel(axes[3, j], H_rf_gauss[j], "RF(gauss)" if j == 0 else "")

        plt.tight_layout()
        hist_path = os.path.join(out_dir, f"decoder_path_hists{tag}.png")
    ...
    def _save_recon_over_time_frame(self, round_idx: int, t_steps: int = 64):
        ...
        out_dir = self.cfg.viz_cross_dir  # you already use this for crossings = "viz_crossings3d"
        os.makedirs(out_dir, exist_ok=True)

        # 1) grab a stable evaluation batch (like hist grid)
        N = int(getattr(self.cfg, "viz_hist_n", 8192))
        batch = self._get_viz_logging_batch(N)
        x_tgt_enc = batch["x_tgt_enc"]       # (N, D_data)
        z0        = batch["model_z0"]        # (N, D_latent)
        mu_0, _, _ = unpack_encoder_output(self.Enc(x_tgt_enc))  # z1 = μ(x)
        ...
        for t in t_grid:
            ...
            t_tensor = torch.full(
                (N, 1),
                float(t_val if t_val > 0.0 else self.t_eps),
                device=self.device,
                dtype=TDTYPE,
            )
        ...
        for r in range(1, rounds + 1):
            ...
            if self.cfg.k_plot and r % self.cfg.k_plot == 0:
                try:
                    ...
                    # NEW
                    self._save_recon_over_time_frame(round_idx=r)
                except Exception as exc:
...
def main1(target="checker", embedding="identity", K=2):
    ...
    # NEW: latent-path animation (120 frames, 30fps)
