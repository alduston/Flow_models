...
class ExperimentConfig:
    ...
    # Self-consistency diagnostic: compare the sampler score s^k to an
    # empirical Tweedie estimate of the OU score of the law induced by s^k.
    # This is deliberately separate from fisher_rmse, which compares s^k to
    # the true target OU score.
    self_consistency_bank_n: int = 0
    self_consistency_n_t: int = 16
    self_consistency_n_per_t: int = 2048
    self_consistency_t_min: float = 0.05
    self_consistency_t_max: Optional[float] = None  # defaults to t_start
    self_consistency_time_grid: str = "log"  # log or linear
    self_consistency_ref_block: int = 4096
    self_consistency_eval_chunk: int = 256
    ...
    plot_histogram_array: bool = True
...
METRIC_ALIASES: Dict[str, str] = {
    ...
    "self_consistency": "self_rmse",
    "self_consistency_rmse": "self_rmse",
    "self_score_rmse": "self_rmse",
    "self_mse": "self_rmse",
}

AVAILABLE_METRICS: Tuple[str, ...] = ("nll", "ksd", "mmd", "sw2", "kl", "kl_rev", "fisher_rmse", "self_rmse")
...
def integrated_score_fisher_metric(score_fn: Callable[[torch.Tensor, float], torch.Tensor], metric_ctx: MetricContext, cfg: ExperimentConfig) -> Dict[str, float]:
    ...
    for t_tensor, y, s_true in zip(metric_ctx.fisher_times, metric_ctx.fisher_y, metric_ctx.fisher_score):
        t = float(t_tensor.item())
        s_hat = score_fn(y, t)
        l2 = torch.linalg.norm(s_hat - s_true, dim=1)
        mse_vals.append(torch.mean(l2 ** 2))
        l2_vals.append(torch.mean(l2))
    mse = float(torch.mean(torch.stack(mse_vals)).item())
    l2_mean = float(torch.mean(torch.stack(l2_vals)).item())
    rmse = float(math.sqrt(max(mse, 0.0)))
    return {"fisher_rmse": rmse, "target_l2": l2_mean}


def make_self_consistency_time_grid(cfg: ExperimentConfig, device: torch.device, dtype: torch.dtype) -> torch.Tensor:
    t_min = max(float(cfg.self_consistency_t_min), 1.0e-8)
    t_max = cfg.t_start if cfg.self_consistency_t_max is None else float(cfg.self_consistency_t_max)
    t_max = max(float(t_max), t_min)
    n_t = int(cfg.self_consistency_n_t)
    if n_t <= 1:
        return torch.tensor([t_max], device=device, dtype=dtype)
    if cfg.self_consistency_time_grid == "log":
        return torch.exp(torch.linspace(math.log(t_min), math.log(t_max), n_t, device=device, dtype=dtype))
    if cfg.self_consistency_time_grid == "linear":
        return torch.linspace(t_min, t_max, n_t, device=device, dtype=dtype)
    raise ValueError("self_consistency_time_grid must be 'log' or 'linear'")


class EmpiricalTweedieScore:
    """Tweedie score of the OU-smoothed empirical law supported on x0_samples.

    For x0_i sampled from the law induced by s^k, this estimates
    s^{k*}_t(y) = ∇ log [(K_t * μ_k)(y)] by the equal-weight empirical
    Tweedie formula.  It only uses the induced samples as atoms; no target score
    or curvature is queried here.
    """

    def __init__(self, x0_samples: torch.Tensor, ref_block: int = 4096, eval_chunk: int = 256):
        self.x = x0_samples.detach()
        self.device = self.x.device
        self.dtype = self.x.dtype
        self.ref_block = max(1, int(ref_block))
        self.eval_chunk = max(1, int(eval_chunk))

    @torch.no_grad()
    def estimate_chunk(self, y: torch.Tensor, t: float) -> torch.Tensor:
        t_tensor = torch.as_tensor(float(t), device=self.device, dtype=self.dtype)
        alpha, gamma = alpha_gamma(t_tensor)
        gamma = torch.clamp(gamma, min=torch.as_tensor(1.0e-12, device=self.device, dtype=self.dtype))

        q = y.shape[0]
        block_max = torch.full((q,), -float("inf"), device=self.device, dtype=self.dtype)
        block_sumexp = torch.zeros((q,), device=self.device, dtype=self.dtype)
        block_x_num = torch.zeros((q, self.x.shape[1]), device=self.device, dtype=self.dtype)

        for r0 in range(0, self.x.shape[0], self.ref_block):
            xr = self.x[r0:r0 + self.ref_block]
            diff = y[:, None, :] - alpha * xr[None, :, :]
            logw = -0.5 * torch.sum(diff * diff, dim=-1) / gamma
            local_max = torch.max(logw, dim=1).values
            new_max = torch.maximum(block_max, local_max)
            old_scale = torch.exp(block_max - new_max)
            w_scaled = torch.exp(logw - new_max[:, None])

            block_sumexp = block_sumexp * old_scale + torch.sum(w_scaled, dim=1)
            block_x_num = block_x_num * old_scale[:, None] + w_scaled @ xr
            block_max = new_max

        mean_x = block_x_num / torch.clamp(block_sumexp[:, None], min=1.0e-300)
        return (alpha * mean_x - y) / gamma

    @torch.no_grad()
    def estimate(self, y: torch.Tensor, t: float) -> torch.Tensor:
        outs: List[torch.Tensor] = []
        for start in range(0, y.shape[0], self.eval_chunk):
            outs.append(self.estimate_chunk(y[start:start + self.eval_chunk], t))
        return torch.cat(outs, dim=0)


@torch.no_grad()
def sample_induced_bank(
    target: GMM2D,
    score_fn: Callable[[torch.Tensor, float], torch.Tensor],
    cfg: ExperimentConfig,
    n_samples: int,
    ...
    """Generate a large empirical bank from the law induced by score_fn."""
    sample_cfg = replace(cfg, n_samples=int(n_samples))
    return reverse_ou_heun_sde(target, score_fn, sample_cfg, generator=generator)


@torch.no_grad()
def integrated_self_consistency_metric(
    target: GMM2D,
    score_fn: Callable[[torch.Tensor, float], torch.Tensor],
    cfg: ExperimentConfig,
    generator: torch.Generator,
) -> Tuple[Dict[str, float], List[Dict[str, float]]]:
    """Compare s^k to s^{k*}, the score of its own induced sample law.

    The induced-law score is estimated as the Tweedie score of a large empirical
    bank sampled by running the reverse OU sampler with s^k.  Probes are drawn
    from that same empirical law diffused to each t, so the metric estimates

        E_{t, X_0~μ_k, X_t~K_t(.|X_0)} || s^k_t(X_t) - s^{k*}_t(X_t) ||^2.

    Use self_consistency_t_min≈0.05 to avoid the high-variance empirical
    Tweedie regime near t=0.
    """
    if not metric_enabled(cfg, "self_rmse"):
        return {}, []
    bank_n = int(cfg.self_consistency_bank_n)
    if bank_n <= 0:
        return {"self_rmse": float("nan")}, []

    induced_samples, sampler_info = sample_induced_bank(
        target=target,
        score_fn=score_fn,
        cfg=cfg,
        n_samples=bank_n,
    ...
    tweedie_star = EmpiricalTweedieScore(
        induced_samples,
        ref_block=cfg.self_consistency_ref_block,
        eval_chunk=cfg.self_consistency_eval_chunk,
    )

    times = make_self_consistency_time_grid(cfg, target.device, target.dtype)
    n_probe = max(1, int(cfg.self_consistency_n_per_t))
    mse_vals: List[torch.Tensor] = []
    l2_vals: List[torch.Tensor] = []
    per_t_rows: List[Dict[str, float]] = []

    for t_tensor in times:
        t = float(t_tensor.item())
        alpha, gamma = alpha_gamma(torch.as_tensor(t, device=target.device, dtype=target.dtype))
        idx = torch.randint(
            induced_samples.shape[0],
            (n_probe,),
            device=target.device,
            generator=generator,
        )
        x0 = induced_samples[idx]
        noise = torch.randn(x0.shape, device=target.device, dtype=target.dtype, generator=generator)
        y = alpha * x0 + torch.sqrt(gamma) * noise

        s_k = score_fn(y, t)
        s_star = tweedie_star.estimate(y, t)
        l2 = torch.linalg.norm(s_k - s_star, dim=1)
        mse_t = torch.mean(l2 ** 2)
        l2_t = torch.mean(l2)
        mse_vals.append(mse_t)
        l2_vals.append(l2_t)
        per_t_rows.append({
            "t": float(t),
            "self_mse_t": float(mse_t.item()),
            "self_rmse_t": float(math.sqrt(max(float(mse_t.item()), 0.0))),
            "self_l2_t": float(l2_t.item()),
            "self_mean_norm_s_k_t": float(torch.linalg.norm(s_k, dim=1).mean().item()),
            "self_mean_norm_s_star_t": float(torch.linalg.norm(s_star, dim=1).mean().item()),
        })

    mse = float(torch.mean(torch.stack(mse_vals)).item()) if mse_vals else float("nan")
    l2_mean = float(torch.mean(torch.stack(l2_vals)).item()) if l2_vals else float("nan")
    out = {
        "self_rmse": float(math.sqrt(max(mse, 0.0))) if math.isfinite(mse) else float("nan"),
        "self_mse": mse,
        "self_l2": l2_mean,
        "self_bank_n": float(bank_n),
        "self_probe_n_per_t": float(n_probe),
        "self_t_min": float(cfg.self_consistency_t_min),
        "self_t_max": float(cfg.t_start if cfg.self_consistency_t_max is None else cfg.self_consistency_t_max),
        "self_sampler_failed": bool(sampler_info.get("failed", False)),
        "self_sampler_max_abs_score": safe_float(sampler_info.get("max_abs_score", float("nan"))),
    }
    return out, per_t_rows
...
def bootstrap_family_name(method: str) -> str:
    method = str(method)
    if method in {"PERTURBED_REF", "ORACLE"}:
        return method
    parts = [p.strip() for p in method.split("_") if p.strip()]
...
def _numeric_series(df: pd.DataFrame, col: str) -> pd.Series:
    """Small helper so plot functions tolerate missing/non-numeric columns."""
    if col not in df.columns:
        return pd.Series(np.nan, index=df.index, dtype=float)
    return pd.to_numeric(df[col], errors="coerce")


def plot_bootstrap_score_diagnostics(metrics_df: pd.DataFrame, outpath: str) -> None:
    """Overlay self-consistency and target-consistency score errors vs bootstrap depth.

    Solid lines are self-consistency errors E||s^k - s^{k*}||, where s^{k*}
    is the empirical Tweedie score of the law induced by sampling with s^k.
    Dashed lines are target-consistency errors E||s^k - s_target||.

    If both L2 and RMSE columns are available, the figure has two panels:
    one for mean L2 and one for RMSE.  Color identifies the method family
    (HLSI vs CE-HLSI); linestyle identifies self vs target consistency.
    """
    if metrics_df.empty or "bootstrap_depth" not in metrics_df.columns:
        return

    df = metrics_df.copy()
    df = df[~df["method"].isin(["PERTURBED_REF", "ORACLE"])]
    if df.empty:
        return

    df["family"] = df["method"].map(bootstrap_family_name)
    df["bootstrap_depth"] = pd.to_numeric(df["bootstrap_depth"], errors="coerce")
    df = df[df["bootstrap_depth"].notna()]
    if df.empty:
        return

    # Each tuple is (panel_title, self_metric, target_metric, y_label).
    panels = []
    if "self_l2" in df.columns and "target_l2" in df.columns:
        panels.append((
            "Mean L2 score error",
            "self_l2",
            "target_l2",
            r"$\mathbb{E}_{t,x_t}\|s^k-s^{\bullet}\|$",
        ))
    if "self_rmse" in df.columns and "fisher_rmse" in df.columns:
        panels.append((
            "Score RMSE",
            "self_rmse",
            "fisher_rmse",
            r"$\sqrt{\mathbb{E}_{t,x_t}\|s^k-s^{\bullet}\|^2}$",
        ))
    if not panels:
        # Backward-compatible fallback if only one side of the diagnostic exists.
        metric_names = [m for m in ["target_l2", "self_l2", "fisher_rmse", "self_rmse"] if m in df.columns]
        if not metric_names:
            return
        panels = [(m, m, None, m) for m in metric_names]

    sigmas = sorted(df["perturb_sigma"].unique()) if "perturb_sigma" in df.columns else [None]
    show_err = cfg_n_trials_from_df(metrics_df) > 1

    ncols = len(panels)
    fig, axes = plt.subplots(1, ncols, figsize=(6.2 * ncols, 4.5), constrained_layout=True)
    axes = np.array(axes).reshape(-1)

    for ax, (title, self_metric, target_metric, ylabel) in zip(axes, panels):
        for sigma in sigmas:
            sub_sigma = df if sigma is None else df[df["perturb_sigma"] == sigma]
            for family in sub_sigma["family"].dropna().unique():
                sub = sub_sigma[sub_sigma["family"] == family].sort_values("bootstrap_depth")
                if sub.empty:
                    continue

                label_prefix = f"{family}" if len(sigmas) == 1 else f"{family}, sigma={float(sigma):g}"
                x = pd.to_numeric(sub["bootstrap_depth"], errors="coerce")

                # Solid: self-consistency.
                y_self = _numeric_series(sub, self_metric)
                plotted_color = None
                if y_self.notna().sum() > 0:
                    yerr_col = f"{self_metric}_sem"
                    if show_err and yerr_col in sub.columns:
                        yerr = _numeric_series(sub, yerr_col)
                        line = ax.errorbar(
                            x,
                            y_self,
                            yerr=yerr,
                            ...
                            linewidth=1.9,
                            capsize=2,
                            label=f"{label_prefix}: self",
                        )
                        plotted_color = line.lines[0].get_color()
                    else:
                        (line,) = ax.plot(
                            x,
                            y_self,
                ...
                # Dashed: target-consistency, reusing the family color.
                if target_metric is not None:
                    y_target = _numeric_series(sub, target_metric)
                    if y_target.notna().sum() > 0:
                        yerr_col = f"{target_metric}_sem"
                        plot_kwargs = dict(marker="s", linestyle="--", linewidth=1.9, label=f"{label_prefix}: target")
                        if plotted_color is not None:
                            plot_kwargs["color"] = plotted_color
                        if show_err and yerr_col in sub.columns:
                            yerr = _numeric_series(sub, yerr_col)
                            ax.errorbar(x, y_target, yerr=yerr, capsize=2, **plot_kwargs)
                        else:
                            ax.plot(x, y_target, **plot_kwargs)

        ax.set_xlabel("bootstrap round k")
        ax.set_ylabel(ylabel)
        ax.set_title(title + " vs bootstrap round")
        ax.grid(True, alpha=0.25)
        ax.set_xticks(sorted(df["bootstrap_depth"].dropna().unique()))

    # A single legend on the first panel is enough; solid/dashed encoding is
    # repeated in every panel.
    axes[0].legend(fontsize=8, ncol=1)
    fig.suptitle("Bootstrap self-consistency (solid) vs target-consistency (dashed)", fontsize=13)
    fig.savefig(outpath, dpi=190)
    plt.close(fig)


def plot_self_consistency_by_time(self_df: pd.DataFrame, outpath: str) -> None:
    per_time_metric = "self_l2_t" if "self_l2_t" in self_df.columns else "self_rmse_t"
    if self_df.empty or per_time_metric not in self_df.columns:
        return
    df = self_df.copy()
    df["family"] = df["method"].map(bootstrap_family_name)
    df["bootstrap_depth"] = pd.to_numeric(df["bootstrap_depth"], errors="coerce")
    show_err = cfg_n_trials_from_df(df) > 1

    families = list(df["family"].dropna().unique())
    ncols = min(2, max(1, len(families)))
    nrows = int(math.ceil(max(1, len(families)) / ncols))
    fig, axes = plt.subplots(nrows, ncols, figsize=(5.8 * ncols, 4.3 * nrows), constrained_layout=True)
    axes = np.array(axes).reshape(-1)
    for ax, family in zip(axes, families):
        fam_df = df[df["family"] == family]
        for depth in sorted(fam_df["bootstrap_depth"].dropna().unique()):
            sub = fam_df[fam_df["bootstrap_depth"] == depth].sort_values("t")
            y = pd.to_numeric(sub[per_time_metric], errors="coerce")
            if y.notna().sum() == 0:
                continue
            yerr_col = f"{per_time_metric}_sem"
            if show_err and yerr_col in sub.columns:
                yerr = pd.to_numeric(sub[yerr_col], errors="coerce")
                ax.errorbar(sub["t"], y, yerr=yerr, marker="o", linewidth=1.5, capsize=2, label=f"k={depth:g}")
            else:
                ax.plot(sub["t"], y, marker="o", linewidth=1.5, label=f"k={depth:g}")
        ax.set_xscale("log")
        ax.set_xlabel("diffusion time t")
        ax.set_ylabel("per-time self E||s^k - s_k*||" if per_time_metric == "self_l2_t" else r"per-time self RMSE")
        ax.set_title(f"{family}: self-consistency by t")
        ax.grid(True, alpha=0.25)
...
def run_experiment(cfg: ExperimentConfig) -> None:
    ...
    self_rows: List[Dict[str, float | str | int | bool]] = []
    refs_by_sigma: Dict[float, torch.Tensor] = {}
    samples_by_sigma: Dict[float, Dict[str, torch.Tensor]] = {}
    ...
    for sigma_idx, sigma in enumerate(cfg.perturb_sigmas):
        ...
        for trial_idx in range(cfg.n_trials):
            ...
            ref_row = {
                ...
                **({"self_rmse": float("nan"), "self_mse": float("nan")} if metric_enabled(cfg, "self_rmse") else {}),
                **ref_metrics,
            }
            ...
            for method_idx, method in enumerate(methods):
                ...
                self_metrics, self_per_t_rows = integrated_self_consistency_metric(
                    target=target,
                    score_fn=score_fn,
                    cfg=cfg,
                    generator=make_generator(trial_seed + 73737 + 1009 * method_idx + 101 * sigma_idx, device),
                )
                row = {
                    "perturb_sigma": float(sigma),
                    "trial": int(trial_idx),
                ...
                for self_row in self_per_t_rows:
                    self_rows.append({
                        "perturb_sigma": float(sigma),
                        "trial": int(trial_idx),
                        "trial_seed": int(trial_seed),
                        "method": method,
                        "n_ref": int(cfg.n_ref),
                        "bootstrap_depth": int(run_info.get("bootstrap_depth", method_bootstrap_depth(method))),
                        "execution_order": str(run_info.get("execution_order", "")),
                        "final_stage_method": str(run_info.get("final_stage_method", "")),
                        "corruption_mode": cfg.corruption_mode,
    ...
    self_trials_df = pd.DataFrame(self_rows)
    self_df = aggregate_trials(self_trials_df, ["perturb_sigma", "method", "t"]) if not self_trials_df.empty else self_trials_df
    ...
    self_trials_csv = os.path.join(cfg.outdir, "self_consistency_trials.csv")
    self_csv = os.path.join(cfg.outdir, "self_consistency.csv")
    ...
    self_trials_df.to_csv(self_trials_csv, index=False)
    self_df.to_csv(self_csv, index=False)
    ...
    plot_bootstrap_score_diagnostics(metrics_df, os.path.join(cfg.outdir, "bootstrap_score_diagnostics.png"))
    plot_self_consistency_by_time(self_df, os.path.join(cfg.outdir, "self_consistency_by_time.png"))
    if cfg.plot_histogram_array:
    ...
    if not ess_df.empty:
        # Keep this simple: constrained_layout occasionally becomes very slow
        # on log axes in headless batch runs.
    ...
    print(
        ...
        f"  {self_csv}                 # trial-averaged self-consistency by t\n"
        f"  {self_trials_csv}          # raw per-trial self-consistency by t\n"
        f"  {cfg.outdir}"
    )
...
def parse_args() -> ExperimentConfig:
    ...
    p.add_argument(
        ...
        help=f"Metrics to compute. Use any of: {', '.join(AVAILABLE_METRICS)}. Aliases accepted: sliced_w2->sw2, score_rmse->fisher_rmse, self_consistency_rmse->self_rmse, old KL names->kl/kl_rev.",
    )
    ...
    p.add_argument("--self_consistency_bank_n", type=int, default=ExperimentConfig.self_consistency_bank_n, help="If metric self_rmse is requested, generate this many extra samples from each method score field to estimate s^{k*} with empirical Tweedie. Set 0 to disable/return NaN.")
    p.add_argument("--self_consistency_n_t", type=int, default=ExperimentConfig.self_consistency_n_t)
    p.add_argument("--self_consistency_n_per_t", type=int, default=ExperimentConfig.self_consistency_n_per_t)
    p.add_argument("--self_consistency_t_min", type=float, default=ExperimentConfig.self_consistency_t_min, help="Lower t cutoff for s^k vs s^{k*}; keep around 0.05 or larger to avoid high-variance empirical Tweedie near t=0.")
    p.add_argument("--self_consistency_t_max", type=float, default=None)
    p.add_argument("--self_consistency_time_grid", type=str, choices=["log", "linear"], default=ExperimentConfig.self_consistency_time_grid)
    p.add_argument("--self_consistency_ref_block", type=int, default=ExperimentConfig.self_consistency_ref_block)
    p.add_argument("--self_consistency_eval_chunk", type=int, default=ExperimentConfig.self_consistency_eval_chunk)
    ...
    p.add_argument("--no_histogram_array", action="store_true", help="Skip the all-method/all-sigma histogram grid; useful for score-only bootstrap diagnostics.")
    args = p.parse_args()
    ...
    return ExperimentConfig(
        ...
        self_consistency_bank_n=args.self_consistency_bank_n,
        self_consistency_n_t=args.self_consistency_n_t,
        self_consistency_n_per_t=args.self_consistency_n_per_t,
        self_consistency_t_min=args.self_consistency_t_min,
        self_consistency_t_max=args.self_consistency_t_max,
        self_consistency_time_grid=args.self_consistency_time_grid,
        self_consistency_ref_block=args.self_consistency_ref_block,
        self_consistency_eval_chunk=args.self_consistency_eval_chunk,
        ...
        plot_histogram_array=not args.no_histogram_array,
    )
