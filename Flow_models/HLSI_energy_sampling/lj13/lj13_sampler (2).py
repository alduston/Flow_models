...
Original file is located at
    https://colab.research.google.com/drive/19qA13__72Ab1YxdT6LSaZAKE0ujxDjPA
"""
...
# -*- coding: utf-8 -*-
"""
lj13_sampler.py  —  HLSI / CE-HLSI / Leaf-HLSI score estimators on LJ-13.

LJ-13 benchmark: 13 Lennard-Jones particles in R^3, total dimension d = 39.
  U(x) = 4ε · Σ_{i<j} [(σ/r_{ij})^12 − (σ/r_{ij})^6]
  log p(x) = −U(x)  (unnormalized)
  ε = σ = 1  (dimensionless, matching the iDEM benchmark)

Distance singularity: numerical stabiliser d² ≥ 1e-8 only (iDEM convention).
No physical soft core.

MALA initialised from the LJ-13 icosahedral global minimum (energy ≈ −44.3ε),
step_size=3e-5 (targeting ~67% acceptance; Tr(H)≈6750 → E[ΔU]=0.4 per step).

Additional surrogate-transition ablations
──────────────────────────────────────────
  Surrogate-HLSI              : HLSI with exact local-surrogate transition weights
  Surrogate-Leaf-CE-HLSI      : Leaf-CE-HLSI with exact leaf-surrogate transition weights
  Surrogate-Adaptive Leaf-CE  : Adaptive Leaf-CE with exact adaptive-leaf surrogate weights

Visualisations produced
───────────────────────
  lj13_metrics_bar.png            : Bar chart of all metrics per method
  lj13_2d_heatmaps_xy/xz/yz.png  : 2D particle density (3 projections), GT vs each method
  lj13_pairwise_dist.png          : Pairwise inter-particle distance histograms (g(r))
  lj13_radial_from_com.png        : Per-particle radial distance from CoM
  lj13_energy_dist.png            : Energy (LJ potential) distribution
  lj13_copying.png                : Per-method copying detector
  lj13_copying_ratio.png          : Summary copying ratio bar chart
"""
