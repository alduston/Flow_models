...
────────────────
  ...
  Adaptive Leaf-CE : CE-HLSI with leaf-rescued non-PSD dirs & adaptive p_leaf
  Surrogate-HLSI : HLSI with exact local-surrogate transition weights instead of OU weights
  Surrogate-Leaf-CE-HLSI : Leaf-CE-HLSI with exact leaf-surrogate transition weights
"""
...
def run(target, out_dir='outputs', methods=None):
    ...
    """
    if methods is None:
        methods = ['Surrogate-Leaf-CE-HLSI', 'Surrogate-HLSI', 'Adaptive Leaf-CE', 'Leaf-CE-HLSI', 'CE-HLSI', 'HLSI', 'Blended', 'Tweedie']
    ...
    def make_fn(meth):
        def fn(y, t):
            t  = t if isinstance(t, torch.Tensor) else \
                 torch.tensor(t, dtype=torch.get_default_dtype())
            w  = snis_w(y, t, xr)
            if   meth == 'Tweedie':
                return est_tweedie(y, t, xr, w)
            elif meth == 'TSI':
                return est_tsi(y, t, xr, w, target, s0_ref=precomp['s0'])
            elif meth == 'HLSI':
                return est_hlsi(y, t, xr, w, target, precomp=precomp)
                ...
                return est_ce_hlsi(y, t, xr, w, target, precomp=precomp)
            elif meth == 'Leaf-HLSI':
                return est_leaf_hlsi(y, t, xr, w, target, precomp=precomp)
            elif meth == 'Leaf-CE-HLSI':
                return est_leaf_ce_hlsi(y, t, xr, w, target, precomp=precomp)
                ...
                return est_blended(y, t, xr, w, target, s0_ref=precomp['s0'])
            elif meth == 'Adaptive Leaf':
                return est_leaf_hlsi(y, t, xr, w, target, precomp=precomp_adaptive)
            elif meth == 'Adaptive Leaf-CE':
                return est_leaf_ce_hlsi(y, t, xr, w, target, precomp=precomp_adaptive)
...
MARKERS = {
    ...
    'Surrogate-Adaptive Leaf-CE': '8', 'GT floor': '*',
}
...
def meta_run(root_dir='outputs_stress_test',
    ...
    if methods is None:
         methods = ['Surrogate-Adaptive Leaf-CE', 'Surrogate-Leaf-CE-HLSI', 'Surrogate-HLSI', 'Adaptive Leaf-CE', 'Leaf-CE-HLSI', 'CE-HLSI', 'HLSI', 'Blended', 'Tweedie']
    ...
    # ---- Relative advantage table: (Blended metric) / (Leaf-CE metric) ----
    print(f"\n\n{'='*80}")
    print("HLSI RELATIVE ADVANTAGE:  Blended / Leaf-CE  (>1 means Leaf-CE is better)")
    print("="*80)
    print(f"  {'Variant':<16s} {'W2':>8} {'KSD':>8} {'KL-E':>8} {'RMSE':>8} {'|s|max':>8}")
    print(f"  {'─'*16} {'─'*8} {'─'*8} {'─'*8} {'─'*8} {'─'*8}")
    for vname in variants:
        res = all_results[vname][0]
        if 'Blended' not in res or 'Adaptive Leaf-CE' not in res:
            continue
        rb = res['Blended']
        rl = res['Adaptive Leaf-CE']
        def ratio(key):
            b, l = rb[key], rl[key]
            if not np.isfinite(b) or not np.isfinite(l) or l < 1e-15:
                return float('nan')
            return b / l
        r_ms = rb['ms'] / max(rl['ms'], 1e-15)
        print(f"  {vname:<16s}"
              f" {ratio('w2'):8.3f}"
              f" {ratio('ksd'):8.3f}"
    ...
    print(f"\n  Ratio > 1 → Leaf-CE outperforms Blended on that metric")
    print(f"  Ratio ≈ 1 → no advantage from Hessian information")
    print(f"  Ratio < 1 → Leaf-CE is worse (HLSI machinery hurts)")
    print("=" * 80)
