# -*- coding: utf-8 -*-
"""Allen--Cahn inverse-problem iterative-transport benchmark.

Runs the same comparison as the Darcy experiment: GN-LFGI with completed flow
ratio, GN-LFGI with static reweighting, local matrix/scalar blends, global
matrix blend, and Tweedie.  Each method performs three transport/ratio rounds
and reports transport rounds 1 and 3.  Flow-ratio nodes move a fresh unweighted
cloud with the completed correction; the static baseline instead carries its
endpoint log weights into the next LFGI transport round.
...
# ==========================================
# Each method alternates a public transport node T_r with a hidden ratio node
# R_r.  Flow-ratio nodes return a new unweighted particle law; the static LFGI
# baseline returns the settled endpoint cloud with log weights.  Either state
# becomes the recursive reference for T_{r+1}.  Only requested T rounds enter
# tables and plots.
...
METHOD_SPECS = OrderedDict([
    ('LFGI', {
        'score': 'lfgi',
        'display': 'GN-LFGI + flow ratio',
    }),
    ('LFGIStatic', {
        'score': 'lfgi',
        'display': 'GN-LFGI + static reweighting',
        'ratio_overrides': {
            'ratio_mode': 'static',
            'steps': 0,
        },
...
def make_iterative_transport_sampler_configs(method_specs, rounds=ITERATIVE_TRANSPORT_ROUNDS):
    """Build recursive T_r/R_r schedules with a per-method ratio policy."""
    configs = OrderedDict()
    for method_key, spec in method_specs.items():
        ...
        for round_idx in range(1, int(rounds) + 1):
            ...
            ratio_cfg.update(RATIO_NODE_COMMON)
            ratio_cfg.update(spec.get('ratio_overrides', {}))
...
run_ctx = init_run_results('allen_cahn_iterative_transport')
...
RUN_COMMAND_HINT = (
    ...
    'IP_ITER_DENSITY_STEPS={density_steps} python allen_cahn_static_lfgi_comparison.py'
).format(
    n_ref=N_REF,
    rounds=ITERATIVE_TRANSPORT_ROUNDS,
    transport_steps=TRANSPORT_STEPS,
    ratio_steps=RATIO_STEPS,
    density_steps=RATIO_NODE_COMMON['density_steps'],
)


dashboard = DashboardPDF(
    DASHBOARD_PDF_PATH,
    title='Allen-Cahn iterative transport dashboard',
)
dashboard.add_text_page(
    'Allen-Cahn iterative transport dashboard',
    [
        f"Created: {datetime.now().isoformat(timespec='seconds')}",
        'This dashboard contains the two canonical saved-results tables plus every PNG diagnostic plot saved in the run directory. Public rows are transport outputs T_r; hidden flow-ratio nodes move fresh unweighted clouds, while the GN-LFGI static baseline carries endpoint log weights into its next transport round.',
        ...
        f'RATIO_NODE_COMMON = {RATIO_NODE_COMMON}',
...
save_reproducibility_log(
    title='Allen–Cahn iterative transport run reproducibility log',
    config={
        'seed': seed,
        'ACTIVE_DIM': ACTIVE_DIM,
        'DEFAULT_N_GEN': DEFAULT_N_GEN,
        ...
        'TRANSPORT_STEPS': TRANSPORT_STEPS,
        'RATIO_STEPS': RATIO_STEPS,
        'RATIO_NODE_COMMON': RATIO_NODE_COMMON,
...
print('\n=== Allen–Cahn iterative transport comparison pipeline complete ===')
