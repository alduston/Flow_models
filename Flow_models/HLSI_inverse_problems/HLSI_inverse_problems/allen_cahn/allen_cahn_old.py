...
Original file is located at
    https://colab.research.google.com/drive/1-bYIgpLSHEIvaoEDjpMf5Vyw0jQg2rbk
"""
...
# -*- coding: utf-8 -*-
"""allen_cahn_ic_wc_ce_hlsi.py
HLSI (Hessian-Laplace Score Identity) posterior sampling for a
2D Allen–Cahn reaction–diffusion initial-condition inverse problem.
Refactored from the advection-diffusion notebook by changing only the
...
PDE:      u_t = eps^2 Δu + u - u^3    on [0,1]^2 with periodic BCs
Unknown:  initial condition u0(x) represented in a KL basis,
          u0(x) = sum_i phi_i(x) alpha_i
Prior:    alpha ~ N(0, I)
Data:     noisy sparse observations of u(x, T_obs)
"""
# One-time: generate KL basis for the Allen–Cahn initial condition prior
import numpy as np
...
# Matérn-like covariance for smooth initial conditions
ell = 0.18
sigma_prior = 1.0
...
# ==========================================
# CONFIGURATION GENERATOR (Allen–Cahn IC inversion)
# ==========================================
N = 32
num_observation = 40
num_truncated_series = 32
seed = 42
print(f"Generating Allen–Cahn configuration for {num_observation} observations...")
key = jax.random.PRNGKey(seed)
all_indices = jnp.arange(N * N)
obs_indices = jax.random.choice(key, all_indices, shape=(num_observation,), replace=False)
obs_indices = np.array(obs_indices)
try:
    df_modes = pd.read_csv('data/AllenCahn_Basis_Modes.csv', header=None)
    if isinstance(df_modes.iloc[0, 0], str):
        df_modes = pd.read_csv('data/AllenCahn_Basis_Modes.csv')
    ...
    raise FileNotFoundError(
        "Could not find 'data/AllenCahn_Basis_Modes.csv'. "
...
print("Initializing JAX Allen–Cahn physics engine...")
# ==========================================
...
try:
    ...
    raise
# ==========================================
# 2. DEFINE FORWARD SOLVER (Allen–Cahn)
# ==========================================
# PDE:  u_t = eps^2 Δu + u - u^3   on [0,1]^2, periodic BC.
# Unknown: initial condition u0(x) represented in the KL basis.
# We use a differentiable IMEX spectral integrator: diffusion implicit,
# local reaction explicit.
# ==========================================
...
def solve_forward(alpha):
    """
    Forward map F(alpha) for Allen–Cahn IC inversion.
    Input:  alpha — KL coefficients, shape (q,)
    Output: y_pred — state values at sensor locations at time OBS_TIME, shape (n_obs,)
    """
    u0 = jnp.reshape(Basis @ alpha, (N, N))
    uT = _propagate_allen_cahn_obs_time(u0)
    y_obs = uT.reshape(-1)[obs_locations]
    return y_obs

def solve_full_state(alpha, t=OBS_TIME):
    """Return the full Allen–Cahn state u(x,t) on the N x N grid."""
    u0 = jnp.reshape(Basis @ alpha, (N, N))
...
print("Allen–Cahn physics engine loaded successfully.")
...
RUN_RESULTS_DIR = os.path.join(RUN_RESULTS_ROOT, f'ac_advect_diff_hlsi_{RUN_TIMESTAMP}')
os.makedirs(RUN_RESULTS_DIR, exist_ok=True)
RUN_RESULTS_STEM = f'ac_advect_diff_hlsi_{RUN_TIMESTAMP}'
...
def save_reproducibility_log(extra_sections=None):
    ...
    lines = []
    lines.append('Allen–Cahn IC HLSI run reproducibility log')
    lines.append('=' * 72)
    ...
    excluded_names = {
        ...
        'X_ref_base', 'X_ref_gnl', 'base_bank', 'gnl_bank', 'mean_fields', 'mean_final_states',
...
# Configuration for Allen–Cahn flow
ACTIVE_DIM = num_truncated_series  # Latent dimension (KL modes)
NOISE_STD = 0.016              # Observation noise std
...
# ==========================================
# 10. EXECUTION: ALLEN–CAHN INVERSE PROBLEM
# ==========================================
print(f"--- Setting up {ACTIVE_DIM}D Allen–Cahn IC inverse problem ---")
import gc
gc.collect()
if torch.cuda.is_available():
    torch.cuda.empty_cache()
...
# ==========================================
# 13. SCIENTIFIC VISUALIZATION (Allen–Cahn specific)
# ==========================================
...
def reconstruct_initial_condition(latents):
    """Map KL coefficients -> initial condition field u0 on the (N, N) grid."""
...
def solve_state_field(alpha_vec, t=OBS_TIME):
    """Propagate the initial condition to time t and return the full state on (N,N)."""
...
true_u0 = reconstruct_initial_condition(alpha_true_lat)[0]
true_uT = solve_state_field(alpha_true_lat, t=OBS_TIME)
norm_true_u0 = np.linalg.norm(true_u0) + 1e-12
norm_true_uT = np.linalg.norm(true_uT) + 1e-12
norm_y_clean = np.linalg.norm(y_clean_np) + 1e-12

print('\n=== Physical parameter space metrics (Initial condition + Allen–Cahn state) ===')
print(f"{'Method':<24} | {'IC RelL2(%)':<12} | {'RMSE_alpha':<12} | {'FinalRel':<12} | {'SensorRel':<12}")
print('-' * 95)
...
for label, samps in samples_darcy.items():
    ...
    mean_u0 = reconstruct_initial_condition(mean_latent)[0]
    mean_uT = solve_state_field(mean_latent, t=OBS_TIME)
    mean_fields[label] = mean_u0
    mean_final_states[label] = mean_uT

    ic_rel_l2_pct = 100.0 * (np.linalg.norm(mean_u0 - true_u0) / norm_true_u0)
    ...
    metrics[label].update(dict(
        ...
        RMSE_alpha=rmse_a,
        RMSE_field=rmse_array(mean_u0, true_u0),
        RelL2_field=float(np.linalg.norm(mean_u0 - true_u0) / norm_true_u0),
        RMSE_final=rmse_array(mean_uT, true_uT),
        RelL2_final=final_rel,
        FwdRelErr=sensor_rel,
    ))

    print(f"{display_names.get(label, label):<24} | {ic_rel_l2_pct:<12.4f} | "
          f"{rmse_a:<12.4e} | {final_rel:<12.4e} | {sensor_rel:<12.4e}")
...
print('\n=== Final Allen–Cahn metrics (paper-ready) ===')
print(
    f"{'Method':<24} | {'RMSE_a':<9} | {'RMSE_u0':<9} | {'RMSE_uT':<9} | {'SensorRel':<10} | "
...
for label, data in metrics.items():
    ...
    print(
        ...
        f"{data.get('RMSE_final', float('nan')):<9.3e} | "
...
print('\nVisualizing initial condition fields, Allen–Cahn states, and sensor residual maps...')

methods_to_plot = [label for label in samples_darcy.keys() if label in mean_fields]
n_cols = len(methods_to_plot) + 1

# --- Figure 1: Initial condition reconstruction ---
fig, axes = plt.subplots(4, n_cols, figsize=(4 * n_cols, 14))

has_true_u0 = 'true_u0' in globals() and true_u0 is not None
...
vis_reference_field = true_u0 if has_true_u0 else mean_fields[vis_anchor_key]
...
if vis_anchor_key in samples_darcy and vis_anchor_key in mean_fields:
    ...
    if anchor_vis_samps.shape[0] > 0:
        anchor_vis_fields = reconstruct_initial_condition(anchor_vis_samps[:, :d_lat])
...
if has_true_u0:
    axes[0, 0].set_title('Ground Truth\nInitial Condition $u_0(x)$', fontsize=18)
else:
    axes[0, 0].set_title(f"Normalizer\n{vis_anchor_title} $u_0$", fontsize=18)
axes[0, 0].axis('off')
plt.colorbar(im0, ax=axes[0, 0], fraction=0.046, pad=0.04)

axes[3, 0].imshow(vis_reference_field, cmap='RdBu_r', origin='lower', vmin=vmin, vmax=vmax)
axes[3, 0].set_title('Ground Truth' if has_true_u0 else 'Normalizer', fontsize=14)
axes[3, 0].axis('off')
axes[1, 0].axis('off')
axes[2, 0].axis('off')
...
if vis_anchor_key not in mean_fields:
    ...
    for label in methods_to_plot:
        ...
        max_err = max(max_err, np.abs(mean_f - vis_reference_field).max())
        samps = get_valid_samples(samples_darcy[label])[:500]
        if samps.shape[0] > 0:
            fields = reconstruct_initial_condition(samps[:, :d_lat])
...
plt.suptitle(
    f"Allen–Cahn IC Inversion (Latent Dim={d_lat}): Initial Condition Reconstruction",
    fontsize=18, y=1.02,
)
plt.tight_layout()
plt.show()
...
# --- Figure 2: Final-time Allen–Cahn state comparison ---
print('\nVisualizing Allen–Cahn states at observation time...')
fig2, axes2 = plt.subplots(1, n_cols, figsize=(4 * n_cols, 4))

has_true_uT = 'true_uT' in globals() and true_uT is not None
vis_reference_final = true_uT if has_true_uT else mean_final_states[vis_anchor_key]
final_vmin = float(np.min(vis_reference_final))
final_vmax = float(np.max(vis_reference_final))

im_true_final = axes2[0].imshow(vis_reference_final, cmap='RdBu_r', origin='lower', vmin=final_vmin, vmax=final_vmax)
...
if has_true_uT:
    ...
    axes2[0].set_title(f'Normalizer\n{vis_anchor_title} state', fontsize=14)
axes2[0].axis('off')
...
for i, label in enumerate(methods_to_plot):
    ...
    if samps_clean.shape[0] < 10:
        axes2[col].axis('off')
        continue
    mean_uT = mean_final_states[label]
    axes2[col].imshow(mean_uT, cmap='RdBu_r', origin='lower',
...
plt.suptitle(
    f"Allen–Cahn IC Inversion (Latent Dim={d_lat}): Allen–Cahn State at Observation Time",
    fontsize=16, y=1.05,
)
plt.tight_layout()
plt.show()
...
plt.suptitle(
    f"Allen–Cahn IC Inversion (Latent Dim={d_lat}): Sensor-Space Residuals",
    fontsize=16, y=1.05,
)
plt.tight_layout()
plt.show()
