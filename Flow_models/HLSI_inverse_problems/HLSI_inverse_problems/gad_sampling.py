# -*- coding: utf-8 -*-
"""Iterative LFGI transport, MALA baseline, metrics, and plotting.
...
An independent ``mala`` node provides an exact-score, Metropolis-adjusted
baseline.  It runs parallel chains from the same prior, adapts its scalar step
size only during warmup, freezes the kernel before collection, and returns
post-warmup thinned draws together with acceptance and split-R-hat diagnostics.
"""
...
_CONFIG_KEYS = {
    ...
    'mala_n_chains', 'mala_warmup', 'mala_thin', 'mala_dt',
    'mala_adapt', 'mala_target_accept', 'mala_min_dt', 'mala_max_dt',
    'mala_progress_every',
}
...
def normalize_sampler_config(label, config, default_n_samples=None, default_dim=None):
    ...
    if node not in {'transport', 'ratio', 'mala'}:
        raise ValueError(f"Sampler {label!r}: node must be 'transport', 'ratio', or 'mala'.")
    cfg['node'] = node
    cfg['score'] = (
        canonicalize_score_method(cfg.get('score', 'lfgi'))
        if node in {'transport', 'ratio'} else None
    )
    ...
    if node in {'transport', 'ratio'} and cfg['t_max'] <= cfg['t_min']:
        raise ValueError(f'Sampler {label!r}: t_max must exceed t_min.')
    if node in {'transport', 'ratio'} and cfg['steps'] <= 0 and not (
        node == 'ratio' and cfg['ratio_mode'] == 'static'
    ):
        raise ValueError(f'Sampler {label!r}: flow nodes require steps > 0.')

    if node == 'mala':
        cfg['mala_n_chains'] = max(1, int(cfg.get('mala_n_chains', min(32, cfg['n_samples']))))
        cfg['mala_warmup'] = max(0, int(cfg.get('mala_warmup', 100)))
        cfg['mala_thin'] = max(1, int(cfg.get('mala_thin', 1)))
        cfg['mala_dt'] = float(cfg.get('mala_dt', 1e-4))
        cfg['mala_adapt'] = _coerce_bool(cfg.get('mala_adapt', True), True)
        cfg['mala_target_accept'] = float(cfg.get('mala_target_accept', 0.574))
        cfg['mala_min_dt'] = float(cfg.get('mala_min_dt', 1e-12))
        cfg['mala_max_dt'] = float(cfg.get('mala_max_dt', 1e-1))
        cfg['mala_progress_every'] = max(0, int(cfg.get('mala_progress_every', 25)))
        if cfg['mala_dt'] <= 0.0 or cfg['mala_min_dt'] <= 0.0:
            raise ValueError(f'Sampler {label!r}: MALA step sizes must be positive.')
        if cfg['mala_max_dt'] < cfg['mala_min_dt']:
            raise ValueError(f'Sampler {label!r}: mala_max_dt must be >= mala_min_dt.')
        if not 0.0 < cfg['mala_target_accept'] < 1.0:
            raise ValueError(f'Sampler {label!r}: mala_target_accept must lie in (0, 1).')
        cfg['mala_dt'] = min(max(cfg['mala_dt'], cfg['mala_min_dt']), cfg['mala_max_dt'])
    else:
        cfg['mala_n_chains'] = 0
        cfg['mala_warmup'] = 0
        cfg['mala_thin'] = 0
        cfg['mala_dt'] = float('nan')
        cfg['mala_adapt'] = False
        cfg['mala_target_accept'] = float('nan')
        cfg['mala_min_dt'] = float('nan')
        cfg['mala_max_dt'] = float('nan')
        cfg['mala_progress_every'] = 0
...
def _posterior_log_density_and_score(x, prior_model, lik_model):
    """Evaluate the exact finite-dimensional posterior energy and score."""
    log_lik, grad_lik = lik_model.log_likelihood_and_grad(x)
    log_post = prior_model.log_prob(x) + log_lik
    score = prior_model.score0(x) + grad_lik
    return log_post, score


def _split_rhat(draws):
    """Return per-coordinate split R-hat for draws shaped [draw, chain, dim]."""
    if draws.ndim != 3 or draws.shape[0] < 4 or draws.shape[1] < 2:
        return None
    half = int(draws.shape[0] // 2)
    first = draws[:half].permute(1, 0, 2)
    second = draws[-half:].permute(1, 0, 2)
    split = torch.cat([first, second], dim=0)
    chain_means = torch.mean(split, dim=1)
    within = torch.var(split, dim=1, unbiased=True)
    w = torch.mean(within, dim=0)
    b = float(half) * torch.var(chain_means, dim=0, unbiased=True)
    var_hat = ((float(half) - 1.0) / float(half)) * w + b / float(half)
    tiny = torch.finfo(draws.dtype).eps
    ratio = torch.where(
        w > tiny,
        var_hat / torch.clamp(w, min=tiny),
        torch.where(b <= tiny, torch.ones_like(w), torch.full_like(w, float('inf'))),
    )
    return torch.sqrt(torch.clamp(ratio, min=0.0))


def run_mala_sampler(n_samples, prior_model, lik_model, *, dim,
                     n_chains=32, warmup=100, thin=1, dt=1e-4,
                     adapt=True, target_accept=0.574,
                     min_dt=1e-12, max_dt=1e-1,
                     progress_every=25):
    """Run parallel identity-preconditioned MALA chains.

    ``dt`` follows the legacy convention

        x' = x + dt * score(x) + sqrt(2 * dt) * Normal(0, I).

    Dual averaging changes ``dt`` only during warmup.  The final kernel is then
    frozen and every ``thin``-th state is retained.  Unlike the older benchmark
    helper, this routine returns actual post-warmup chain draws rather than one
    terminal state per chain.
    """
    n_samples = int(n_samples)
    n_chains = min(max(1, int(n_chains)), n_samples)
    warmup = max(0, int(warmup))
    thin = max(1, int(thin))
    draws_per_chain = int(math.ceil(float(n_samples) / float(n_chains)))
    production_transitions = draws_per_chain * thin
    total_transitions = warmup + production_transitions
    ...
    log_eps = math.log(math.sqrt(2.0 * dt_initial))
    mu = math.log(10.0) + log_eps
    log_eps_bar = log_eps
    h_bar = 0.0
    da_gamma = 0.05
    da_t0 = 10.0
    ...
    x = prior_model.sample(n_chains).detach().to(device=device, dtype=torch.float64)
    if x.ndim != 2 or x.shape[1] != int(dim):
        raise ValueError(
            f'prior_model.sample returned shape {tuple(x.shape)}; expected ({n_chains}, {int(dim)}).'
        )
    with torch.no_grad():
        log_post, score = _posterior_log_density_and_score(x, prior_model, lik_model)

    score_norm_initial = float(torch.linalg.vector_norm(score, dim=1).mean().item())
    score_norm_sum = 0.0
    score_norm_max = score_norm_initial
    warmup_accept_sum = 0.0
    production_accept_sum = 0.0
    production_accept_count = 0
    production_jump_sq_sum = 0.0
    retained = []

    for transition_idx in range(total_transitions):
        eps = math.exp(log_eps)
        proposal_dt = 0.5 * eps * eps
        x_old = x
        drift = proposal_dt * score
        proposal = x + drift + eps * torch.randn_like(x)

        with torch.no_grad():
            log_post_prop, score_prop = _posterior_log_density_and_score(
                proposal, prior_model, lik_model
            )

        reverse_drift = proposal_dt * score_prop
        log_q_forward = -torch.sum((proposal - x - drift) ** 2, dim=1) / (4.0 * proposal_dt)
        log_q_reverse = -torch.sum((x - proposal - reverse_drift) ** 2, dim=1) / (4.0 * proposal_dt)
        log_alpha = log_post_prop - log_post + log_q_reverse - log_q_forward
        proposal_finite = (
            torch.isfinite(log_post_prop)
            & torch.isfinite(score_prop).all(dim=1)
            & torch.isfinite(log_alpha)
        )
        current_finite = torch.isfinite(log_post) & torch.isfinite(score).all(dim=1)
        log_alpha = torch.where(proposal_finite, log_alpha, torch.full_like(log_alpha, -torch.inf))
        ...
        accept_prob = torch.exp(clipped_log_alpha)
        accept_prob = torch.nan_to_num(accept_prob, nan=0.0, posinf=1.0, neginf=0.0)
        accept = torch.log(torch.rand(n_chains, device=x.device)) < clipped_log_alpha

        mask = accept.unsqueeze(1)
        x = torch.where(mask, proposal, x)
        log_post = torch.where(accept, log_post_prop, log_post)
        score = torch.where(mask, score_prop, score)

        step_score_norm = float(torch.linalg.vector_norm(score, dim=1).mean().item())
        ...
        if transition_idx < warmup:
            warmup_accept_sum += float(accept.to(torch.float64).mean().item())
            if adapt:
                m = transition_idx + 1
                eta = 1.0 / (float(m) + da_t0)
                h_bar = (1.0 - eta) * h_bar + eta * (float(target_accept) - mean_accept_prob)
                candidate = mu - (math.sqrt(float(m)) / da_gamma) * h_bar
                candidate = min(max(candidate, math.log(eps_min)), math.log(eps_max))
                avg_weight = float(m) ** (-da_kappa)
                log_eps_bar = avg_weight * candidate + (1.0 - avg_weight) * log_eps_bar
                log_eps = candidate
                if m == warmup:
                    log_eps = min(max(log_eps_bar, math.log(eps_min)), math.log(eps_max))
        else:
            production_accept_sum += float(accept.to(torch.float64).sum().item())
            production_accept_count += n_chains
            production_jump_sq_sum += float(
                torch.sum((x - x_old) ** 2, dim=1).mean().item()
            )
            production_idx = transition_idx - warmup + 1
            if production_idx % thin == 0:
                retained.append(x.detach().cpu().clone())

        if progress_every and (
            transition_idx == 0
            or transition_idx + 1 == warmup
            or transition_idx + 1 == total_transitions
            or (transition_idx + 1) % int(progress_every) == 0
        ):
            phase = 'warmup' if transition_idx < warmup else 'sampling'
            print(
                f'  MALA {phase} {transition_idx + 1}/{total_transitions}: '
                f'dt={proposal_dt:.3e}, accept_prob={mean_accept_prob:.3f}'
            )

    if len(retained) != draws_per_chain:
        raise RuntimeError(
            f'MALA retained {len(retained)} draws per chain; expected {draws_per_chain}.'
        )
    draw_tensor = torch.stack(retained, dim=0)
    samples = draw_tensor.reshape(-1, int(dim))[:n_samples].contiguous()
    rhat = _split_rhat(draw_tensor)
    if rhat is None:
        rhat_median = float('nan')
        rhat_max = float('nan')
        rhat_num_finite = 0
    else:
        finite_rhat = rhat[torch.isfinite(rhat)]
        rhat_num_finite = int(finite_rhat.numel())
        rhat_median = float(torch.median(finite_rhat).item()) if rhat_num_finite else float('inf')
        rhat_max = float(torch.max(rhat).item())

    unique_fraction = float(torch.unique(samples, dim=0).shape[0]) / float(samples.shape[0])
    posterior_evals = n_chains * (total_transitions + 1)
    final_dt = 0.5 * math.exp(2.0 * log_eps)
    info = {
        'mala_n_chains': n_chains,
        'mala_warmup': warmup,
        'mala_thin': thin,
        'mala_draws_per_chain': draws_per_chain,
        'mala_total_transitions': total_transitions,
        'mala_production_transitions': production_transitions,
        'mala_dt_initial': dt_initial,
        'mala_dt_final': final_dt,
        'mala_adapt': bool(adapt and warmup > 0),
        'mala_target_accept': float(target_accept),
        'mala_acceptance_warmup': warmup_accept_sum / float(max(1, warmup)),
        'mala_acceptance_rate': production_accept_sum / float(max(1, production_accept_count)),
        'mala_mean_squared_jump': production_jump_sq_sum / float(max(1, production_transitions)),
        'mala_unique_fraction': unique_fraction,
        'mala_rhat_median': rhat_median,
        'mala_rhat_max': rhat_max,
        'mala_rhat_num_finite': rhat_num_finite,
        'mala_posterior_evals': posterior_evals,
        'score_norm': float(torch.linalg.vector_norm(score, dim=1).mean().item()),
        'score_norm_initial': score_norm_initial,
        'score_norm_mean': score_norm_sum / float(max(1, total_transitions)),
        'score_norm_final': float(torch.linalg.vector_norm(score, dim=1).mean().item()),
        'score_norm_max': score_norm_max,
        'score_norm_num_steps': total_transitions,
    }
    return samples, info
...
def run_single_sampler_config(label, config, prior_model, lik_model, *, states,
    ...
    if cfg['node'] == 'mala':
        samples, mala_info = run_mala_sampler(
            cfg['n_samples'], prior_model, lik_model,
            dim=cfg['dim'],
            n_chains=cfg['mala_n_chains'],
            warmup=cfg['mala_warmup'],
            thin=cfg['mala_thin'],
            dt=cfg['mala_dt'],
            adapt=cfg['mala_adapt'],
            target_accept=cfg['mala_target_accept'],
            min_dt=cfg['mala_min_dt'],
            max_dt=cfg['mala_max_dt'],
            progress_every=cfg['mala_progress_every'],
        )
        posterior_evals = int(mala_info['mala_posterior_evals'])
        run_info = dict(cfg)
        ...
        run_info.update({
            ...
            'n_gate': 0,
            'init_steps': 0,
            'steps': int(mala_info['mala_total_transitions']),
            'score': 'mala',
            'method': 'mala',
            'weight_mode': 'unweighted',
            'pde_likelihood_evals': posterior_evals,
            'pde_score_evals': posterior_evals,
            'pde_gn_hessian_evals': 0,
            'pde_solve_count': 2 * posterior_evals,
        })
        state = NodeState(
            samples=samples,
...
def run_tree_sampler_suite(sampler_configs, prior_model, lik_model, n_ref=10000):
    ...
    for label in execution_order:
        ...
        states[label] = state
        info = dict(info)
        local_runtime = time.time() - t0
        parent_label = cfg.get('ref_source')
        parent_info = precomp['all_run_info'].get(parent_label, {})
        count_keys = (
            'pde_likelihood_evals',
            'pde_score_evals',
            'pde_gn_hessian_evals',
            'pde_solve_count',
        )
        for key in count_keys:
            local_value = int(info.get(key, 0))
            info[f'local_{key}'] = local_value
            info[key] = local_value + int(parent_info.get(key, 0))
        info['local_runtime_seconds'] = local_runtime
        info['runtime_seconds'] = local_runtime + float(parent_info.get('runtime_seconds', 0.0))
        info['path_depth'] = int(parent_info.get('path_depth', 0)) + 1
        precomp['all_run_info'][label] = info
        ...
        print(
            f"{label}: local={local_runtime:.2f}s, "
            f"cumulative={info['runtime_seconds']:.2f}s, "
            f"cumulative PDE proxy={info['pde_solve_count']}"
        )
...
def summarize_sampler_run(sampler_run_info):
    ...
    for label, info in sampler_run_info.items():
        ...
        mala = ''
        if info.get('node') == 'mala':
            mala = (
                f" | accept={info.get('mala_acceptance_rate', float('nan')):.3f}"
                f" | split-Rhat(max)={info.get('mala_rhat_max', float('nan')):.3f}"
                f" | dt={info.get('mala_dt_final', float('nan')):.3e}"
            )
        print(
            ...
            f"PDE proxy={info.get('pde_solve_count', 0)}{mala}"
        )
...
def build_results_dataframes(metrics_dict, run_info_dict, n_ref, target_name,
    ...
    for label in ordered_methods:
        ...
        runinfo_rows.append({
            ...
            'N_ref': int(info.get('n_ref', n_ref) or 0),
            'steps': int(info.get('steps', info.get('init_steps', 0))),
            'path_depth': int(info.get('path_depth', 1)),
            ...
            'local_pde_likelihood_evals': int(info.get('local_pde_likelihood_evals', info.get('pde_likelihood_evals', 0))),
            'local_pde_score_evals': int(info.get('local_pde_score_evals', info.get('pde_score_evals', 0))),
            'local_pde_gn_hessian_evals': int(info.get('local_pde_gn_hessian_evals', info.get('pde_gn_hessian_evals', 0))),
            'local_pde_solve_count': int(info.get('local_pde_solve_count', info.get('pde_solve_count', 0))),
            'mala_n_chains': int(info.get('mala_n_chains', 0)),
            'mala_warmup': int(info.get('mala_warmup', 0)),
            'mala_thin': int(info.get('mala_thin', 0)),
            'mala_draws_per_chain': int(info.get('mala_draws_per_chain', 0)),
            'mala_dt_initial': float(info.get('mala_dt_initial', np.nan)),
            'mala_dt_final': float(info.get('mala_dt_final', np.nan)),
            'mala_acceptance_rate': float(info.get('mala_acceptance_rate', np.nan)),
            'mala_rhat_median': float(info.get('mala_rhat_median', np.nan)),
            'mala_rhat_max': float(info.get('mala_rhat_max', np.nan)),
            'mala_unique_fraction': float(info.get('mala_unique_fraction', np.nan)),
            'runtime_seconds': float(info.get('runtime_seconds', np.nan)),
            'local_runtime_seconds': float(info.get('local_runtime_seconds', info.get('runtime_seconds', np.nan))),
