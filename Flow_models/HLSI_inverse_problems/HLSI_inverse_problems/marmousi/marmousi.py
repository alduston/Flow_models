...
@jax.jit
def batch_solve_forward(alpha_batch):
    return jax.vmap(solve_forward)(alpha_batch)
...
run_results_info = init_run_results('marmousi_hlsi')
...
y_holdout_obs_np = y_clean_np + np.random.normal(0.0, NOISE_STD, size=y_clean_np.shape)

dashboard = DashboardPDF(
    DASHBOARD_PDF_PATH,
    title='Marmousi acoustic FWI HLSI dashboard',
)
dashboard.add_text_page(
    'Marmousi acoustic FWI HLSI dashboard',
    [
        ...
        f'HELDOUT_BATCH_SIZE = {HELDOUT_BATCH_SIZE}',
        f'HESS_MIN = {HESS_MIN}, HESS_MAX = {HESS_MAX}',
        f'PLOT_NORMALIZER = {PLOT_NORMALIZER}',
        f'Marmousi crop = x[{MARM_X_START}:{MARM_X_END}], z[{MARM_Z_START}:{MARM_Z_END}], smooth_passes={MARM_SMOOTH_PASSES}',
    ],
)
...
try:
    metrics = compute_heldout_predictive_metrics(
        samples,
        metrics,
        heldout_forward_eval_fn=lambda a: np.array(solve_forward(jnp.array(a))),
        batched_forward_eval_fn=lambda a_batch: np.asarray(
            batch_solve_forward(jnp.asarray(a_batch, dtype=jnp.float64))
        ),
...
plt.suptitle(
    f'Marmousi acoustic shot gathers and residual images ({len(shot_plot_indices)} of {N_SOURCES} sources shown: {shot_plot_indices.tolist()})',
    fontsize=16,
    y=1.002,
)
plt.tight_layout()
