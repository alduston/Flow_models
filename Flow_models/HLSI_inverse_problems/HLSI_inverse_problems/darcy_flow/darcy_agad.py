...
matrix/scalar blends, and Tweedie.  Each method performs three transport/ratio
rounds, with the same score estimator used by both nodes, and reports transport
rounds 1 and 3.  Ratio nodes move an unweighted cloud with the completed flow
correction instead of carrying static particle weights.
...
# ==========================================
# Each method alternates a public transport node T_r with a hidden completed
# ratio-flow node R_r.  R_r returns a new unweighted particle law, which becomes
# the reference for T_{r+1}.  Only the requested T rounds enter tables and plots.
...
def make_iterative_transport_sampler_configs(method_specs, rounds=ITERATIVE_TRANSPORT_ROUNDS):
    ...
    for method_key, spec in method_specs.items():
        ...
        for round_idx in range(1, int(rounds) + 1):
            ...
            ratio_cfg.update(RATIO_FLOW_COMMON)
...
RUN_COMMAND_HINT = (
    ...
    'IP_ITER_DENSITY_STEPS={density_steps} python darcy_flow.py'
).format(
    n_ref=N_REF,
...
dashboard.add_text_page(
    ...
    [
        ...
        'This dashboard contains the two canonical saved-results tables plus every PNG diagnostic plot saved in the run directory. Public rows are transport outputs T_r; hidden ratio nodes R_r move fresh unweighted clouds with the completed flow correction.',
