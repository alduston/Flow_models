# -*- coding: utf-8 -*-
"""ip_density_bench_two.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1J7SqT1baUal5UFmwN2ky9dvCo1vB5Nka
"""

# -*- coding: utf-8 -*-
"""ip_density_bench.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Gbg6Ectlf9tjGKwwkolQzFhIRUoDgb3b
"""
...
# ==========================================
# This benchmark first constructs a reference/evaluation bank using two
# bootstrapped alternating DRC-LFGI rounds, then runs three ratio-only DRC-R
# density-evaluation nodes on that bank: Tweedie, scalar blend, and CE-HLSI/LFGI.
# Each density node computes probability-flow log q and compares -log q against
# the true unnormalized posterior energy -log pi at the same reference locations.
#
# Override these from Colab without editing the file, e.g.
#   %env IP_DENSITY_BOOT_INIT_STEPS=100
#   %env IP_DENSITY_DRC_PF_STEPS=16
#   %env IP_DENSITY_BLEND_DIVERGENCE=hutchinson
...
# Bootstrap reference generation: two alternating DRC-LFGI rounds.
BOOT_N_REF = _env_int('IP_DENSITY_BOOT_N_REF', N_REF)
...
# finite differences by default so the energy-correlation benchmark is not polluted
# by Hutchinson probe noise. Switch blend to hutchinson for speed if desired.
...
MALA_STEPS = _env_int('IP_DENSITY_MALA_STEPS', 360)
MALA_BURNIN = _env_int('IP_DENSITY_MALA_BURNIN', 90)
...
run_ctx = init_run_results('darcy_alt_drc_density_bench')
...
dashboard = DashboardPDF(
    DASHBOARD_PDF_PATH,
    title='Darcy flow ALT-DRC density comparison dashboard',
)
dashboard.add_text_page(
    'Darcy flow ALT-DRC density comparison dashboard',
    [
        f"Created: {datetime.now().isoformat(timespec='seconds')}",
        'This dashboard uses two bootstrapped alternating DRC-LFGI rounds to construct the reference bank, then compares density-evaluation score families on that bank.',
        'Primary density diagnostic: true posterior energy -log pi(x) versus probability-flow estimated energy -log q(x), evaluated for Tweedie, scalar blend, and CE-HLSI/LFGI.',
        ...
        f'N_REF = {N_REF}',
        f'BOOT_N_REF = {BOOT_N_REF}',
        f'BOOT_INIT_STEPS = {BOOT_INIT_STEPS}',
        f'BOOT_DRC_PF_STEPS = {BOOT_DRC_PF_STEPS}',
        f'BOOT_DRC_DIVERGENCE = {BOOT_DRC_DIVERGENCE}',
        f'BOOT_DRC_CLIP = {BOOT_DRC_CLIP}',
...
def _density_eval_config(ref_source, score_init, divergence, label, display_name):
    return {
        ...
        # The final ALT-DRC-LFGI bank is already treated as the empirical
        # reference/posterior bank. Do not likelihood-reweight again when fitting
        # each score estimator for density evaluation.
        'drc_score_init_weights': 'None',
        'transition_w': 'ou',
        'n_ref': BOOT_N_REF,
        'include_results': False,
...
SAMPLER_CONFIGS = OrderedDict([
    # Baseline posterior reference construction.
    # This is the bank on which all density estimators are evaluated.
    ('MALA', {
    ...
    # Density-evaluation nodes: no sampling. Each node fits/evaluates one score
    # family on the same MALA reference bank and stores log q, log pi-log q,
    # and energy-correlation diagnostics.
...
SAMPLER_CONFIGS = OrderedDict([
    ...
    ('ALT-DRC-LFGI1', {
        'display_name': 'ALT-DRC LFGI round 0',
        'init': 'CE-HLSI',
        'init_weights': 'None',
        'transition_w': 'ou',
        'n_ref': BOOT_N_REF,
        'init_steps': BOOT_INIT_STEPS,
        'mala_steps': 0,
        'mala_burnin': 0,
        'log_mean_ess': True,
        'include_results': False,
    ...
    # First R-step and weighted LFGI sampling step.
    ('ALT-DRC-R1', _boot_drc_r_config('ALT-DRC-LFGI1', 'ALT-DRC R1')),
    ('ALT-DRC-LFGI2', {
        'display_name': 'ALT-DRC LFGI round 1',
        'ref_source': 'ALT-DRC-R1',
        'init': 'CE-HLSI',
        'init_weights': 'DRC',
        'transition_w': 'ou',
        'n_ref': BOOT_N_REF,
    ...
    # Second R-step and weighted LFGI sampling step. This final bank is the
    # reference/evaluation bank for the density benchmark below.
    ('ALT-DRC-R2', _boot_drc_r_config('ALT-DRC-LFGI2', 'ALT-DRC R2')),
    ('ALT-DRC-LFGI3', {
        'display_name': 'ALT-DRC LFGI reference',
        'ref_source': 'ALT-DRC-R2',
        'init': 'CE-HLSI',
        'init_weights': 'DRC',
        'transition_w': 'ou',
        'n_ref': BOOT_N_REF,
    ...
    # Density-evaluation nodes: no sampling. Each node fits/evaluates one score
    # family on the same final bootstrapped reference bank and stores log q,
    # log pi-log q, and energy-correlation diagnostics.
    ('DENS-Tweedie', _density_eval_config(
        DENSITY_REF_SOURCE, 'tweedie', DENSITY_TWEEDIE_DIVERGENCE,
        'DENS-Tweedie', 'Density eval: Tweedie',
    )),
...
save_reproducibility_log(
    title='Darcy flow ALT-DRC density-comparison reproducibility log',
    config={
        'seed': seed,
        'ACTIVE_DIM': ACTIVE_DIM,
        'BUILD_GNL_BANKS': BUILD_GNL_BANKS,
