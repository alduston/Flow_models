...
* commit its KL basis, observation indices, and observations once so every
  distributed worker samples exactly the same coefficient-space posterior;
...
-------
    ...
    python darcy_nuts_offline_reference_20260821_v3.py \
...
def problem_metadata(args: argparse.Namespace) -> Dict[str, object]:
    return {
...
def problem_data_fingerprint(
    ...
    *,
    ...
    return hash_arrays(
        problem_metadata(args),
        [
            basis,
            obs_locations_train,
            obs_locations_holdout,
            alpha_true,
            y_obs,
        ],
    )
...
def build_problem_and_data(args: argparse.Namespace, module):
    ...
    fingerprint = problem_data_fingerprint(
        args,
        basis=np.asarray(problem.basis_np),
        obs_locations_train=np.asarray(problem.obs_locations_train),
        obs_locations_holdout=np.asarray(problem.obs_locations_holdout),
...
def build_problem_from_prepared_arrays(
    args: argparse.Namespace,
    *,
    basis_np: np.ndarray,
    obs_locations_train: np.ndarray,
    ...
    """Rebuild only the Darcy solver around Stage-1-owned immutable arrays.

    ``numpy.linalg.eigh`` is permitted to choose different eigenvector signs
    (and bases inside nearly degenerate eigenspaces) in independent processes.
    Re-running the KL construction on every node can therefore change the
    coefficient-space posterior.  Distributed workers must use the exact basis
    and observation indices committed by the preparation process.
    """
    n_grid = int(args.grid_size)
    basis_np = np.asarray(basis_np, dtype=np.float64)
    obs_train = np.asarray(obs_locations_train)
    obs_holdout = np.asarray(obs_locations_holdout)
    ...
    return SimpleNamespace(
        ...
        solve_forward_holdout=jax.jit(
            lambda alpha: solve_at(alpha, obs_holdout_jax)
        ),
    )


def load_prepared_problem_data(
    path: Path,
    args: argparse.Namespace,
    *,
    ...
    with np.load(path, allow_pickle=False) as archive:
        ...
        if missing:
            raise RuntimeError(
                f"{path.name} is missing required arrays: "
                + ", ".join(sorted(missing))
            )
        alpha_true = np.asarray(archive["alpha_true"], dtype=np.float64)
        y_clean = np.asarray(archive["y_clean"], dtype=np.float64)
        y_obs = np.asarray(archive["y_obs"], dtype=np.float64)
        basis_raw = np.asarray(archive["basis"])
        obs_train_raw = np.asarray(archive["obs_locations_train"])
        obs_holdout_raw = np.asarray(archive["obs_locations_holdout"])
        stored_fingerprint = str(archive["problem_fingerprint"].item())
        stored_signature = str(archive["configuration_signature"].item())

    expected_shapes = {
        "basis": ((args.grid_size ** 2, args.latent_dim), basis_raw.shape),
        "alpha_true": ((args.latent_dim,), alpha_true.shape),
        "y_clean": ((args.n_observations,), y_clean.shape),
        "y_obs": ((args.n_observations,), y_obs.shape),
        "obs_locations_train": ((args.n_observations,), obs_train_raw.shape),
        "obs_locations_holdout": ((args.n_holdout,), obs_holdout_raw.shape),
    }
    malformed = {
    ...
    if malformed:
        raise RuntimeError(f"{path.name} has incompatible array shapes: {malformed}")
    if not np.issubdtype(obs_train_raw.dtype, np.integer) or not np.issubdtype(
        obs_holdout_raw.dtype, np.integer
    ):
        raise RuntimeError(f"{path.name} observation locations must be integer arrays.")
    if not all(
        np.isfinite(values).all()
        for values in (basis_raw, alpha_true, y_clean, y_obs)
    ):
        raise FloatingPointError(f"{path.name} contains non-finite problem data.")
    all_locations = np.concatenate([obs_train_raw, obs_holdout_raw])
    if (
        np.any(all_locations < 0)
        or np.any(all_locations >= args.grid_size ** 2)
        or np.unique(all_locations).size != all_locations.size
    ):
        raise RuntimeError(
            f"{path.name} contains invalid or duplicated observation locations."
        )
    if stored_signature != expected_configuration_signature:
        raise RuntimeError(
            f"{path.name} has incompatible sampler configuration."
        )
    recomputed_fingerprint = problem_data_fingerprint(
        args,
        basis=basis_raw,
        obs_locations_train=obs_train_raw,
        obs_locations_holdout=obs_holdout_raw,
        alpha_true=alpha_true,
        y_obs=y_obs,
    )
    if stored_fingerprint != recomputed_fingerprint:
        raise RuntimeError(f"{path.name} failed its problem-data integrity check.")
    if stored_fingerprint != expected_problem_fingerprint:
        raise RuntimeError(
            "Prepared problem data does not match the resume-directory fingerprint."
        )

    problem = build_problem_from_prepared_arrays(
        args,
        basis_np=basis_raw,
        obs_locations_train=obs_train_raw,
        obs_locations_holdout=obs_holdout_raw,
    ...
    if not np.allclose(reconstructed, y_clean, rtol=1e-9, atol=1e-11):
        max_error = float(np.max(np.abs(reconstructed - y_clean)))
        raise RuntimeError(
            "Prepared Darcy solver failed its forward-solution check; "
            f"maximum absolute error is {max_error:.3e}."
        )
    return problem, alpha_true, y_clean, y_obs, stored_fingerprint
...
def main() -> None:
    ...
    problem_data_path = output_dir / "problem_data.npz"
    previous = None
    if args.resume:
    ...
    if previous is None:
        problem, alpha_true, y_clean, y_obs, problem_fingerprint = (
            build_problem_and_data(args, darcy_module)
        )
        problem_source = "constructed once by the preparation process"
    else:
        if not problem_data_path.is_file():
            raise FileNotFoundError(
                "Resume directory is missing its immutable prepared problem data: "
                f"{problem_data_path}"
            )
        problem, alpha_true, y_clean, y_obs, problem_fingerprint = (
            load_prepared_problem_data(
                problem_data_path,
                args,
                expected_problem_fingerprint=str(
                    previous.get("problem_fingerprint", "")
                ),
                expected_configuration_signature=config_signature,
            )
        )
        problem_source = "loaded exactly from immutable Stage-1 problem_data.npz"
        print(f"Prepared problem fingerprint: {problem_fingerprint}")
        print("Using the Stage-1 Darcy basis and observations exactly (no KL rebuild).")
    ...
    config = {
        ...
        "distributed_problem_policy": (
            "The preparation process commits basis, observation indices, truth, "
            "and observations once. Every resumed worker rebuilds only the solver "
            "around those immutable arrays and never recomputes the KL basis."
        ),
    ...
    if previous is None:
        ...
        # run_config.json is the commit marker. Write it only after the complete
        # immutable problem payload is safely in place.
