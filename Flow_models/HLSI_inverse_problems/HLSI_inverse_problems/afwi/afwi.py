...
# Exponential / Matérn-like covariance for the latent prior over the
# acoustic velocity perturbation field.
ell = 0.10
sigma_prior = 1.0
...
# --- Acoustic full-waveform inversion experiment design ---
...
print(
    f"Generating acoustic FWI configuration with {N_SOURCES} shots, "
    f"{N_RECEIVERS} receivers, {N_TIME_STEPS} time steps, and dt={DT:g}..."
)
...
source_xs = np.linspace(0.16, 0.84, N_SOURCES)
source_centers = np.column_stack([source_xs, np.full(N_SOURCES, SOURCE_DEPTH)])
receiver_xs = np.linspace(0.08, 0.92, N_RECEIVERS)
receiver_coords = np.column_stack([receiver_xs, np.full(N_RECEIVERS, RECEIVER_DEPTH)])

receiver_rows = np.array([np.argmin(np.abs(x - RECEIVER_DEPTH))] * N_RECEIVERS, dtype=int)
receiver_cols = np.array([np.argmin(np.abs(x - xx)) for xx in receiver_xs], dtype=int)
...
# Quadratic sponge profile to reduce boundary reflections.
...
print(
    f"Total observations: {num_observation} "
    f"({N_SOURCES} shots x {N_RECEIVERS} receivers x {N_RECORD_STEPS} recorded times)"
)
...
try:
    ...
    if isinstance(df_modes.iloc[0, 0], str):
        df_modes = pd.read_csv('data/AcousticFWI_Basis_Modes.csv')
    ...
    print("Saved 'data/Basis.csv', 'data/obs_locations.csv', and 'data/source_locations.csv'")

except FileNotFoundError:
    raise FileNotFoundError("Could not find 'data/AcousticFWI_Basis_Modes.csv'.")
...
print("Initializing JAX acoustic FWI Physics Engine...")
...
print(f"Source patterns shape: {source_patterns_jax.shape}")
print(f"Source wavelet shape: {source_time_series_jax.shape}")

# ==========================================
# 2. DEFINE FORWARD SOLVER (ACOUSTIC FWI)
# ==========================================
# PDE:
#     u_tt = c(x)^2 Δu + s_j(x) w(t)
# on [0,1]^2 with homogeneous Dirichlet walls and a quadratic sponge layer near
# the boundary. We record pressure traces at a near-surface receiver array.
#
# The unknown is a latent velocity field
#     c(x) = c0 * exp(gamma * tanh(m(x)) * mask(x)).
#
# This keeps the sampler loop unchanged: solve_forward still maps latent KL
# coefficients to a flattened observation vector, now consisting of shot gathers.
# ==========================================
...
def _flatten_measurements_by_source(gathers):
    """(n_sources, n_receivers, n_record) -> flat real vector."""
...
def _simulate_single_shot_gather(velocity_field, source_idx):
    ...
    rec_sub = rec_all[record_step_indices_jax, :]   # [n_record, n_receivers]
    return rec_sub.T                                # [n_receivers, n_record]
...
def solve_single_pattern(alpha, pattern_idx):
    """Return one source shot gather as [n_receivers, n_recorded_times]."""
    _, velocity_field = _alpha_to_raw_and_velocity(alpha)
    return _simulate_single_shot_gather(velocity_field, pattern_idx)

print("Acoustic FWI Physics Engine Loaded Successfully.")
print(
...
RUN_RESULTS_DIR = os.path.join(RUN_RESULTS_ROOT, f'acoustic_fwi_hlsi_{RUN_TIMESTAMP}')
os.makedirs(RUN_RESULTS_DIR, exist_ok=True)
RUN_RESULTS_STEM = f'acoustic_fwi_hlsi_{RUN_TIMESTAMP}'
...
def save_reproducibility_log(extra_sections=None):
    ...
    lines = []
    lines.append('Acoustic FWI HLSI run reproducibility log')
    lines.append('=' * 72)
...
# ==========================================
# 10. EXECUTION: ACOUSTIC FULL-WAVEFORM INVERSION
# ==========================================

print(f"--- Setting up {ACTIVE_DIM}D acoustic full-waveform inversion problem ---")
...
# 1. Ground truth: structured lens + channel model projected into the KL basis.
np.random.seed(seed)
...
# Slightly enrich the projected truth while keeping it smooth and structured.
...
# ==========================================
# 13. SCIENTIFIC VISUALIZATION (ACOUSTIC-FWI-SPECIFIC)
# ==========================================
...
def unpack_measurement_vector(y_vec):
    ...
    return y_vec.reshape(N_SOURCES, N_RECEIVERS, N_RECORD_STEPS)
...
def simulate_single_shot_numpy(alpha_latent, source_idx=0):
    ...
    for n in range(N_TIME_STEPS):
        ...
        numer = (
            2.0 * u_curr
            - (1.0 - 0.5 * DT * damping_field) * u_prev
            + (DT ** 2) * (c2 * lap + forcing)
        )
...
def trace_style_for_label(label):
    """Consistent visual hierarchy for amplitude/phase overlays."""
    label_l = label.lower()
    base = dict(
        linestyle='--',
        linewidth=1.55,
...
def wrapped_phase_residual(phi, phi_ref):
    return np.abs(np.angle(np.exp(1j * (phi - phi_ref))))
...
for label, raw in samples_helm.items():
    ...
    mean_field = latent_to_velocity(mean_latent)
...
print('\n=== Final acoustic FWI metrics (paper-ready) ===')
print(
    f"{'Method':<24} | {'RMSE_a':<9} | {'RMSE_c':<9} | {'FwdRel':<9} | "
...
repro_log_path = save_reproducibility_log(
    extra_sections={
        ...
        'summary_stats': {
            ...
            'dominant_trace_frequency_hz': float('nan'),
        },
    }
)
...
# ==========================================
# FIGURE 1: VELOCITY RECONSTRUCTION
# ==========================================

print('\nVisualizing velocity reconstructions, uncertainty, and samples...')
...
if vis_anchor_key in samples_helm and vis_anchor_key in mean_fields:
    ...
    if anchor_vis_samps.shape[0] > 0:
        anchor_vis_fields = reconstruct_velocity_field(anchor_vis_samps[:, :d_lat])
...
im0 = axes[0, 0].imshow(vis_reference_field, cmap='viridis', origin='lower', vmin=vmin, vmax=vmax)
axes[0, 0].scatter(receiver_col, receiver_row, c='lime', s=10, marker='s', alpha=0.8, label='Receivers')
axes[0, 0].scatter(source_cols, source_rows, c='cyan', s=50, marker='*', alpha=0.9, label='Sources')
axes[0, 0].contour(model_support_mask_np, levels=[0.5], colors='white', linewidths=1.0)
if has_true_field:
    axes[0, 0].set_title('Ground Truth\nVelocity $c(x)$', fontsize=18)
else:
    axes[0, 0].set_title(f"Normalizer\n{vis_anchor_title} velocity $c(x)$", fontsize=18)
axes[0, 0].axis('off')
...
axes[3, 0].contour(model_support_mask_np, levels=[0.5], colors='white', linewidths=1.0)
if has_true_field:
...
for i, label in enumerate(methods_to_plot):
    ...
    axes[0, col].scatter(receiver_col, receiver_row, c='lime', s=5, marker='s', alpha=0.45)
    axes[0, col].scatter(source_cols, source_rows, c='cyan', s=20, marker='*', alpha=0.85)
    axes[0, col].contour(model_support_mask_np, levels=[0.5], colors='white', linewidths=0.9)
    ...
    axes[1, col].contour(model_support_mask_np, levels=[0.5], colors='white', linewidths=0.8)
    if has_true_field:
    ...
    if samps.shape[0] > 0:
        fields = reconstruct_velocity_field(samps[:, :d_lat])
        std_f = np.std(fields, axis=0)
    else:
        fields = None
        std_f = np.zeros_like(vis_reference_field)

    im_std = axes[2, col].imshow(std_f, cmap='magma', origin='lower', vmin=0, vmax=max_std)
    axes[2, col].contour(model_support_mask_np, levels=[0.5], colors='white', linewidths=0.8)
    ...
    if fields is not None and samps.shape[0] > 0:
        ...
        im_samp = axes[3, col].imshow(samp_f, cmap='viridis', origin='lower',
                                      vmin=vmin, vmax=vmax)
        axes[3, col].contour(model_support_mask_np, levels=[0.5], colors='white', linewidths=0.8)
...
plt.suptitle(
    f"Acoustic FWI (d={d_lat}, {N_SOURCES} shots): velocity reconstruction",
    fontsize=18, y=1.02,
)
plt.tight_layout()
plt.show()
...
fig3.legend(
    ...
    frameon=False, fontsize=10, bbox_to_anchor=(0.5, 1.03)
)
fig3.suptitle(
