"""
navier_stokes_bespoke_from_darcy.py

HLSI (Hessian-Laplace Score Identity) posterior sampling for the
2D incompressible Navier-Stokes initial-vorticity inverse problem.
...
PDE:   d/dt omega + u · grad omega = nu Laplacian omega + f,
       -Laplacian psi = omega,   u = grad^perp psi,
       on the periodic square [0,1)^2.
Unknown:  initial vorticity field omega_0(x) parameterized in a reduced basis
Prior:    lpha ~ N(0, I)  (latent coefficients)
Data:     noisy final-time vorticity measurements at sparse sensor locations
"""
...
num_observation = 100       # current implementation/run-log setting
num_truncated_series = 24   # active latent dimension from current NS setup
num_modes_available = 100   # keep same total basis-bank size as current run log
seed = 42
prior_length_scale = 0.18   # smooth periodic prior, analogous to the KL-style bases elsewhere

print(f"Generating bespoke Navier-Stokes configuration for {num_observation} observations...")
...
def build_periodic_fourier_basis(N, q_max=100, length_scale=0.18):
    """
    Build a smooth periodic KL-like basis on the torus using low-frequency
    Fourier modes with exponentially decaying amplitudes.

    This replaces the external Basis_Modes.csv dependency while keeping the
    latent->field map smooth, periodic, and fully self-contained.
    """
    ...
    if len(cols) < q_max:
        raise RuntimeError(f"Only built {len(cols)} periodic modes; need {q_max}.")

    basis = np.column_stack(cols[:q_max]).astype(np.float64)
    return basis, meta[:q_max]


full_basis, basis_meta = build_periodic_fourier_basis(
    N=N,
...
print(f"Generated self-contained periodic basis with {full_basis.shape[1]} modes.")
print(f"Saved full basis to '{basis_modes_path}'")
print("Saved truncated active basis to 'data/Basis.csv'")
...
try:
    ...
    obs_raw = df_obs.to_numpy().flatten().astype(int)
...
# ==========================================
# 2D incompressible vorticity equation on the periodic square [0,1)^2:
#     d/dt omega + u · grad omega = nu Laplacian omega + f,
#     -Laplacian psi = omega,   u = grad^perp psi.
#
# The forward map returns the final-time vorticity sampled at sparse sensors,
# matching the current implementation report for the 100-observation run.
# We use a Fourier pseudo-spectral solver with 2/3 dealiasing and a
# Crank-Nicolson-style semi-implicit treatment of viscosity.
# ==========================================
...
forcing_field = 0.1 * (
    jnp.sin(2.0 * jnp.pi * (X_grid + Y_grid))
    + jnp.cos(2.0 * jnp.pi * (X_grid + Y_grid))
)
...
def _ns_step(_, omega_hat):
    ...
    adv = vel_x * omega_x + vel_y * omega_y
    ...
    omega_hat_next = jnp.where(K2 == 0.0, 0.0, omega_hat_next)
    return omega_hat_next
...
def solve_forward_full(alpha):
    ...
    omega_T = jnp.fft.ifftn(omega_hatT).real
    return omega_T
...
RUN_RESULTS_DIR = os.path.join(RUN_RESULTS_ROOT, f'navier_stokes_bespoke_{RUN_TIMESTAMP}')
os.makedirs(RUN_RESULTS_DIR, exist_ok=True)
RUN_RESULTS_STEM = f'navier_stokes_bespoke_{RUN_TIMESTAMP}'
...
# Configuration for Navier-Stokes inversion
ACTIVE_DIM = num_truncated_series  # Latent dimension (KL modes)
NOISE_STD = 0.025               # Observation noise std
...
def reconstruct_initial_vorticity(latents):
    """Map latent coefficients -> initial vorticity field on the periodic grid."""
    ...
    fields = fields_flat.reshape(latents.shape[0], N, N)
...
true_final_field = solve_final_vorticity_field(alpha_true_lat)
...
print(
    f"{'Method':<24} | {'RMSE_a':<9} | {'RMSE_omega':<11} | {'FwdRel':<9} | "
...
repro_log_path = save_reproducibility_log(
    extra_sections={
        ...
        'basis_generation': {
            'basis_source': 'internally generated periodic Fourier KL-like basis',
...
print('\nVisualizing Navier-Stokes initial-vorticity fields, final-time vorticity, and sensor residuals...')
...
# --- Figure 1: Initial vorticity reconstruction ---
...
axes[0, 0].scatter(obs_col, obs_row, c='lime', s=8, marker='.', alpha=0.6, label='Sensors')
axes[0, 0].set_title(r'Ground Truth\nInitial Vorticity $\omega_0$', fontsize=18)
axes[0, 0].axis('off')
...
for i, label in enumerate(methods_to_plot):
    ...
    axes[0, col].scatter(obs_col, obs_row, c='lime', s=8, marker='.', alpha=0.4)
...
# --- Figure 2: Final-time vorticity comparison ---
...
plt.suptitle(
    f"Inverse Navier-Stokes (Latent Dim={d_lat}): Final-Time Vorticity",
    fontsize=16, y=1.05,
)
plt.tight_layout()
plt.show()
...
sc0 = axes3[0].scatter(obs_col, obs_row, c=resid0, cmap='inferno', s=40, vmin=0.0, vmax=1.0)
...
plt.suptitle(
    f"Inverse Navier-Stokes (Latent Dim={d_lat}): Final-Time Sensor Residuals",
    fontsize=16, y=1.05,
)
plt.tight_layout()
plt.show()
