...
HLSI (Hessian-Laplace Score Identity) posterior sampling for the 2D
Navier-Stokes initial-vorticity inverse problem.

Spine identical to darcy_flow.py / eit.py: only the physics layer (Sections 0-2)
and scientific visualization (Section 13) are changed. The HLSI core
(Sections 3-9) is the common shared apparatus.

Physics
-------
Vorticity-streamfunction formulation of the 2D incompressible Navier-Stokes
equations on the unit torus T^2 = [0,1]^2:
    dt omega + u . grad omega = nu * Laplace omega + f
    - Laplace psi = omega
    u = grad^perp psi = (dy psi, -dx psi)

Forcing    f(x,y) = 0.1 * ( sin(2*pi*(x+y)) + cos(2*pi*(x+y)) )
Viscosity  nu     = 1e-3
End time   T      = 10.0
Time step  dt     = 0.05   (200 steps)

Discretization: Fourier pseudo-spectral with 2/3 dealiasing; IMEX time
stepping (Crank-Nicolson on the viscous term, explicit Euler on advection
plus forcing).

Inverse problem
---------------
Unknown:  initial vorticity  omega_0(x) = sum_j  phi_j(x) * alpha_j
Latent:   alpha in R^{24}, alpha ~ N(0, I)   (KL coefficients)
Data:     omega(x, T) sampled at 100 fixed random grid points + Gaussian noise

The KL basis is constructed in-script from the eigendecomposition of a
periodic exponential (Matern-1/2-like) kernel on the 32x32 grid. This
replaces the external basis_modes.csv input used in the previous NS setup
so that the basis is a controlled variable directly comparable to the
Darcy/EIT scripts. Everything downstream of `Basis` is identical to the
other physics scripts, which isolates any behavioral difference (including
the PoU-HLSI blowup we're investigating) to the physics and basis layers.
"""


# ==========================================
# 0. KL BASIS GENERATION (periodic Matern-1/2 on the unit torus)
# ==========================================
import numpy as np
import os
...
N = 32
# Periodic grid: x_k = k/N for k = 0, ..., N-1 (no endpoint duplication).
x_grid_1d = np.arange(N, dtype=np.float64) / N
X_grid_np, Y_grid_np = np.meshgrid(x_grid_1d, x_grid_1d, indexing='ij')
coords = np.column_stack([X_grid_np.ravel(), Y_grid_np.ravel()])   # (1024, 2)

# Minimum-image periodic distance on the unit torus.
dx_mat = coords[:, None, 0] - coords[None, :, 0]
dy_mat = coords[:, None, 1] - coords[None, :, 1]
dx_mat = dx_mat - np.round(dx_mat)
dy_mat = dy_mat - np.round(dy_mat)
dists_periodic = np.sqrt(dx_mat ** 2 + dy_mat ** 2)

# Matern-1/2-like (exponential) covariance.  ell = 0.1 gives a prior with
# correlation length ~ 3 grid cells, matching the regime in which the
# external Basis_Modes.csv was built.
ell = 0.1
sigma_prior = 1.0
C_prior = (sigma_prior ** 2) * np.exp(-dists_periodic / ell)

# Eigendecompose: cov matrix is symmetric PSD.
eigvals, eigvecs = np.linalg.eigh(C_prior)
idx = np.argsort(eigvals)[::-1]
eigvals = eigvals[idx]
eigvecs = eigvecs[:, idx]
eigvals = np.clip(eigvals, a_min=0.0, a_max=None)

# Keep top q_max KL modes. We truncate further to num_truncated_series below.
q_max = 100
Basis_Modes = eigvecs[:, :q_max] * np.sqrt(eigvals[:q_max])
np.savetxt('data/NS_Basis_Modes.csv', Basis_Modes, delimiter=',')
print(f"Generated periodic Matern-1/2 KL basis: {Basis_Modes.shape} "
      f"(leading eigvals = {eigvals[:5]})")

del dx_mat, dy_mat, dists_periodic, C_prior, eigvecs
...
# ==========================================
# CONFIGURATION GENERATOR (Navier-Stokes)
# ==========================================

N = 32
num_truncated_series = 24    # Latent dimension d (KL modes)
num_observation = 100        # Number of sparse vorticity sensors
seed = 42

# --- Navier-Stokes experiment parameters (must match basis_modes.csv version) ---
L_domain = 1.0               # Unit torus [0, L]^2
T_end = 10.0                 # Final integration time
delta_t = 0.05               # Time step
n_steps_ns = int(round(T_end / delta_t))   # 200
nu_visc = 1e-3               # Viscosity
forcing_amp = 0.1            # Forcing amplitude

print(f"Generating Navier-Stokes configuration with {num_observation} sensors "
      f"on a {N}x{N} periodic grid (latent dim d={num_truncated_series})...")

# 1. Generate Random Observation Locations on the full periodic grid.
# No interior restriction (periodic BC; every node is equivalent).
rng = np.random.RandomState(seed)
dimension_of_PoI = N * N
obs_indices = rng.choice(dimension_of_PoI, size=num_observation, replace=False)
obs_indices = np.sort(obs_indices).astype(np.int64)

# 2. Truncate Basis to num_truncated_series columns.
full_basis = Basis_Modes                                   # (N*N, q_max)
num_modes_available = full_basis.shape[1]
...
# ==========================================
# 2. DEFINE FORWARD SOLVER (Navier-Stokes)
# ==========================================
# PDE (vorticity-streamfunction):
#   dt w + u . grad w = nu * Laplace w + f
#   -Laplace psi = w,   u = (dy psi, -dx psi)
# on T^2 = [0, L]^2 with L = 1.
#
# Discretization: pseudo-spectral FFT + 2/3 dealiasing;
# IMEX time-stepping (CN on viscous, explicit on advection).
# The initial vorticity is parameterized via KL:
#     w_0(x) = sum_j  phi_j(x) * alpha_j,   alpha ~ N(0, I).
# ==========================================

# Integer wavenumbers: jnp.fft.fftfreq(N) * N = [0,1,...,N/2-1,-N/2,...,-1].
_n_1d = (jnp.fft.fftfreq(N) * N).astype(jnp.int32)
_NX_int, _NY_int = jnp.meshgrid(_n_1d, _n_1d, indexing='ij')

# Angular wavenumbers (rad / length) on [0, L]^2 with L = L_domain.
_kx_1d = 2.0 * jnp.pi * jnp.fft.fftfreq(N, d=L_domain / N)
_KX, _KY = jnp.meshgrid(_kx_1d, _kx_1d, indexing='ij')
_K2 = _KX ** 2 + _KY ** 2
# Avoid division by zero at the (0,0) mode (mean of omega is a conserved
# quantity and has no corresponding streamfunction).
_K2_safe = jnp.where(_K2 == 0, 1.0, _K2)
_inv_K2 = jnp.where(_K2 == 0, 0.0, 1.0 / _K2_safe)

# 2/3 dealiasing mask on integer wavenumbers.
_dealias_cutoff = N // 3
_dealias_mask = (jnp.abs(_NX_int) <= _dealias_cutoff) & (jnp.abs(_NY_int) <= _dealias_cutoff)

# Physical-space grid and forcing term.
_x_phys = jnp.linspace(0.0, L_domain, N, endpoint=False)
_X_phys, _Y_phys = jnp.meshgrid(_x_phys, _x_phys, indexing='ij')
_f_phys = forcing_amp * (
    jnp.sin(2.0 * jnp.pi * (_X_phys + _Y_phys))
    + jnp.cos(2.0 * jnp.pi * (_X_phys + _Y_phys))
)
_f_hat = jnp.fft.fft2(_f_phys) * _dealias_mask

# IMEX coefficients (Crank-Nicolson on the viscous term).
_visc_plus = 1.0 + 0.5 * nu_visc * delta_t * _K2
_visc_minus = 1.0 - 0.5 * nu_visc * delta_t * _K2


def _ns_single_step(w_hat, _):
    """One IMEX step of the vorticity equation in Fourier space."""
    # Streamfunction and velocity in Fourier space
    psi_hat = w_hat * _inv_K2
    u_hat =  1j * _KY * psi_hat   # u  = dy psi
    v_hat = -1j * _KX * psi_hat   # v  = -dx psi
    wx_hat = 1j * _KX * w_hat
    wy_hat = 1j * _KY * w_hat

    # Move advection ingredients to physical space (dealiased)
    u  = jnp.real(jnp.fft.ifft2(u_hat * _dealias_mask))
    v  = jnp.real(jnp.fft.ifft2(v_hat * _dealias_mask))
    wx = jnp.real(jnp.fft.ifft2(wx_hat * _dealias_mask))
    wy = jnp.real(jnp.fft.ifft2(wy_hat * _dealias_mask))

    # Nonlinear term u . grad w, back to Fourier, dealiased
    NL_hat = jnp.fft.fft2(u * wx + v * wy) * _dealias_mask

    # IMEX update: (I + nu*dt/2 * K^2) w^{n+1} = (I - nu*dt/2 * K^2) w^n
    #                                            + dt * (f_hat - NL_hat)
    w_hat_new = (_visc_minus * w_hat + delta_t * (_f_hat - NL_hat)) / _visc_plus
    w_hat_new = w_hat_new * _dealias_mask
    return w_hat_new, None


def _integrate_ns(w_hat0, n_steps):
    """Integrate n_steps of the NS vorticity equation from Fourier IC w_hat0."""
    w_hat_final, _ = jax.lax.scan(_ns_single_step, w_hat0, None, length=n_steps)
    return w_hat_final
...
def solve_forward(alpha):
    """
    The Forward Map F(alpha) for Navier-Stokes initial-vorticity inversion.
    Input:  alpha      - KL coefficients, shape (q,)
    Output: y_pred     - final vorticity sampled at sensor locations, shape (n_obs,)
    """
    # 1. KL expansion: alpha -> initial vorticity omega_0 on (N, N)
    omega0 = jnp.reshape(Basis @ alpha, (N, N))
    w_hat0 = jnp.fft.fft2(omega0) * _dealias_mask

    # 2. Integrate n_steps_ns IMEX Crank-Nicolson + explicit advection
    w_hat_T = _integrate_ns(w_hat0, n_steps_ns)

    # 3. Back to physical space
    omega_T = jnp.real(jnp.fft.ifft2(w_hat_T))

    # 4. Extract sparse sensor observations
    y_obs = omega_T.ravel()[obs_locations]
    return y_obs


@jax.jit
def solve_initial_vorticity(alpha):
    """Return the initial vorticity field omega_0 on (N, N) for given alpha."""
    return jnp.reshape(Basis @ alpha, (N, N))


@jax.jit
def solve_final_vorticity(alpha):
    """Return the final vorticity field omega(T) on (N, N) for given alpha."""
    omega0 = jnp.reshape(Basis @ alpha, (N, N))
    w_hat0 = jnp.fft.fft2(omega0) * _dealias_mask
    w_hat_T = _integrate_ns(w_hat0, n_steps_ns)
    return jnp.real(jnp.fft.ifft2(w_hat_T))


print("Navier-Stokes Physics Engine Loaded Successfully.")
print(f"Forward map 'solve_forward' is ready. "
      f"Input dim: {num_truncated_series}, Output dim: {num_observation}")
print(f"NS time stepping: nu={nu_visc}, T={T_end}, dt={delta_t} ({n_steps_ns} steps), "
      f"2/3 dealiasing at |n| <= {_dealias_cutoff}.")
# ==========================================
...
def save_reproducibility_log(extra_sections=None):
    ...
    lines = []
    lines.append('Navier-Stokes HLSI run reproducibility log')
    lines.append('=' * 72)
    ...
    excluded_names = {
        'PRECOMP', 'metrics', 'results_df', 'results_runinfo_df',
        'samples_ns', 'ess_logs_ns', 'mean_fields', 'method_voltage_preds',
        'fields', 'ref_hist', 'model_trace_data'
    }
    config_names = []
    for name, value in globals().items():
        ...
        if name.isupper() or name in {
            'seed', 'device', 'N_REF', 'BUILD_GNL_BANKS',
            'SAMPLER_CONFIGS', 'SAMPLER_CONFIGS_NS',
            'reference_key', 'reference_key_ns',
            'reference_title', 'reference_title_ns',
            'display_names', 'display_names_ns',
            'sampler_run_info', 'sampler_run_info_ns',
            'num_observation', 'num_truncated_series', 'dimension_of_PoI',
            'obs_locations', 'd_lat',
            'delta_t', 'nu_visc', 'L_domain', 'T_end', 'n_steps_ns',
            'forcing_amp', 'ell', 'sigma_prior',
        }:
...
# Configuration for Navier-Stokes
ACTIVE_DIM = num_truncated_series   # Latent dimension (KL modes)
NOISE_STD = 0.025                     # Navier-Stokes observation noise std
...
def ns_hess_lik_jax(alpha_k, y_obs_jax, sigma):
    if USE_GAUSS_NEWTON_HESSIAN:
        return ns_hess_lik_gn_jax(alpha_k, y_obs_jax, sigma)
    return ns_hess_lik_exact_jax(alpha_k, y_obs_jax, sigma)
...
class PhysicsLikelihood:
    """Generic likelihood wrapper: JAX physics -> PyTorch tensors (identical to Darcy)."""
...
# ==========================================
# 10. EXECUTION: NAVIER-STOKES INVERSE PROBLEM
# ==========================================
...
DEFAULT_N_GEN = DEFAULT_N_GEN_NS
...
# In Stage-A PoU mode we switch to a PoU-adjusted local bank
#   (s0_pou_ref, P_pou_ref, mu_pou_ref, log_pou_ref), so PoU-HLSI /
# CE-PoU-HLSI use PoU-adjusted local geometry rather than a query-time
...
# Alias for compatibility with the shared spine (which references SAMPLER_CONFIGS).
SAMPLER_CONFIGS = SAMPLER_CONFIGS_NS

# --- RUN ---
samples_ns, ess_logs_ns, sampler_run_info_ns = run_sampler_suite(
    SAMPLER_CONFIGS_NS, prior_model, lik_model, PRECOMP,
)
sampler_run_info = sampler_run_info_ns

display_names_ns = {label: info.get('display_name', label)
                    for label, info in sampler_run_info_ns.items()}
display_names = display_names_ns
...
for label, raw in samples_ns.items():
    ...
    print(
        f"{display_names_ns.get(label, label):<24} | {rmse_alpha:<10.4f} | "
        f"{rel_alpha:<11.4f} | {mmd:<14.4f} | {ksd:<10.4f} | {kl:<10.4f}"
    )


# ==========================================
# 13. SCIENTIFIC VISUALIZATION (Navier-Stokes-specific)
# ==========================================
#
# The visualization panel mirrors the suite-report Figure for
# Navier-Stokes initial-vorticity inversion:
#   Row 0: ground truth omega_0 / posterior-mean omega_0
#   Row 1: absolute error |mean - truth|
#   Row 2: pointwise posterior standard deviation
#   Row 3: a random random posterior sample
#
# A secondary figure shows the final-time vorticity omega(T) on the
# ground truth and on each method's mean, overlaid with the 100 sensor
# locations.  A third figure compares the observed sensor vorticities to
# the forward map evaluated at each posterior mean.
# ==========================================
...
def reconstruct_initial_vorticity(latents):
    """Map KL coefficients -> initial vorticity field omega_0 on (N, N) grid."""
...
def solve_final_vorticity_np(alpha_vec):
    """Solve NS for given KL coefficients, return omega(T) on (N, N) as numpy."""
    return np.array(solve_final_vorticity(jnp.array(alpha_vec)))
...
true_field = reconstruct_initial_vorticity(alpha_true_lat)[0]
true_final_field = solve_final_vorticity_np(alpha_true_lat)
...
for label, samps in samples_ns.items():
    ...
    print(f"{display_names_ns.get(label, label):<24} | {inv_rel_l2_pct:<12.4f} | "
...
plot_normalizer_key = resolve_plot_normalizer(
    ...
    fallback=reference_key_ns,
    best_metric_key='RelL2_field',
)
plot_normalizer_title = display_names_ns.get(plot_normalizer_key, plot_normalizer_key)
...
plot_pca_histograms(
    ...
    metrics_dict=metrics,
    fallback_key=reference_key_ns,
)

# Paper-ready table
print('\n=== Final Navier-Stokes metrics (paper-ready) ===')
print(
...
for label, data in metrics.items():
    ...
    print(
        f"{display_names_ns.get(label, label):<24} | "
...
repro_log_path = save_reproducibility_log(
    extra_sections={
        ...
        'summary_stats': {
            ...
            'num_methods_with_samples': len(samples_ns),
            'num_methods_with_mean_fields': len(mean_fields),
            'num_methods_with_ess_logs': len(ess_logs_ns),
        },
    }
)
...
print('\nVisualizing Initial Vorticity Reconstructions (4-row panel)...')

methods_to_plot = [label for label in samples_ns.keys() if label in mean_fields]
n_cols = len(methods_to_plot) + 1

# --- Figure 1: Initial-vorticity 4-row panel (suite-report style) ---
...
vis_anchor_title = display_names_ns.get(vis_anchor_key, vis_anchor_key)
vis_reference_field = true_field if has_true_field else mean_fields[vis_anchor_key]

# Use the ground-truth field (when available) to normalize the top/bottom
# rows on a stable, physically meaningful scale.
vmin = float(np.min(vis_reference_field))
vmax = float(np.max(vis_reference_field))
# Symmetrize for a diverging colormap (vorticity is signed)
...
if vis_anchor_key in samples_ns and vis_anchor_key in mean_fields:
    ...
    if anchor_vis_samps.shape[0] > 0:
        anchor_vis_fields = reconstruct_initial_vorticity(anchor_vis_samps[:, :d_lat])
...
if vis_anchor_key not in mean_fields:
    ...
    for label in methods_to_plot:
        ...
        if samps.shape[0] > 0:
            fields = reconstruct_initial_vorticity(samps[:, :d_lat])
            max_std = max(max_std, float(np.std(fields, axis=0).max()))

im0 = axes[0, 0].imshow(vis_reference_field, cmap='RdBu_r', origin='lower',
                        vmin=vmin, vmax=vmax)
axes[0, 0].scatter(obs_col, obs_row, c='lime', s=8, marker='.', alpha=0.7, label='Sensors')
if has_true_field:
    axes[0, 0].set_title('Ground Truth\nInitial Vorticity $\\omega_0(x)$', fontsize=18)
else:
    axes[0, 0].set_title(f"Normalizer\n{vis_anchor_title} $\\omega_0(x)$", fontsize=18)
axes[0, 0].axis('off')
...
for i, label in enumerate(methods_to_plot):
    ...
    # Row 0: Mean posterior initial vorticity
    ...
    # Row 3: A random posterior sample (raw vorticity, same colorscale as row 0)
...
plt.suptitle(
    f"Inverse Navier-Stokes (Latent Dim={d_lat}): Initial Vorticity Reconstruction",
    fontsize=18, y=1.02,
)
plt.tight_layout()
plt.show()
...
# --- Figure 2: Final-time vorticity omega(T) ---
print('\nVisualizing Final-Time Vorticity Fields omega(T)...')
fig2, axes2 = plt.subplots(1, n_cols, figsize=(4 * n_cols, 4))

has_true_final = 'true_final_field' in globals() and true_final_field is not None
final_vabs = max(abs(float(np.min(true_final_field))),
                 abs(float(np.max(true_final_field))), 1e-12)
final_vmin, final_vmax = -final_vabs, final_vabs

axes2[0].imshow(true_final_field, cmap='RdBu_r', origin='lower',
                vmin=final_vmin, vmax=final_vmax)
axes2[0].scatter(obs_col, obs_row, c='lime', s=12, marker='.', alpha=0.7, label='Sensors')
axes2[0].set_title('Ground Truth\nFinal Vorticity $\\omega(x,T)$', fontsize=14)
axes2[0].axis('off')
axes2[0].legend(fontsize=8, loc='upper right')

for i, label in enumerate(methods_to_plot):
    col = i + 1
    samps_clean = get_valid_samples(samples_ns[label])
    if samps_clean.shape[0] < 10:
        axes2[col].axis('off')
        continue
    mean_lat = np.mean(samps_clean, axis=0)[:d_lat]
    try:
        w_final = solve_final_vorticity_np(mean_lat)
        axes2[col].imshow(w_final, cmap='RdBu_r', origin='lower',
                          vmin=final_vmin, vmax=final_vmax)
        axes2[col].scatter(obs_col, obs_row, c='lime', s=10, marker='.', alpha=0.5)
        axes2[col].set_title(f"{display_names_ns.get(label, label)}\n$\\omega(x,T)$", fontsize=14)
    except Exception:
        axes2[col].set_title(f"{display_names_ns.get(label, label)}\n(solve failed)", fontsize=14)
    axes2[col].axis('off')

plt.suptitle(
    f"Inverse Navier-Stokes (Latent Dim={d_lat}): Final-Time Vorticity $\\omega(T)$",
    fontsize=16, y=1.05,
)
plt.tight_layout()
plt.show()
...
# --- Figure 3: Sensor observation fit ---
print('\nVisualizing Sensor Vorticity Profiles...')
...
for label in methods_to_plot:
    ...
    mean_lat = np.mean(samps_clean, axis=0)[:d_lat]
    try:
        method_sensor_preds[label] = np.array(solve_forward(jnp.array(mean_lat)))
    except Exception:
        pass
...
labels_for_sensor_plot = [lab for lab in preferred_order if lab in method_sensor_preds]
if len(labels_for_sensor_plot) == 0:
    labels_for_sensor_plot = list(method_sensor_preds.keys())

sensor_order = np.argsort(y_clean_np)

fig3, ax3 = plt.subplots(1, 1, figsize=(10, 5))
ax3.plot(np.arange(num_observation), y_clean_np[sensor_order], 'k-o',
         label='Clean', linewidth=2, markersize=4)
ax3.plot(np.arange(num_observation), y_obs_np[sensor_order], 'r.',
         label='Noisy obs', markersize=6, alpha=0.7)

for label in labels_for_sensor_plot:
    y_pred_full = method_sensor_preds[label]
    ax3.plot(
        np.arange(num_observation),
        y_pred_full[sensor_order],
        '--',
        label=display_names_ns.get(label, label),
        linewidth=1.5,
        alpha=0.85,
    )

ax3.set_xlabel('Sensor index (sorted by clean $\\omega(x,T)$)', fontsize=13)
ax3.set_ylabel('Sensor vorticity $\\omega(x_i, T)$', fontsize=13)
ax3.set_title('Navier-Stokes Sensor Fit (posterior mean forward predictions)', fontsize=14)
...
print('\n=== Navier-Stokes HLSI Pipeline Complete ===')
