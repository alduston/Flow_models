# -*- coding: utf-8 -*-
"""Navier-Stokes inverse-problem density-evaluation benchmark.

Refactor of the moderately hard anisotropic Navier-Stokes script to match the
Darcy density benchmark: build a MALA posterior reference bank, evaluate
...
Useful overrides:
    ...
    %env IP_DENSITY_DRC_PF_STEPS=32
    %env IP_DENSITY_DRC_PLOT_LAYOUT=comparison_grid
...
# Density-evaluation correction-factor benchmark.  By default, the score field
# is fitted on MALA while density values are evaluated on an independent MALA-EVAL
# bank so score-reference particles and density-evaluation locations do not overlap.
...
DENSITY_BLEND_DIVERGENCE = os.environ.get('IP_DENSITY_BLEND_DIVERGENCE',     'auto')            #'auto')
...
run_ctx = init_run_results('navier_stokes_density_eval_bench')
...
dashboard = DashboardPDF(
    DASHBOARD_PDF_PATH,
    title='Navier-Stokes density-evaluation dashboard',
)
dashboard.add_text_page(
    'Navier-Stokes density-evaluation dashboard',
    [
        f"Created: {datetime.now().isoformat(timespec='seconds')}",
        'This dashboard matches the Darcy density-evaluation benchmark, with configurable score-signal, gate, and density-evaluation banks. By default, density values are evaluated on MALA-EVAL rather than on the score-reference bank.',
...
results_df, results_runinfo_df, results_df_path, results_runinfo_df_path = save_results_tables(
    ...
    sampler_run_info,
    n_ref=N_REF_SIGNAL,
    target_name='Moderately hard anisotropic Navier-Stokes density evaluation',
    display_names=display_names,
...
save_reproducibility_log(
    title='Moderately hard anisotropic Navier-Stokes density-evaluation reproducibility log',
    config={
        'seed': seed,
        'ACTIVE_DIM': ACTIVE_DIM,
        'N_REF': N_REF,
