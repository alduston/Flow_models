# -*- coding: utf-8 -*-
"""Navier-Stokes inverse-problem LFGI-GAD versus MALA benchmark.
...
run_ctx = init_run_results('navier_stokes_lfgi_gad1_gad3_mala_20260814')
...
RUN_COMMAND_HINT = (
    ...
    'python navier_stokes_gad_mala_comparison_20260814.py'
).format(
    n_ref=N_REF,
...
dashboard = DashboardPDF(
    DASHBOARD_PDF_PATH,
    title='Navier-Stokes LFGI-GAD versus MALA dashboard',
)
dashboard.add_text_page(
    'Navier-Stokes LFGI-GAD versus MALA dashboard',
    [
...
results_df, results_runinfo_df, results_df_path, results_runinfo_df_path = save_results_tables(
    ...
    sampler_run_info,
    n_ref=N_REF,
    target_name='Navier-Stokes initial vorticity',
    display_names=display_names,
...
save_reproducibility_log(
    title='Navier-Stokes LFGI-GAD versus MALA reproducibility log',
    config={
        'seed': seed,
        'ACTIVE_DIM': ACTIVE_DIM,
        'N_REF': N_REF,
...
plt.suptitle('Navier-Stokes LFGI-GAD versus MALA: observation geometry', fontsize=15)
plt.tight_layout()
plt.show()
...
plot_field_reconstruction_grid(
    ...
    suptitle=f'Navier-Stokes LFGI-GAD versus MALA (d={ACTIVE_DIM}): initial vorticity reconstruction',
...
for i, label in enumerate(methods_to_plot):
    ...
    axes2[col].axis('off')
plt.suptitle(
    f'Navier-Stokes LFGI-GAD versus MALA (d={ACTIVE_DIM}): final-time vorticity',
    fontsize=16,
    y=1.05,
)
plt.tight_layout()
...
plt.suptitle(
    f'Navier-Stokes LFGI-GAD versus MALA (d={ACTIVE_DIM}): sensor residuals',
    fontsize=16,
    y=1.05,
)
plt.tight_layout()
...
print('\n=== Navier-Stokes LFGI-GAD versus MALA comparison pipeline complete ===')
