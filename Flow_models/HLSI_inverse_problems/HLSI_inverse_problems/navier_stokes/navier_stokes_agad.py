...
Runs the same comparison as the Darcy and Allen--Cahn experiments: GN-LFGI
with completed flow ratio, GN-LFGI with static reweighting, local matrix/scalar
blends, global matrix blend, and Tweedie.  Each method performs three
transport/ratio rounds and reports transport rounds 1 and 3.  Flow-ratio nodes
move a fresh unweighted cloud with the completed correction; the static
baseline instead carries endpoint log weights into the next LFGI transport
round.
...
RUN_COMMAND_HINT = (
    ...
    'IP_ITER_DENSITY_STEPS={density_steps} python navier_stokes_static_lfgi_comparison.py'
).format(
    n_ref=N_REF,
...
dashboard.add_text_page(
    ...
    [
        ...
        'This dashboard contains the finalized iterative-transport comparison, with the same base estimator used for each transport node and its associated completed flow-ratio node.',
        'Displayed methods are transport rounds 1 and 3 only. Hidden flow-ratio nodes move fresh unweighted clouds, while the GN-LFGI static baseline carries endpoint log weights into its next transport round.',
...
print('\n=== Navier-Stokes iterative transport comparison pipeline complete ===')
