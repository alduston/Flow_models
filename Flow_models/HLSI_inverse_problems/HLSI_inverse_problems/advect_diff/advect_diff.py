...
Original file is located at
    https://colab.research.google.com/drive/12N3wqq-XnLU1y-8YL_S3Dwaz2EjOo9wj
"""

# -*- coding: utf-8 -*-
"""advect_diff_hlsi.ipynb
...
# -*- coding: utf-8 -*-
"""advection_diffusion_ic_wc_ce_hlsi.py
HLSI (Hessian-Laplace Score Identity) posterior sampling for a
2D advection-diffusion initial-condition inverse problem.
Refactored from the Darcy-flow notebook by changing only the physics layer
and the domain-specific visualizations. The HLSI / WC / CE / PoU machinery
below is intentionally left structurally unchanged.
PDE:      u_t + v·∇u = κ Δu    on [0,1]^2 with periodic boundary conditions
...
"""
# One-time: generate KL basis for the advection-diffusion initial condition prior
import numpy as np
...
# ==========================================
# CONFIGURATION GENERATOR (Advection-Diffusion IC inversion)
# ==========================================
N = 32
num_observation = 100
num_truncated_series = 32
seed = 42
print(f"Generating advection-diffusion configuration for {num_observation} observations...")
...
try:
    df_modes = pd.read_csv('data/AdvecDiff_Basis_Modes.csv', header=None)
    if isinstance(df_modes.iloc[0, 0], str):
        df_modes = pd.read_csv('data/AdvecDiff_Basis_Modes.csv')
    ...
    raise FileNotFoundError(
        "Could not find 'data/AdvecDiff_Basis_Modes.csv'. "
...
print("Initializing JAX advection-diffusion physics engine...")
# ==========================================
...
try:
    ...
    raise
# ==========================================
# 2. DEFINE FORWARD SOLVER (Advection-Diffusion)
# ==========================================
# PDE:  u_t + v·∇u = κ Δu  on [0,1]^2, periodic BC.
# Unknown: initial condition u0(x) represented in the KL basis.
# We use the exact Fourier-space propagator for constant velocity and diffusion.
# ==========================================
...
def solve_forward(alpha):
    """
    Forward map F(alpha) for advection-diffusion IC inversion.
...
def solve_full_state(alpha, t=OBS_TIME):
    """Return the full propagated state u(x,t) on the N x N grid."""
    u0 = jnp.reshape(Basis @ alpha, (N, N))
...
print("Advection-diffusion physics engine loaded successfully.")
...
RUN_RESULTS_DIR = os.path.join(RUN_RESULTS_ROOT, f'advect_diff_hlsi_{RUN_TIMESTAMP}')
os.makedirs(RUN_RESULTS_DIR, exist_ok=True)
RUN_RESULTS_STEM = f'advect_diff_hlsi_{RUN_TIMESTAMP}'
...
def save_reproducibility_log(extra_sections=None):
    ...
    lines = []
    lines.append('Advection-diffusion IC HLSI run reproducibility log')
    lines.append('=' * 72)
...
# ==========================================
print(f"--- Setting up {ACTIVE_DIM}D advection-diffusion IC inverse problem ---")
import gc
gc.collect()
if torch.cuda.is_available():
    torch.cuda.empty_cache()
...
# ==========================================
# 13. SCIENTIFIC VISUALIZATION (Advection-diffusion specific)
# ==========================================
...
print('\n=== Physical parameter space metrics (Initial condition + propagated state) ===')
...
print('\n=== Final advection-diffusion metrics (paper-ready) ===')
print(
...
print('\nVisualizing initial condition fields, propagated states, and sensor residual maps...')
...
vmin = float(np.min(true_u0))
vmax = float(np.max(true_u0))
...
if vis_anchor_key in samples_darcy and vis_anchor_key in mean_fields:
    ...
    if anchor_vis_samps.shape[0] > 0:
        ...
        max_err = max(1e-12, float(np.abs(mean_fields[vis_anchor_key] - true_u0).max()))
...
im0 = axes[0, 0].imshow(true_u0, cmap='RdBu_r', origin='lower', vmin=vmin, vmax=vmax)
...
axes[3, 0].imshow(true_u0, cmap='RdBu_r', origin='lower', vmin=vmin, vmax=vmax)
...
if vis_anchor_key not in mean_fields:
    ...
    for label in methods_to_plot:
        ...
        max_err = max(max_err, np.abs(mean_f - true_u0).max())
...
plt.suptitle(
    f"Advection-Diffusion IC Inversion (Latent Dim={d_lat}): Initial Condition Reconstruction",
    fontsize=18, y=1.02,
)
plt.tight_layout()
plt.show()
...
# --- Figure 2: Final-time state comparison ---
...
for i, label in enumerate(methods_to_plot):
    ...
    axes2[col].imshow(mean_uT, cmap='viridis', origin='lower',
                      vmin=true_uT.min(), vmax=true_uT.max())
...
plt.suptitle(
    f"Advection-Diffusion IC Inversion (Latent Dim={d_lat}): State at Observation Time",
    fontsize=16, y=1.05,
)
plt.tight_layout()
plt.show()
...
plt.suptitle(
    f"Advection-Diffusion IC Inversion (Latent Dim={d_lat}): Sensor-Space Residuals",
    fontsize=16, y=1.05,
)
plt.tight_layout()
plt.show()
