"""CIFAR CSEM T_K-aware scale-anchor comparison v1.

Derived from csem_split_metric_TKxCSEM_fine_cifar_v1.py. Adds two coherent
T_K-aware continuations of the historical encoder-mean GroupNorm anchor:
  * ou_visible: soft penalty on per-example mean/variance mismatch still visible
    after OU diffusion to T_K; vanishes as T_K -> infinity.
  * ou_partial: parameter-free layer equal to historical GN0 at T_K=0 and
    continuously approaching identity as T_K -> infinity.
The existing GN0 and terminal component-KL mechanisms are otherwise retained.
"""
...
def validate_terminal_time_contract(
    ...
    Terminal-KL runs require either T_K=0 (the two-stage LDM limit) or
    t_min < T_K <= T.  If ``schedule`` is supplied it is the full score
    schedule, so its endpoint must equal T, not T_K.
    """
    ...
    if not math.isfinite(T_K) or T_K < 0.0:
        raise ValueError(
            f"Terminal-KL mode requires finite T_K>=0; got T_K={T_K}"
        )
    if 0.0 < T_K <= t_min:
        raise ValueError(
            f"For a nonzero representation horizon require T_K>t_min; "
            f"got T_K={T_K}, t_min={t_min}. Use T_K=0 for the exact LDM limit."
        )
...
def ou_visible_groupnorm_penalty(mu: torch.Tensor, t_value: float) -> torch.Tensor:
    """Soft GN-like penalty on the per-example moment defect still visible at OU time t.

    Historical one-group non-affine GroupNorm enforces, per example, zero mean
    and unit biased variance across the complete latent tensor.  Under OU, the
    visible mean/variance defects are attenuated to alpha*m and
    alpha^2*(v-1), because sigma^2=1-alpha^2.  This penalty is the diagonal
    Gaussian moment mismatch of that diffused per-example empirical law.

    At t=0 its zero set is exactly the historical GN moment manifold
    (mean=0,var=1). As t->infinity it vanishes for every finite-energy mu.
    """
    t_value = float(t_value)
    if t_value < 0.0 or not math.isfinite(t_value):
        raise ValueError(f"OU-visible anchor time must be finite and nonnegative, got {t_value}")
    flat = mu.flatten(1)
    mean0 = flat.mean(dim=1)
    centered = flat - mean0[:, None]
    var0 = centered.square().mean(dim=1)  # biased variance, matching GroupNorm convention
    alpha = math.exp(-t_value)
    alpha2 = alpha * alpha
    sigma2 = 1.0 - alpha2
    mean_t = alpha * mean0
    var_t = sigma2 + alpha2 * var0
    return 0.5 * (mean_t.square() + var_t - torch.log(var_t.clamp_min(1e-8)) - 1.0).mean()


def ou_partial_groupnorm(mu: torch.Tensor, t_value: float, eps: float = 1e-5) -> torch.Tensor:
    """Parameter-free temporal continuation of historical hard GroupNorm.

    Let rho_m=alpha_t^2 and rho_v=alpha_t^4. The transform corrects only the
    fraction of the original per-example mean/variance defect that remains
    visible after OU smoothing. It equals one-group affine-free GroupNorm at
    t=0 (up to the same eps convention) and becomes the identity as t->infinity.
    """
    t_value = float(t_value)
    if t_value < 0.0 or not math.isfinite(t_value):
        raise ValueError(f"OU-partial anchor time must be finite and nonnegative, got {t_value}")
    flat = mu.flatten(1)
    mean0 = flat.mean(dim=1, keepdim=True)
    centered = flat - mean0
    var0 = centered.square().mean(dim=1, keepdim=True)
    alpha = math.exp(-t_value)
    rho_m = alpha ** 2
    rho_v = alpha ** 4
    target_mean = (1.0 - rho_m) * mean0
    target_var = (1.0 - rho_v) * var0 + rho_v
    # eps interpolation guarantees exact identity when rho_v=0 while matching
    # GroupNorm's sqrt(var+eps) denominator when rho_v=1.
    scale = torch.sqrt(
        (target_var + (1.0 - rho_v) * eps) / (var0 + eps)
    )
    transformed = target_mean + scale * centered
    return transformed.view_as(mu)
...
class VAE(nn.Module):
    ...
    def __init__(
        ...
        self.latent_anchor_mode = str(latent_anchor_mode).lower()
        self.latent_anchor_time = float(latent_anchor_time)
        if self.latent_anchor_mode not in {"none", "gn0", "ou_visible", "ou_partial"}:
            raise ValueError(f"Unknown latent_anchor_mode={self.latent_anchor_mode!r}")
        if self.use_norm and self.latent_anchor_mode not in {"none", "gn0"}:
            raise ValueError("use_norm cannot be combined with OU-visible/OU-partial anchor modes")
        ...
        # GroupNorm on mu
        if self.use_norm or self.latent_anchor_mode == "gn0":
    ...
    def encode(self, x: torch.Tensor, y: torch.Tensor | None = None):
        ...
        if self.use_norm or self.latent_anchor_mode == "gn0":
            mu = self.gn_mu(mu)
        elif self.latent_anchor_mode == "ou_partial":
            mu = ou_partial_groupnorm(mu, self.latent_anchor_time, eps=1e-5)
        # ou_visible is deliberately a loss-level anchor: it leaves mu itself unchanged.
...
def build_empirical_qT_init_bank(
    ...
    *,
    ...
    requested_terminal = (
        float(cfg.get("t_max", 0.0))
        if terminal_time is None
        else float(terminal_time)
    )
    if requested_terminal == 0.0:
        # Exact T_K=0 endpoint: the oracle initialization law is simply q_0.
        return z0

    bank_cfg = dict(cfg)
    bank_cfg["t_max"] = requested_terminal
    ...
    for scfg in configs:
        ...
        with torch.no_grad():
            if method == "VAE_Rec_eps":
                ...
                direct_oracle_q0 = (
                    init_mode == "oracle-qtk-class"
                    and float(cfg.get("T_terminal", 0.0)) == 0.0
                )
                if direct_oracle_q0:
                    # Reversal from T_K=0 to 0 has zero path length.
                    fake_latents = oracle_qtk_bank_all.clone()
                    fake_imgs_list = []
                    for i in range(0, target_count, bs):
                        z_gen = fake_latents[i:i + bs].to(device)
                        t_dec = (
                            torch.full(
                                (z_gen.shape[0],),
                                decode_time,
                    ...
                    fake_flat = fake_latents.reshape(
                        fake_latents.shape[0], -1
                    ).to(device)
                    w2 = compute_sw2(
                        real_flat_A,
                        fake_flat,
...
def train_vae_cotrained_cond(cfg):
    ...
    # For T_K>0, the representation sampler is strictly t<T_K.  For a
    # discrete grid we place its final node at the immediately preceding
    # representable float, while K_{T_K} uses a separate schedule ending
    # exactly at T_K.  At T_K=0 the representation diffusion interval is
    # empty: reconstruction/KL are evaluated directly at z_0 and no CSEM
    # outer gradient is present.
    rep_cfg = dict(cfg)
    if representation_active:
        ...
        ou_sched = None
        kl_sched = None
        score_time_weighting = str(cfg.get("score_time_weighting", "unweighted-eps"))
        if score_time_weighting not in SCORE_TIME_WEIGHTING_CHOICES:
            raise ValueError(
                f"Unknown outer score time weighting {score_time_weighting!r}"
            )
    ...
    T_rep_steps = int(ou_sched["T"].item()) if representation_active else 0
    ...
    noise_sched = ou_sched if representation_active else score_sched
    schedule_name = (
        ou_sched["schedule_type"] if representation_active
        else score_sched["schedule_type"]
    )
    rep_desc = (
        f"representation gradients t<T_K={T_K:g} ({T_rep_steps} nodes)"
        if representation_active
        else "T_K=0: no diffusion-derived representation gradient"
    )
    print(
        f"--> Time schedule: {schedule_name} | {rep_desc} | "
    ...
    vae = VAE(
                  ...
                  latent_anchor_mode=cfg.get("latent_anchor_mode", "none"),
                  latent_anchor_time=float(cfg.get("latent_anchor_time", cfg.get("T_terminal", 0.0))),
    ...
    for ep in range(cfg["epochs_vae"]):
        ...
        # At T_K=0 the CSEM representation interval is empty by definition.
        active_score_w_vae = score_w_vae if representation_active else 0.0
        metrics = {
            k: 0.0 for k in [
                "loss", "recon", "kl", "terminal_kl", "anchor_penalty", "anchor_objective", "latent_mu_sq",
        ...
        for x, y in tqdm(train_l, desc=f"Ep {ep+1}", leave=False):
            ...
            # --- Representation time / forward process (BEFORE decode) ---
            t_idx = None
            if representation_active:
                if str(cfg.get("time_schedule", "")).lower() in ("flow", "flow_matching"):
                ...
                # Exact standard-LDM first-stage limit: reconstruct at z_0.
                t = torch.zeros(B, device=device, dtype=z0.dtype)
                alpha = torch.ones(
                    (B, 1, 1, 1), device=device, dtype=z0.dtype
                )
            ...
            if reg_type == "normal":
                ...
                if representation_active:
                    ...
                    # K_0 = E_x KL(q_phi(z_0|x) || N(0,I)).
                    kl = -0.5 * torch.mean(
                        1.0 + logvar - mu.pow(2) - var_0
                    )

            else:
                raise ValueError(f"Unknown kl_reg_type: {reg_type}")

            # --- T_K-aware GroupNorm-like anchor ---
            latent_anchor_mode = str(cfg.get("latent_anchor_mode", "none")).lower()
            if latent_anchor_mode == "ou_visible":
                anchor_penalty = ou_visible_groupnorm_penalty(
                    mu, float(cfg.get("latent_anchor_time", T_K))
                )
                anchor_weight = float(cfg.get("ou_visible_anchor_w", 1.0))
            else:
                # GN0 and OU-partial are layer-level interventions and therefore
                # have no additional soft anchor objective.
                anchor_penalty = ou_visible_groupnorm_penalty(
                    mu.detach(), float(cfg.get("latent_anchor_time", T_K))
                )
                anchor_weight = 0.0
            anchor_objective = anchor_weight * anchor_penalty
            ...
            if representation_active:
                ...
                score_time_weights = torch.zeros(
                    B, device=device, dtype=z0.dtype
                )
                ...
                if representation_active:
                    with torch.no_grad():
                        ...
                        weighted_mse_per_example = (
                            score_weights_detached
                            * eps_mse_per_example.detach().float()
                        )
                        for time_bin_index in range(time_diagnostic_bins):
                            ...
                            time_bin_unweighted_sums[time_bin_index] += float(
                                eps_mse_per_example.detach().float()[in_bin]
                                .sum().cpu().item()
                            )
            ...
            if freeze_score_in_cotrain:
                ...
                loss_joint = reconstruction_objective + kl_objective + anchor_objective
            else:
            ...
            metrics["anchor_penalty"] += float(anchor_penalty.detach().cpu().item())
            metrics["anchor_objective"] += float(anchor_objective.detach().cpu().item())
        ...
        epoch_metrics = {
            ...
            "anchor_penalty": metrics["anchor_penalty"] / n_batches,
            "anchor_objective": metrics["anchor_objective"] / n_batches,
            "latent_anchor_mode": str(cfg.get("latent_anchor_mode", "none")),
            "latent_anchor_time": float(cfg.get("latent_anchor_time", T_K)),
            "ou_visible_anchor_w": float(cfg.get("ou_visible_anchor_w", 1.0)),
        ...
        print(
            ...
            f"A_GN(TK): {epoch_metrics['anchor_penalty']:.5f} | "
...
def main():
    ...
    parser.add_argument(
        ...
        help="Random seed for model initialization, training, and fixed evaluation banks.",
    )
    parser.add_argument(
        "--latent-anchor-mode",
        choices=["none", "gn0", "ou_visible", "ou_partial"],
        default="none",
        help=(
            "Representation scale anchor. gn0 is exact historical one-group non-affine "
            "GroupNorm on encoder mu; ou_visible is a soft OU-visible moment mismatch "
            "penalty at T_K; ou_partial is a parameter-free layer that equals GN0 at "
            "T_K=0 and tends to identity as T_K grows."
        ),
    )
    parser.add_argument(
        "--ou-visible-anchor-w", type=float, default=1.0,
        help="Coefficient for the OU-visible GN penalty; ignored by other anchor modes.",
    )
    parser.add_argument(
        "--terminal-kl-w",
        type=float,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "T_K=0 is supported exactly and gives the two-stage LDM optimization limit."
        ),
    )
    parser.add_argument(
    ...
    if not math.isfinite(T_terminal) or T_terminal < 0.0:
        raise ValueError(
            f"--T-terminal/--T-K must be finite and nonnegative; got {T_terminal}"
        )
    if 0.0 < T_terminal <= t_min:
        raise ValueError(
            "--T-terminal/--T-K must either be exactly zero (LDM limit) or "
            f"greater than t_min={t_min}; got T_K={T_terminal}"
        )
    ...
    if not math.isfinite(float(args.ou_visible_anchor_w)) or float(args.ou_visible_anchor_w) < 0.0:
        raise ValueError("--ou-visible-anchor-w must be finite and nonnegative")
    ...
    cfg_shared = {
        ...
        "latent_anchor_time": T_terminal,
        "ou_visible_anchor_w": float(args.ou_visible_anchor_w),
    ...
    cfg_terminal.update({
        "use_latent_norm": bool(args.latent_anchor_mode == "gn0"),
        "latent_anchor_mode": str(args.latent_anchor_mode),
        "latent_anchor_time": T_terminal,
        "ou_visible_anchor_w": float(args.ou_visible_anchor_w),
    ...
    if "terminal_kl" in arms:
        ...
        print(f"  anchor_mode     = {cfg_terminal['latent_anchor_mode']}")
        print(f"  anchor_time     = {cfg_terminal['latent_anchor_time']}")
        print(f"  anchor_w        = {cfg_terminal['ou_visible_anchor_w']} (OU-visible only)")
        print(f"  seed            = {cfg_terminal['seed']}")
