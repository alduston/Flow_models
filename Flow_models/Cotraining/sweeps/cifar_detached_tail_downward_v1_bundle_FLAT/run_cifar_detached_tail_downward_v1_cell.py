...
LOG_ROOT = "cifar_detached_tail_downward_v1_config_logs"
...
def validate_contract(row: dict[str, str]) -> None:
    ...
    wK = float(row["terminal_kl_w"])
    if delta not in (0.10, 0.20, 0.30, 0.40):
    ...
    if group == "primary":
        if T not in (1.35, 1.45, 1.55):
            raise RuntimeError(f"Primary T must be in {{1.35,1.45,1.55}}: {row}")
        if not math.isclose(wK, 0.60, abs_tol=1e-12):
            raise RuntimeError(f"Primary terminal KL must be .60: {row}")
    elif group == "kl_control":
        if not math.isclose(T, 1.45, abs_tol=1e-12):
            raise RuntimeError(f"KL control is only at T=1.45: {row}")
        if not math.isclose(wK, 0.40, abs_tol=1e-12):
            raise RuntimeError(f"KL control terminal KL must be .40: {row}")
    else:
        raise RuntimeError(f"Unknown sweep_group={group!r}: {row}")
...
def main() -> int:
    ...
    print("=" * 110)
    print("CIFAR DETACHED-TAIL DOWNWARD EXTENSION — SHORT TAILS + KL CONTROL")
    print(f"cell / rep        = {args.cell_id:02d} / {row['rep_id']}")
    print(f"seed              = {row['seed']}")
    print(f"group             = {row['sweep_group']}")
    print(f"T / delta_T / TK  = {row['T_full']} / {row['delta_T']} / {row['T_K']}")
    print(f"TK/T              = {row['TK_fraction']}")
    print(f"csem / boundaryKL = {row['csem_w']} / {row['terminal_kl_w']}")
    print(f"training          = {row['epochs']} joint + {row['refine_epochs']} refinement")
    print("hypothesis        = locate the short-tail failure boundary; test whether stronger KL substitutes for tail length")
    print("=" * 110)
