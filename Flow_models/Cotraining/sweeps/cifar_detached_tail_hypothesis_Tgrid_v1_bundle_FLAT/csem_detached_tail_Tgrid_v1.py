...
def evaluate_current_state(
    ...
    # -----------------------------------------------------------------------
    # One CFG strength controls every guided evaluation path: the main
    ...
    if unet is not None and not bool(cfg.get("oracle_nfe_eval_only", False)):
        ...
        # The Pareto protocol holds the deployable full-horizon NFE fixed across
        # both model families.  Since every cell has the same T=1.6, there is no
        # reason to let T_K alter the T-horizon numerical budget.
        ...
        if T_K_eval > float(cfg["t_min"]):
            print(
                "  T_K/T initialization diagnostic: "
                f"T_K={T_K_eval:g} uses {tk_steps} RK4 steps ({4 * tk_steps} NFE); "
                f"T={T_eval:g} uses fixed {t_steps} RK4 steps ({4 * t_steps} NFE)."
            )
        else:
            print(
                "  Exact standard two-stage evaluation: T_K=0 has no reverse path; "
                f"T={T_eval:g} uses fixed {t_steps} RK4 steps ({4 * t_steps} NFE)."
            )
