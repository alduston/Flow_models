#!/usr/bin/env python3
"""Submit 18 detached-tail geometry cells, one Slurm job per independent training."""
...
GRID = "generate_cifar_detached_tail_Tgrid_v1.py"
SLURM = "cifar_detached_tail_Tgrid_v1_cell_job.slurm"
RUNNER = "run_cifar_detached_tail_Tgrid_v1_cell.py"
TARGET = "csem_detached_tail_Tgrid_v1.py"
LOG_DIR = "slurm_logs_cifar_detached_tail_Tgrid_v1"
RECEIPT_DIR = "submission_receipts_cifar_detached_tail_Tgrid_v1"
N_CELLS = 18
ENV_NAME = "CSEM_TAIL_CELL"
...
def main() -> int:
    ...
    print(f"Submitting {len(cells)} / {N_CELLS} detached-tail trainings; one cell per Slurm job.")
    receipts = []
    for cell in cells:
        row = next(r for r in rows if int(r["cell_id"]) == cell)
        out = log_dir / f"tail_cell_{cell:02d}_%j.out"
        env = os.environ.copy()
        env[ENV_NAME] = str(cell)
        cmd = ["sbatch", "-J", f"tail{cell:02d}", "-o", str(out), "--export=ALL", str(base / SLURM)]
        ...
        receipts.append({
            "cell_id": cell, "rep_id": row["rep_id"], "seed": row["seed"],
    ...
    with receipt.open("w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=[
            "cell_id", "rep_id", "seed", "T_full", "delta_T", "T_K",
