...
# METRIC-PARITY V1: independent CSEM and terminal-KL weights are supplied by the runner.\nfrom __future__ import annotations
...
def evaluate_current_state(
    ...
    if eval_oracle:
        if bool(cfg.get("eval_oracle_full_train_reference", True)):
            if oracle_reference_loader is None:
                raise ValueError(
                    "eval_oracle_full_train_reference=True requires oracle_reference_loader "
                    "(the training loader) in evaluate_current_state()."
                )
            print("  Encoding the full training set for exact aggregate-score oracle...")
            (
    ...
    if eval_oracle and bool(cfg.get("eval_oracle_standard_samplers", False)):
        configs.extend([
