#!/usr/bin/env python3
"""Run one ordinary (non-array) CIFAR T_K sweep cell."""
...
LOG_ROOT = "cifar_TK_sweep_T1p75_v1_config_logs"
...
def main() -> int:
    ...
    if cell_id is None:
        raw = os.environ.get("CSEM_TK_SWEEP_CELL_ID")
        if raw is None:
            raise SystemExit("Need --cell-id or CSEM_TK_SWEEP_CELL_ID.")
        cell_id = int(raw)
    if cell_id < 0 or cell_id >= 8:
        raise SystemExit(f"cell-id must be in 0..7, got {cell_id}")
    ...
    print("=" * 96)
    print("CIFAR T_K SWEEP")
    print(f"cell                 = {cell_id}")
    print(f"T_K                  = {row['T_K']}")
    print(f"T                    = {row['T_full']}")
    print("outer/head metric    = canonical / unweighted-eps")
    print("CSEM / K_TK weights  = 0.10 / 0.30")
    print("cotrain/refine       = 500 / 100")
    print("score-head LR        = CIFAR preset default (1e-4)")
    print("evaluation           = every 50, 10000 samples")
    print("CFG                   = 3.0")
    if float(row["T_K"]) == 0.0:
        print("T_K=0 interpretation = exact two-stage-LDM optimization limit:")
        print("                       no CSEM gradient to encoder; K_0 is q0 component KL")
    print("=" * 96)
