...
def run_oracle_sampling_decomposition(
    *,
    ...
    """Oracle-NFE mechanism curve at both T_K and T.

    This diagnostic deliberately uses the *exact class-conditional empirical
    aggregate score without CFG*.  That keeps the oracle field a genuine score
    field while making the full-training oracle substantially cheaper than the
    unconditional 50k-component mixture.  Labels are sampled together with the
    empirical posterior components, so aggregating over labels recovers the
    dataset mixture.

    For each requested horizon h in {T_K, T}, and one RK4 step budget supplied by
    the sweep cell, the same paired banks are used to measure:

      oracle_qh_cond:
          exact q_h initialization + exact conditional score.  This is the clean
          finite-NFE dynamics/integration channel.
      learned_qh_cond:
          same exact q_h initialization + learned conditional score.  Difference
          from oracle_qh_cond isolates learned-score approximation propagated
          through identical dynamics.
      oracle_gaussian_cond:
          Gaussian initialization + exact conditional score.  Difference from
          oracle_qh_cond isolates terminal Gaussian mismatch.
      learned_gaussian_cond:
          Gaussian initialization + learned conditional score.  This combines
          the two practical errors at the same NFE.

    The direct_q0_train row is the representation/decoder reference floor.
    Image FID/KID/diversity and latent SW2 are logged for every row.  The sweep
    varies NFE in separate cells so every curve point reloads the identical frozen
    checkpoint and cannot be contaminated by additional training.
    """
    n_ref = int(reference_mus.shape[0])
    ...
    z0_cpu = mu + std * eps0
    # Reuse the same forward-noise and Gaussian banks at T_K and T so the
    # horizon comparison is paired rather than Monte-Carlo confounded.
    ...
    # Deduplicate in the unusual T_K == T case.
    deduped = []
    ...
    def sample_batches(model: Any, init_cpu: torch.Tensor, horizon: float) -> torch.Tensor:
    ...
    print(
        f"  Oracle NFE mechanism diagnostic: n={n}, method={method}, "
        f"steps={steps} ({4 * steps} RK4 NFE)"
    )
    for horizon_name, horizon in horizon_specs:
        ...
        oracle_qh = sample_batches(oracle_model, qh_cpu, horizon)
        learned_qh = sample_batches(learned_score, qh_cpu, horizon)
        oracle_g = sample_batches(oracle_model, gaussian_cpu, horizon)
        learned_g = sample_batches(learned_score, gaussian_cpu, horizon)
    ...
    summary: Dict[str, float] = {
        ...
        "oracle_sampling_nfe": float(4 * steps if method == "rk4_ode" else steps),
    }
    ...
    # K_{T_K} and K_T from the same frozen representation.
    for horizon_name, horizon in horizon_specs:
        h_cfg = dict(cfg)
        h_cfg["t_max"] = float(horizon)
