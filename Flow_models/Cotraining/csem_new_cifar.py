...
def main():
    ...
    parser.add_argument(
        ...
        help="Dataset for both paired runs. Default: CIFAR.",
    )
    parser.add_argument(
        "--terminal-kl-w",
        type=float,
    ...
    parser.add_argument(
        "--master-results-dir",
        default="run_results_cifar_scale_norm_vs_terminal_kl",
    )
    parser.add_argument(
        "--epochs",
        type=int,
        default=800,
        help="Joint co-training epochs per arm. Golden default: 800.",
    )
    parser.add_argument(
        "--refine-epochs",
        type=int,
        default=100,
        help="Frozen-VAE CSEM refinement epochs per arm. Golden default: 100.",
    )
    parser.add_argument(
        "--lr-vae",
        type=float,
        default=2.5e-4,
        help="VAE/encoder-decoder AdamW LR. Golden default: 2.5e-4.",
    )
    parser.add_argument(
        "--lr-ldm",
        type=float,
        default=1.0e-4,
        help=(
            "Base CSEM/DiT AdamW LR. Golden default: 1e-4. "
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.5e-5,
        help="CSEM score-head LR during frozen-VAE refinement. Golden default: 1.5e-5.",
    )
    parser.add_argument(
        "--eval-samples",
        type=int,
        default=10000,
        help="Maximum test examples used in each evaluation. CIFAR-10 test set = 10000.",
    )
    parser.add_argument(
        "--eval-every",
        type=int,
        default=100,
        help=(
            "Evaluate every N co-training epochs. Golden default: 100. "
            "0 means evaluate only at the final epoch."
        ),
    )
    parser.add_argument(
        "--first-arm",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use the legacy high-detail reporting settings in addition to the "
            "already-golden 800+100 training schedule. Auxiliary control/oracle "
    ...
    if args.lr_vae <= 0 or args.lr_ldm <= 0 or args.lr_refine <= 0:
        raise ValueError("--lr-vae, --lr-ldm, and --lr-refine must all be > 0")
    ...
    cfg_shared = {
        ...
        # --- Learning Rates ---
        "lr_vae": float(args.lr_vae),
        "lr_ldm": float(args.lr_ldm),
        ...
        # --- Evaluation & Logging ---
        # Keep the directly useful golden metrics; remove oracle/control and
        # mechanism-only diagnostics that do not affect the learned model.
    ...
    cfg_cotrain.update({
        # Training schedule: matches the canonical golden co-training run.
        "epochs_vae": int(args.epochs),
        "epochs_refine": int(args.refine_epochs),
        "lr_schedule_epochs": 800,
        "lr_refine": float(args.lr_refine),
        ...
        "eval_freq_refine": (
            int(args.eval_every) if int(args.eval_every) > 0
            else max(1, int(args.refine_epochs))
        ),
    })
    ...
    if args.full_golden:
        # The core training defaults are already the canonical 800+100 golden
        # schedule. This flag only restores legacy visualization/reporting
        # density; auxiliary control/oracle diagnostics stay disabled.
        cfg_cotrain.update({
            "epochs_vae": 800,
            "epochs_refine": 100,
            "lr_schedule_epochs": 800,
    ...
    print(
        f"Protocol: CIFAR GOLDEN TRAINING | "
        ...
        f"tracking/oracle diagnostics=OFF"
    )
    effective_score_lr = (
        cfg_cotrain["lr_ldm"] / cfg_cotrain["score_w_vae"]
        if cfg_cotrain.get("score_w_vae", 0.0) > 0 else 0.0
    )
    print(
        "Golden core preserved: VAE/DiT architecture, CSEM/factored head, LPIPS, "
        "TDD, PatchGAN, optimizer, EMA, diffusion schedule, batch size, and seed."
    )
    print(
        f"LRs: VAE={cfg_cotrain['lr_vae']:.3g} | LDM base={cfg_cotrain['lr_ldm']:.3g} | "
        f"effective joint LSI={effective_score_lr:.3g} | refine={cfg_cotrain['lr_refine']:.3g}"
    )
    ...
    print(f"  stiff_w         = {cfg_cotrain['stiff_w']}  (golden)")
