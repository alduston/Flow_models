...
def compute_mean_estimation_mse_by_t(
    ...
    """Compute E||E[mu_t | z_t, t] - mu_t||^2 with a per-t-bin breakdown."""
    ...
    if stype in ("flow", "flow_matching"):
        ...
        ou_sched = make_schedule(cfg, device)
        t_grid = ou_sched["times"].to(device)
        ...
        for i in range(0, num_samples, batch_size):
            ...
            if stype in ("flow", "flow_matching"):
                for t_val in t_grid:
                    ...
                    mu_post = oracle_model.posterior_mean_mu_t(z_t, t, y_batch)
...
def plot_mean_estimation_mse_by_t(
    ...
    """Viz IIIb: posterior mean-estimation MSE vs t (log-log), with consistent y-limits."""
    global _mean_estimation_ylim_cache

    valid = mse_by_bin > 0
    if not np.any(valid):
        print(f"    [plot_mean_estimation_mse_by_t] No valid bins — skipping plot.")
        return

    t_plot = t_bin_centres[valid]
    mse_plot = mse_by_bin[valid]

    if cache_key not in _mean_estimation_ylim_cache:
        y_lo = float(mse_plot.min()) * 0.5
        y_hi = float(mse_plot.max()) * 2.0
        _mean_estimation_ylim_cache[cache_key] = (y_lo, y_hi)

    y_lo, y_hi = _mean_estimation_ylim_cache[cache_key]
...
def evaluate_current_state(
    ...
    if eval_dir is not None:
        ...
        mean_estimation_mse_uncond, mean_estimation_t_centres, mean_estimation_by_t_bin = compute_mean_estimation_mse_by_t(
            oracle_model,
            encoder_mus, encoder_logvars,
            cfg, device,
            labels=None,
        ...
        plot_mean_estimation_mse_by_t(
            mean_estimation_t_centres, mean_estimation_by_t_bin,
