...
_mean_ess_ylim_cache: Dict[str, Tuple[float, float]] = {}
...
def compute_mean_ess_by_t(
    ...
    """Compute E[ESS(z_t, t)] on a fixed log-spaced t-grid."""
    if oracle_model is None:
        return 0.0, np.zeros(num_t_points, dtype=np.float64), np.zeros(num_t_points, dtype=np.float64)
    ...
    with torch.no_grad():
        for tv in t_values:
            ...
            for j in range(0, num_samples, batch_size):
                ...
                ess_per_sample = oracle_model.posterior_effective_sample_size(z_batch, t_vec_j, y_j)
                ess_sum_t += ess_per_sample.sum().item()
                count_t += bsz_j

            ess_t = ess_sum_t / max(count_t, 1)
            ess_values.append(ess_t)

            total_ess += ess_sum_t
            total_count += count_t

    scalar_ess = total_ess / total_count if total_count > 0 else 0.0
    return scalar_ess, t_values, np.asarray(ess_values, dtype=np.float64)
...
def plot_mean_ess_by_t(
    t_values: np.ndarray,
    ess_values: np.ndarray,
    save_path: str,
    cache_key: str = "default",
):
    """Viz VII: mean effective sample size vs t (log-log), evaluated on a fixed t-grid."""
    global _mean_ess_ylim_cache

    valid = np.isfinite(ess_values) & (ess_values > 0)
    if not np.any(valid):
        print("    [plot_mean_ess_by_t] No valid values — skipping plot.")
        return

    t_plot = t_values[valid]
    ess_plot = ess_values[valid]

    y_limits = _get_or_set_common_log_ylim(_mean_ess_ylim_cache, cache_key, ess_plot)
    if y_limits is None:
        print("    [plot_mean_ess_by_t] No positive values for y-limits — skipping plot.")
        return
    y_lo, y_hi = y_limits

    fig, ax = plt.subplots(1, 1, figsize=(6, 4))
    ax.loglog(t_plot, ess_plot, "o-", linewidth=2, markersize=4, color="tab:green")
    ax.set_xlabel("t")
    ax.set_ylabel("Mean ESS")
    ax.set_title("Mean ESS vs t")
    ax.set_ylim(y_lo, y_hi)
    ax.grid(True, which="both", alpha=0.3)
    fig.tight_layout()
    fig.savefig(save_path, dpi=150)
    plt.close(fig)
    print(f"    Saved mean-ESS-by-t plot → {save_path}")
...
def evaluate_current_state(
    ...
    # -----------------------------------------------------------------------
    # Viz VII: Mean ESS breakdown by t (log-log)
    # -----------------------------------------------------------------------
    if eval_dir is not None:
        print("  [Viz VII] Plotting mean ESS by t ...")
        _, ess_t_values, ess_values = compute_mean_ess_by_t(
            oracle_model,
            encoder_mus, encoder_logvars,
            cfg, device,
            labels=None,
        ...
        plot_mean_ess_by_t(
            ess_t_values, ess_values,
            save_path=os.path.join(eval_dir, f"{prefix}_mean_ess_by_t_ep{epoch_idx}.png"),
            cache_key=prefix,
        )
