...
class VAE(nn.Module):
    """
    VAE for CIFAR-10 (32x32x1 grayscale).
...
def plot_vae_recon_loss(loss_df, save_path):
    ...
    # For two-stage, plot vae_pretrain stage
    vae_df = loss_df[loss_df["stage"] == "vae_pretrain"]
    if len(vae_df) > 0 and "recon" in vae_df.columns:
        ax.plot(vae_df["epoch"], vae_df["recon"], 
                color="blue", linewidth=2, marker='o', markersize=3, label="Reconstruction Loss")
    
    ax.set_xlabel("Epoch", fontsize=12)
    ax.set_ylabel("Loss", fontsize=12)
    ax.set_title("VAE Reconstruction Loss (Pre-training Stage)", fontsize=14)
...
def plot_score_losses(loss_df, save_path):
    ...
    # For two-stage, plot score_training stage
    score_df = loss_df[loss_df["stage"] == "score_training"]
    
    if len(score_df) > 0:
        if "score_lsi" in score_df.columns:
            ax.semilogy(score_df["epoch"], score_df["score_lsi"], 
                        color="blue", linewidth=2, marker='o', markersize=3, label="LSI Score Loss")
        if "score_control" in score_df.columns:
            ax.semilogy(score_df["epoch"], score_df["score_control"], 
...
def make_dataloaders(batch_size, num_workers):
    """CIFAR-10 dataloaders. Images are 32x32x1 (grayscale), normalized to [-1,1]."""
...
def train_vae_ldms_indep(cfg):
    ...
    # VAE for CIFAR-10: 1 input channel (grayscale)
    ...
    # ===================================================================
    print("=" * 60)
    print("TWO-STAGE INDEPENDENT TRAINING - CIFAR-10 (Grayscale)")
    ...
    for ep in range(epochs_vae):
        ...
        for x, _ in tqdm(train_l, desc=f"VAE Ep {ep+1}", leave=False):
            ...
            # Grayscale CIFAR needs repeat for LPIPS (expects 3 channels)
            if LPIPS_AVAILABLE:
                x_rec_3ch = x_rec.repeat(1, 3, 1, 1)
                x_3ch = x.repeat(1, 3, 1, 1)
                perc = lpips_fn(x_rec_3ch, x_3ch).mean()
            else:
...
def main():
    ...
    cfg = {
        ...
        # --- Checkpoints ---
        "ckpt_dir": "checkpoints_twostage_cifar",
    ...
    print("=== Two-Stage Independent Training: VAE then Score Networks - CIFAR-10 (Grayscale) ===")
