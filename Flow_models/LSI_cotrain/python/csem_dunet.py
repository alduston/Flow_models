...
class ReconFrontierTracker:
    ...
    def get_log_recon_derivative_curve(self):
        """Return the smoothed |d log R / d log t| curve used by frontier weighting."""
        log_R = torch.log(self.R_ema.clamp(min=1e-12))
        log_R = self._gaussian_smooth_1d(log_R, self.smooth_sigma)
        log_t = torch.log(self.bin_centers)
        d_logR = log_R[1:] - log_R[:-1]
        d_logt = log_t[1:] - log_t[:-1]
        grad = (d_logR / (d_logt + 1e-12)).abs()
        curve = torch.zeros(self.n_bins, device=self.device)
        curve[0] = grad[0]
        curve[-1] = grad[-1]
        curve[1:-1] = 0.5 * (grad[:-1] + grad[1:])
        return self.bin_centers.cpu(), curve.cpu()
...
def plot_frontier_diagnostics(frontier_tracker, save_path, epoch):
    """Plot reconstruction curve R(t) and the smoothed |d log R / d log t| curve."""
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))

    if frontier_tracker is None or not getattr(frontier_tracker, "is_active", False):
        for ax in (ax1, ax2):
            ax.axis("off")
        ax1.text(0.5, 0.5, "Frontier unavailable", ha="center", va="center", transform=ax1.transAxes)
        ax2.text(0.5, 0.5, "time_cond_decode=False\nor frontier inactive", ha="center", va="center", transform=ax2.transAxes)
        fig.tight_layout(); fig.savefig(save_path, dpi=150, bbox_inches='tight'); plt.close(fig)
        return

    t_c, R_vals = frontier_tracker.get_R_curve()
    t_d, deriv_vals = frontier_tracker.get_log_recon_derivative_curve()
    ...
    ax2.loglog(t_d.numpy(), deriv_vals.numpy(), 'r.-', linewidth=1.5, markersize=2)
    ax2.set_xlabel('t'); ax2.set_ylabel(r'$|d \log R / d \log t|$')
    ax2.set_title(f'Smoothed log-Recon Derivative (ep {epoch})')
    ax2.grid(True, which='both', alpha=0.3)
    fig.tight_layout(); fig.savefig(save_path, dpi=150, bbox_inches='tight'); plt.close(fig)


def plot_frontier_log_recon_derivative(
    frontier_tracker,
    save_path,
    epoch,
    cache_key: str = "default",
):
    """Standalone frontier plot used by evaluate_current_state and eval videos."""
    global _frontier_log_recon_derivative_ylim_cache
    ...
    if frontier_tracker is None or not getattr(frontier_tracker, "is_active", False):
        ...
        ax.text(
            0.5,
            0.5,
            "Frontier derivative unavailable\n(time_cond_decode=False or frontier inactive)",
            ha="center",
            va="center",
        ...
        plt.close(fig)
        print(f"    Saved blank frontier log-recon-derivative plot → {save_path}")
        return

    t_vals, deriv_vals = frontier_tracker.get_log_recon_derivative_curve()
    t_np = t_vals.numpy()
    deriv_np = deriv_vals.numpy()

    valid = np.isfinite(deriv_np) & (deriv_np > 0)
    if not np.any(valid):
        ax.axis("off")
        ax.text(0.5, 0.5, "No valid frontier derivative values", ha="center", va="center", transform=ax.transAxes)
        fig.tight_layout()
        fig.savefig(save_path, dpi=150)
        plt.close(fig)
        print(f"    Saved blank frontier log-recon-derivative plot → {save_path}")
    ...
    t_plot = t_np[valid]
    deriv_plot = deriv_np[valid]
    y_limits = _get_or_set_common_log_ylim(
        _frontier_log_recon_derivative_ylim_cache,
        cache_key,
        deriv_plot,
    )
    if y_limits is None:
        ax.axis("off")
        ax.text(0.5, 0.5, "Could not determine y-limits", ha="center", va="center", transform=ax.transAxes)
        fig.tight_layout()
        fig.savefig(save_path, dpi=150)
        plt.close(fig)
        print(f"    Saved blank frontier log-recon-derivative plot → {save_path}")
    ...
    ax.loglog(t_plot, deriv_plot, "o-", linewidth=2, markersize=4, color="tab:red")
    ax.set_xlabel("t")
    ax.set_ylabel(r"$|d \log R / d \log t|$")
    ax.set_title(f"Smoothed log-Recon Derivative vs t (ep {epoch})")
    ax.set_ylim(y_lo, y_hi)
    ax.grid(True, which="both", alpha=0.3)
    fig.tight_layout()
    fig.savefig(save_path, dpi=150)
    plt.close(fig)
    print(f"    Saved frontier log-recon-derivative plot → {save_path}")
...
def compute_projected_diversity_loss(
    ...
    *,
    ...
    """Projected mean-diversity penalty used by anisotropic Minimax CSEM.

    This is the squared projection of the diversity residual onto the stop-grad,
    per-sample normalized denoising direction. It is intended to be *added* as a
    separate positively weighted penalty on top of the existing isotropic diversity
    reward, rather than replacing that isotropic term.
    """
    if eps_hat is None:
        raise ValueError("Projected diversity loss requires an eps_hat tensor for the denoising direction.")
...
_mean_estimation_ylim_cache: Dict[str, Tuple[float, float]] = {}
_vcsem_ylim_cache: Dict[str, Tuple[float, float]] = {}
_mean_ess_derivative_ylim_cache: Dict[str, Tuple[float, float]] = {}
_frontier_log_recon_derivative_ylim_cache: Dict[str, Tuple[float, float]] = {}


def _get_or_set_common_log_ylim(
    ...
    """Cache log-scale y-limits from the first eval and reuse them thereafter."""
    ...
    if cache_key not in cache:
        vals = values[valid]
        y_lo = max(float(vals.min()) * lower_pad, 1e-12)
        y_hi = max(float(vals.max()) * upper_pad, y_lo * 1.01)
        cache[cache_key] = (y_lo, y_hi)

    return cache[cache_key]


def _get_or_set_common_linear_ylim(
    cache: Dict[str, Tuple[float, float]],
    cache_key: str,
    values: np.ndarray,
    pad_frac: float = 0.20,
    min_half_span: float = 1e-8,
) -> Tuple[float, float] | None:
    """Cache linear-scale y-limits from the first eval and reuse them thereafter."""
    ...
    if cache_key not in cache:
        ...
        half_span = max(0.5 * (y_hi - y_lo), min_half_span)
        half_span *= (1.0 + pad_frac)
        cache[cache_key] = (center - half_span, center + half_span)
...
def compute_mean_estimation_mse_by_t(
    oracle_model,
    encoder_mus: torch.Tensor,
    encoder_logvars: torch.Tensor,
    cfg: Dict[str, Any],
    ...
    """Compute E||E[mu_t | z_t, t] - mu_t||^2 on a fixed log-spaced t-grid.

    This mirrors plot_recon_error_vs_t:
      - choose a fixed log-spaced grid in t over [t_min, t_max]
      - sample z_t at each grid point
      - use the oracle posterior mean model to estimate E[mu_t | z_t, t]
      - return one MSE value per chosen t
    """
    if oracle_model is None:
        return 0.0, np.zeros(num_t_points, dtype=np.float64), np.zeros(num_t_points, dtype=np.float64)
    ...
    t_values = np.logspace(np.log10(t_min), np.log10(t_max), num_t_points).astype(np.float64)

    mu_sub = encoder_mus[sample_indices].to(device)
    logvar_sub = encoder_logvars[sample_indices].to(device)
    std_sub = torch.exp(0.5 * logvar_sub)
    labels_sub = labels[sample_indices].to(device=device, dtype=torch.long) if labels is not None else None

    # Sample one z0 per selected datapoint, exactly as in the other eval plots.
    ...
    with torch.no_grad():
        for tv in t_values:
            t_scalar = torch.tensor(float(tv), device=device, dtype=torch.float32)
            t_4d = t_scalar.view(1, 1, 1, 1)

            alpha, sigma = get_alpha_sigma_for_schedule(
                t_4d,
                schedule_type=stype,
                cosine_s=cosine_s,
            )
            ...
            for j in range(0, num_samples, batch_size):
                ...
                mu_batch = mu_sub[j:j + batch_size]
                bsz_j = z_batch.shape[0]

                t_vec_j = torch.full((bsz_j,), float(tv), device=device, dtype=torch.float32)
                y_j = labels_sub[j:j + batch_size] if labels_sub is not None else None

                mu_t = alpha * mu_batch
                mu_post = oracle_model.posterior_mean_mu_t(z_batch, t_vec_j, y_j)

                mse_per_sample = ((mu_post - mu_t) ** 2).sum(dim=(1, 2, 3))
                mse_sum_t += mse_per_sample.sum().item()
                count_t += bsz_j

            mse_t = mse_sum_t / max(count_t, 1)
            mse_values.append(mse_t)
    ...
    valid = np.isfinite(mse_arr) & (mse_arr > 0)

    '''
    print("    [mean-estimation-by-t debug]")
    print(f"      cfg time_schedule={stype} t_min={t_min:.6g} t_max={t_max:.6g} num_t_points={num_t_points}")
    print(f"      t_values={np.array2string(t_values, precision=6, separator=', ')}")
    print(f"      mse_values={np.array2string(mse_arr, precision=6, separator=', ')}")
    print(f"      valid_count={int(valid.sum())}/{len(valid)}")
    print(f"      zero_count={int((mse_arr == 0).sum())}")
    print(f"      neg_count={int((mse_arr < 0).sum())}")
    print(f"      nan_count={int(np.isnan(mse_arr).sum())}")
    print(f"      inf_count={int(np.isinf(mse_arr).sum())}")
    print(f"      valid_indices={np.where(valid)[0].tolist()}")
    if np.any(valid):
        print(f"      first_valid_t={t_values[valid][0]:.6g} last_valid_t={t_values[valid][-1]:.6g}")
        print(f"      min_positive_mse={mse_arr[valid].min():.6e} max_positive_mse={mse_arr[valid].max():.6e}")
    '''
    return scalar_mse, t_values, np.asarray(mse_values, dtype=np.float64)


def compute_vcsem_by_t(
    oracle_model,
    encoder_mus: torch.Tensor,
    encoder_logvars: torch.Tensor,
    cfg: Dict[str, Any],
    ...
    """Compute E[VCSEM_eps(z_t, t)] on a fixed log-spaced t-grid."""
    if oracle_model is None:
        return 0.0, np.zeros(num_t_points, dtype=np.float64), np.zeros(num_t_points, dtype=np.float64)
    ...
    with torch.no_grad():
        for tv in t_values:
            ...
            alpha, sigma = get_alpha_sigma_for_schedule(
                t_4d,
                schedule_type=str(cfg.get("time_schedule", "log_snr")).lower(),
                cosine_s=float(cfg.get("cosine_s", 0.008)),
            )
            alpha = alpha.squeeze()
            sigma = sigma.squeeze()
            ...
            for j in range(0, num_samples, batch_size):
                ...
                vcsem_per_sample = oracle_model.posterior_vcsem_eps(z_batch, t_vec_j, y_j)
                vcsem_sum_t += vcsem_per_sample.sum().item()
                count_t += bsz_j

            vcsem_t = vcsem_sum_t / max(count_t, 1)
            vcsem_values.append(vcsem_t)

            total_vcsem += vcsem_sum_t
            total_count += count_t

    scalar_vcsem = total_vcsem / total_count if total_count > 0 else 0.0
    return scalar_vcsem, t_values, np.asarray(vcsem_values, dtype=np.float64)


def compute_mean_ess_derivative_by_t(
    oracle_model,
    encoder_mus: torch.Tensor,
    encoder_logvars: torch.Tensor,
    cfg: Dict[str, Any],
    ...
    """Compute E[d/dt ESS(z_t, t)] on a fixed log-spaced t-grid."""
    if oracle_model is None:
        return 0.0, np.zeros(num_t_points, dtype=np.float64), np.zeros(num_t_points, dtype=np.float64)
    ...
    ess_derivative_values = []
    total_ess_derivative = 0.0
    total_count = 0
    ...
    with torch.no_grad():
        for tv in t_values:
            ...
            for j in range(0, num_samples, batch_size):
                ...
                ess_derivative_per_sample = oracle_model.posterior_effective_sample_size_derivative(z_batch, t_vec_j, y_j)
                ess_derivative_sum_t += ess_derivative_per_sample.sum().item()
                count_t += bsz_j

            ess_derivative_t = ess_derivative_sum_t / max(count_t, 1)
            ess_derivative_values.append(ess_derivative_t)

            total_ess_derivative += ess_derivative_sum_t
            total_count += count_t

    scalar_ess_derivative = total_ess_derivative / total_count if total_count > 0 else 0.0
    return scalar_ess_derivative, t_values, np.asarray(ess_derivative_values, dtype=np.float64)
...
def plot_mean_estimation_mse_by_t(
    t_values: np.ndarray,
    mse_values: np.ndarray,
    save_path: str,
    cache_key: str = "default",
):
    """Viz IIIb: posterior mean-estimation MSE vs t (log-log), evaluated on a fixed t-grid."""
    global _mean_estimation_ylim_cache

    valid = np.isfinite(mse_values) & (mse_values > 0)
    if not np.any(valid):
        print("    [plot_mean_estimation_mse_by_t] No valid values — skipping plot.")
        return

    t_plot = t_values[valid]
    mse_plot = mse_values[valid]

    y_limits = _get_or_set_common_log_ylim(_mean_estimation_ylim_cache, cache_key, mse_plot)
    if y_limits is None:
        print("    [plot_mean_estimation_mse_by_t] No positive values for y-limits — skipping plot.")
        return
    y_lo, y_hi = y_limits

    fig, ax = plt.subplots(1, 1, figsize=(6, 4))
    ax.loglog(t_plot, mse_plot, "o-", linewidth=2, markersize=4, color="tab:purple")
    ax.set_xlabel("t")
    ax.set_ylabel(r"MSE of $\mathbb{E}[\mu_t \mid z_t,t]$")
    ax.set_title(r"Mean-Estimation MSE vs t")
    ax.set_ylim(y_lo, y_hi)
    ax.grid(True, which="both", alpha=0.3)
    fig.tight_layout()
    fig.savefig(save_path, dpi=150)
    plt.close(fig)
    print(f"    Saved mean-estimation-by-t plot → {save_path}")

def plot_vcsem_by_t(
    t_values: np.ndarray,
    vcsem_values: np.ndarray,
    save_path: str,
    cache_key: str = "default",
):
    """Viz VI: VCSEM in eps-space vs t (log-log), evaluated on a fixed t-grid."""
    global _vcsem_ylim_cache

    valid = np.isfinite(vcsem_values) & (vcsem_values > 0)
    if not np.any(valid):
        print("    [plot_vcsem_by_t] No valid values — skipping plot.")
        return

    t_plot = t_values[valid]
    vcsem_plot = vcsem_values[valid]

    y_limits = _get_or_set_common_log_ylim(_vcsem_ylim_cache, cache_key, vcsem_plot)
    if y_limits is None:
        print("    [plot_vcsem_by_t] No positive values for y-limits — skipping plot.")
        return
    y_lo, y_hi = y_limits

    fig, ax = plt.subplots(1, 1, figsize=(6, 4))
    ax.loglog(t_plot, vcsem_plot, "o-", linewidth=2, markersize=4, color="tab:orange")
    ax.set_xlabel("t")
    ax.set_ylabel("VCSEM (eps-space)")
    ax.set_title("VCSEM vs t")
    ax.set_ylim(y_lo, y_hi)
    ax.grid(True, which="both", alpha=0.3)
    fig.tight_layout()
    fig.savefig(save_path, dpi=150)
    plt.close(fig)
    print(f"    Saved VCSEM-by-t plot → {save_path}")



def plot_mean_ess_derivative_by_t(
    t_values: np.ndarray,
    ess_derivative_values: np.ndarray,
    save_path: str,
    cache_key: str = "default",
):
    """Viz VII: mean d/dt ESS vs t, evaluated on a fixed t-grid."""
    global _mean_ess_derivative_ylim_cache

    valid = np.isfinite(ess_derivative_values) & (ess_derivative_values > 0)
    if not np.any(valid):
        print("    [plot_mean_ess_derivative_by_t] No positive finite values — skipping plot.")
        return

    t_plot = t_values[valid]
    ess_derivative_plot = ess_derivative_values[valid]

    y_limits = _get_or_set_common_log_ylim(
        _mean_ess_derivative_ylim_cache,
        cache_key,
        ess_derivative_plot,
    )
    if y_limits is None:
        print("    [plot_mean_ess_derivative_by_t] Could not determine log y-limits — skipping plot.")
        return
    y_lo, y_hi = y_limits

    fig, ax = plt.subplots(1, 1, figsize=(6, 4))
    ax.loglog(t_plot, ess_derivative_plot, "o-", linewidth=2, markersize=4, color="tab:green")
    ax.set_xlabel("t")
    ax.set_ylabel(r"$d\,\mathrm{ESS}/dt$")
    ax.set_title(r"Mean $d\,\mathrm{ESS}/dt$ vs t")
    ax.set_ylim(y_lo, y_hi)
    ax.grid(True, which="both", alpha=0.3)
    fig.tight_layout()
    fig.savefig(save_path, dpi=150)
    plt.close(fig)
    print(f"    Saved mean-ESS-derivative-by-t plot → {save_path}")


def make_eval_plot_videos(
    evals_dir: str,
    prefix: str = "LSI_Diff",
    total_duration_sec: float = 5.0,
) -> None:
    """Create per-metric mp4s by stitching together eval plots across epochs."""
    if not evals_dir or not os.path.isdir(evals_dir):
        print("--> Skipping eval plot videos: evals directory not found.")
        return

    eval_video_dir = os.path.join(evals_dir, "eval_videos")
    os.makedirs(eval_video_dir, exist_ok=True)

    eval_dirs = []
    for name in os.listdir(evals_dir):
        full = os.path.join(evals_dir, name)
        if not os.path.isdir(full) or not name.startswith("eval_"):
            continue
        try:
            epoch = int(name.split("eval_")[-1])
        except ValueError:
            continue
        eval_dirs.append((epoch, full))
    eval_dirs.sort(key=lambda x: x[0])

    if not eval_dirs:
        print("--> Skipping eval plot videos: no eval_* directories found.")
        return

    metric_specs = [
        ("mean_estimation_mse_by_t", "mean_estimation_mse_by_t"),
        ("ess_derivative_by_t", "ess_derivative_by_t"),
        ("vcsem_by_t", "vcsem_by_t"),
        ("frontier_log_recon_derivative", "frontier_log_recon_derivative"),
    ]

    from PIL import Image as _PILImage
    import imageio.v2 as imageio

    for metric_tag, metric_stub in metric_specs:
        frame_paths = []
        for epoch, eval_dir in eval_dirs:
            candidate = os.path.join(eval_dir, f"{prefix}_{metric_stub}_ep{epoch}.png")
            if os.path.exists(candidate):
                frame_paths.append(candidate)

        if not frame_paths:
            print(f"--> Skipping {metric_tag} video: no matching frames found for prefix={prefix!r}.")
            continue

        video_path = os.path.join(eval_video_dir, f"{prefix}_{metric_tag}.mp4")
        frames_np = [np.asarray(_PILImage.open(fp).convert("RGB")) for fp in frame_paths]
        fps = max(len(frames_np) / max(total_duration_sec, 1e-8), 1e-8)

        try:
            imageio.mimsave(video_path, frames_np, fps=fps)
            print(f"--> Saved eval plot video -> {video_path}")
        except Exception as e_mp4:
            try:
                writer_fps = 30
                total_frames = max(int(round(total_duration_sec * writer_fps)), len(frames_np))
                boundaries = np.linspace(0, total_frames, len(frames_np) + 1).round().astype(int)
                repeated_frames = []
                for idx, frame in enumerate(frames_np):
                    repeat = max(1, int(boundaries[idx + 1] - boundaries[idx]))
                    repeated_frames.extend([frame] * repeat)
                imageio.mimsave(video_path, repeated_frames, fps=writer_fps)
                print(f"--> Saved eval plot video with repeated frames -> {video_path}")
            except Exception as e_fallback:
                print(f"--> Warning: failed to save eval plot video {video_path}. mp4 error: {e_mp4}; fallback error: {e_fallback}")
...
def plot_reverse_trajectory_grid(
    ...
    init_mode: str = "prior",
    oracle_z0_bank: torch.Tensor | None = None,
    oracle_noise_bank: torch.Tensor | None = None,
    oracle_label_bank: torch.Tensor | None = None,
    t_max: float = None,
    t_min: float = None,
):
    ...
    if init_mode not in ("prior", "oracle"):
        raise ValueError(f"init_mode must be 'prior' or 'oracle', got {init_mode!r}")
    ...
    if init_mode == "prior":
        ...
        if oracle_z0_bank is None:
            raise ValueError(
                "plot_reverse_trajectory_grid(init_mode='oracle') requires oracle_z0_bank."
            )
        if oracle_z0_bank.shape[0] < num_rows:
            raise ValueError(
                f"oracle_z0_bank has only {oracle_z0_bank.shape[0]} samples, need at least {num_rows}."
            )

        z0_init = oracle_z0_bank[:num_rows].to(device)
        expected_shape = latent_shape[1:]
        if tuple(z0_init.shape[1:]) != expected_shape:
            raise ValueError(
                f"oracle_z0_bank latent shape mismatch: got {tuple(z0_init.shape[1:])}, "
                f"expected {expected_shape}."
            )

        noise_init = None
        if oracle_noise_bank is not None:
            if oracle_noise_bank.shape[0] < num_rows:
                raise ValueError(
                    f"oracle_noise_bank has only {oracle_noise_bank.shape[0]} samples, need at least {num_rows}."
                )
            noise_init = oracle_noise_bank[:num_rows].to(device)
            if tuple(noise_init.shape[1:]) != expected_shape:
                raise ValueError(
                    f"oracle_noise_bank latent shape mismatch: got {tuple(noise_init.shape[1:])}, "
                    f"expected {expected_shape}."
                )

        z = sample_forward_latent_from_z0(
            z0_init,
            t_value=t_max,
            schedule_type=stype,
            cosine_s=float(cfg.get("cosine_s", 0.008)),
    ...
    if init_mode == "oracle":
        if oracle_label_bank is None:
            raise ValueError(
                "plot_reverse_trajectory_grid(init_mode='oracle') requires oracle_label_bank."
            )
        if oracle_label_bank.shape[0] < num_rows:
            raise ValueError(
                f"oracle_label_bank has only {oracle_label_bank.shape[0]} samples, need at least {num_rows}."
            )
        row_labels = [int(v) for v in oracle_label_bank[:num_rows].view(-1).tolist()]
...
class DecoderBandFuse(nn.Module):
    """Inject a scale-specific band as a zero-init residual into the decoder trunk.

    The side path is initialized to zero so enabling the multiscale decoder
    preserves the legacy decoder behavior at initialization. When a time
    embedding is provided, it modulates the side path with a FiLM-style affine
    transformation before the residual is added.
    """
    def __init__(self, band_ch: int, out_ch: int, t_dim: int | None = None):
        super().__init__()
        self.norm = make_group_norm(band_ch)
        self.proj = nn.Conv2d(band_ch, out_ch, 3, 1, 1)
        nn.init.zeros_(self.proj.weight)
        nn.init.zeros_(self.proj.bias)
        self.time_proj = None
    ...
    def forward(self, trunk: torch.Tensor, band: torch.Tensor, t_emb: torch.Tensor | None = None) -> torch.Tensor:
        side = self.proj(F.silu(self.norm(band)))
        if self.time_proj is not None and t_emb is not None:
            scale, shift = self.time_proj(t_emb).chunk(2, dim=1)
            side = side * (1.0 + scale[:, :, None, None]) + shift[:, :, None, None]
        return trunk + side
...
class VAE(nn.Module):
    ...
    def __init__(
        ...
        # --- Multiscale decoder side-bands ---
        multiscale_decoder: bool = False,    # Laplacian-style bottleneck decomposition
        decoder_num_bands: int = 3,          # current implementation expects 3 (coarse/mid/fine)
        decoder_band_premix: bool = True,    # learned 1x1 premix before Laplacian split
        ...
        self.multiscale_decoder = bool(multiscale_decoder)
        self.decoder_num_bands = int(decoder_num_bands)
        self.decoder_band_premix = bool(decoder_band_premix)
        if self.multiscale_decoder and self.decoder_num_bands != 3:
            raise ValueError("multiscale_decoder currently expects decoder_num_bands=3 (coarse/mid/fine).")
        ...
        # ---- multiscale bottleneck side-bands ---------------------------
        if self.multiscale_decoder:
            ms_t_dim = dec_time_emb_dim if time_cond_decoder else None
            if self.decoder_band_premix:
                self.dec_ms_premix = nn.Conv2d(ch4, ch4, 1)
                with torch.no_grad():
                    self.dec_ms_premix.weight.zero_()
                    eye = torch.eye(ch4, dtype=self.dec_ms_premix.weight.dtype)
                    self.dec_ms_premix.weight[:, :, 0, 0].copy_(eye)
                    self.dec_ms_premix.bias.zero_()
            else:
                self.dec_ms_premix = None

            self.dec_stage0_band_fuse = DecoderBandFuse(ch4, ch4, t_dim=ms_t_dim)
            self.dec_stage1_band_up = nn.Sequential(
                nn.Upsample(scale_factor=2, mode="nearest"),
                nn.Conv2d(ch4, ch2, 3, 1, 1),
            )
            self.dec_stage1_band_fuse = DecoderBandFuse(ch2, ch2, t_dim=ms_t_dim)
            self.dec_stage2_band_up = nn.Sequential(
                nn.Upsample(scale_factor=2, mode="nearest"),
                nn.Conv2d(ch4, ch2, 3, 1, 1),
                nn.SiLU(),
                nn.Upsample(scale_factor=2, mode="nearest"),
            ...
            self.dec_stage2_band_fuse = DecoderBandFuse(ch1, ch1, t_dim=ms_t_dim)
        else:
            self.dec_ms_premix = None
            self.dec_stage0_band_fuse = None
            self.dec_stage1_band_up = None
            self.dec_stage1_band_fuse = None
            self.dec_stage2_band_up = None
            self.dec_stage2_band_fuse = None
    ...
    def _decoder_lowpass(self, x: torch.Tensor) -> torch.Tensor:
        return F.avg_pool2d(x, kernel_size=3, stride=1, padding=1, count_include_pad=False)

    def _compute_decoder_bands(self, h: torch.Tensor) -> dict[str, torch.Tensor] | None:
        if not self.multiscale_decoder:
            return None
        h_ms = self.dec_ms_premix(h) if self.dec_ms_premix is not None else h
        low1 = self._decoder_lowpass(h_ms)
        low2 = self._decoder_lowpass(low1)
        return {
            "coarse": low2,
            "mid": low1 - low2,
            "fine": h_ms - low1,
    ...
    def decode(self, z, t=None, y: torch.Tensor | None = None):
        ...
        if self.time_cond_decoder and t is not None:
            ...
            bands = self._compute_decoder_bands(h)
            if bands is not None:
                h = self.dec_stage0_band_fuse(h, bands["coarse"], t_emb)
            ...
            if bands is not None:
                h = self.dec_stage1_band_fuse(h, self.dec_stage1_band_up(bands["mid"]), t_emb)
            ...
            if bands is not None:
                h = self.dec_stage2_band_fuse(h, self.dec_stage2_band_up(bands["fine"]), t_emb)
            ...
            if bands is None:
                ...
                h = self.dec_stage0_band_fuse(h, bands["coarse"], None)
                h = self.dec_blocks[0](h)

                stage1 = self.dec_blocks[1]
                h = stage1[0](h)
                h = stage1[1](h)
                h = self.dec_stage1_band_fuse(h, self.dec_stage1_band_up(bands["mid"]), None)
                for module in stage1[2:]:
                    h = module(h)

                stage2 = self.dec_blocks[2]
                h = stage2[0](h)
                h = stage2[1](h)
                h = self.dec_stage2_band_fuse(h, self.dec_stage2_band_up(bands["fine"]), None)
                for module in stage2[2:]:
                    h = module(h)
        else:
...
class UniversalSampler:
    ...
    def sample(
        ...
        if self.init_mode == "oracle" and y_init is not None:
            y = y_init.to(device=device, dtype=torch.long).view(-1)
        elif y is not None:
...
class OracleScoreModel:
    ...
    def _get_alpha_sigma_derivatives(self, t_scalar: torch.Tensor):
        """Return (alpha_dot, sigma_dot) for the active schedule family."""
        stype = str(self.cfg.get("time_schedule", "log_snr")).lower()
        t = t_scalar.view(1, 1)
        ...
        if stype in ("flow", "flow_matching"):
            ...
            return alpha_dot.squeeze(), sigma_dot.squeeze()

        if stype == "cosine":
            cosine_s = float(self.cfg.get("cosine_s", 0.008))
            g = ((t + cosine_s) / (1.0 + cosine_s)) * (math.pi / 2.0)
            g0 = (cosine_s / (1.0 + cosine_s)) * (math.pi / 2.0)
            g_prime = math.pi / (2.0 * (1.0 + cosine_s))
            alpha = torch.cos(g) / math.cos(g0)
            alpha_dot = -torch.sin(g) * g_prime / math.cos(g0)
            sigma = torch.sqrt((1.0 - alpha * alpha).clamp_min(1e-12))
            sigma_dot = -(alpha * alpha_dot) / sigma.clamp_min(1e-12)
            return alpha_dot.squeeze(), sigma_dot.squeeze()

        alpha = torch.exp(-t)
        sigma = torch.sqrt(1.0 - torch.exp(-2.0 * t) + 1e-8)
        alpha_dot = -alpha
        sigma_dot = (alpha * alpha) / sigma.clamp_min(1e-12)
        return alpha_dot.squeeze(), sigma_dot.squeeze()

    # ------------------------------------------------------------------

    def _compute_posterior_terms(
        self,
        z_t: torch.Tensor,
        t: torch.Tensor,
        label_filter: torch.Tensor | None = None,
    ) -> Dict[str, torch.Tensor]:
        """Return posterior weights and Gaussian-component terms at time ``t``."""
        B = z_t.shape[0]
        device = z_t.device
        ...
        z_flat = z_t.reshape(B, self.D)

        mu_t = alpha * self.all_mu_flat
        var_t = alpha_sq * self.all_var_flat + sigma_sq
        one_over_var = 1.0 / var_t
        mu_over_var = mu_t * one_over_var

        log_var_sum = torch.log(var_t).sum(dim=1)
        mu_sq_over_var_sum = (mu_t * mu_over_var).sum(dim=1)

        all_log_w = -0.5 * (
            log_var_sum.unsqueeze(0)
            + (z_flat * z_flat) @ one_over_var.T
            - 2.0 * z_flat @ mu_over_var.T
            + mu_sq_over_var_sum.unsqueeze(0)
        )

        if label_filter is not None:
            mask = (self.all_labels.unsqueeze(0) == label_filter.unsqueeze(1))
            all_log_w = all_log_w.masked_fill(~mask, float("-inf"))

        w = torch.softmax(all_log_w, dim=1)

        return {
            "w": w,
            "mu_t": mu_t,
            "one_over_var": one_over_var,
            "mu_over_var": mu_over_var,
            "z_flat": z_flat,
            "sigma": sigma,
        }
    ...
    def _compute_eps(
        ...
        """Core oracle score computation (all reference data GPU-resident, no chunking)."""
        terms = self._compute_posterior_terms(z_t, t, label_filter=label_filter)
        w = terms["w"]
        one_over_var = terms["one_over_var"]
        mu_over_var = terms["mu_over_var"]
        z_flat = terms["z_flat"]
        sigma = terms["sigma"]
        ...
        return eps_flat.reshape(z_t.shape[0], *self.spatial_shape)

    def posterior_mean_mu_t(
        self,
        z_t: torch.Tensor,
        t: torch.Tensor,
        y: torch.Tensor | None = None,
    ) -> torch.Tensor:
        """Compute the exact Bayes posterior mean E[mu_t | z_t, t]."""
        terms = self._compute_posterior_terms(z_t, t, label_filter=y)
        mu_post_flat = terms["w"] @ terms["mu_t"]
        return mu_post_flat.reshape(z_t.shape[0], *self.spatial_shape)

    def posterior_vcsem_eps(
        self,
        z_t: torch.Tensor,
        t: torch.Tensor,
        y: torch.Tensor | None = None,
    ) -> torch.Tensor:
        """Compute the exact irreducible CSEM variance in eps-parameterization."""
        terms = self._compute_posterior_terms(z_t, t, label_filter=y)
        w = terms["w"]
        one_over_var = terms["one_over_var"]
        mu_over_var = terms["mu_over_var"]
        ...
        A = w @ one_over_var
        Bv = w @ mu_over_var
        eps_mean_flat = sigma * (z_flat * A - Bv)

        one_over_var_sq = one_over_var * one_over_var
        one_mu_over_var = one_over_var * mu_over_var
        mu_over_var_sq_sum = (mu_over_var * mu_over_var).sum(dim=1).unsqueeze(0)
        comp_eps_sq = (sigma * sigma) * (
            (z_flat * z_flat) @ one_over_var_sq.T
            - 2.0 * (z_flat @ one_mu_over_var.T)
            + mu_over_var_sq_sum
        )

        second_moment = (w * comp_eps_sq).sum(dim=1)
        mean_sq = (eps_mean_flat * eps_mean_flat).sum(dim=1)
        return (second_moment - mean_sq).clamp_min(0.0)

    def posterior_effective_sample_size(
        self,
        z_t: torch.Tensor,
        t: torch.Tensor,
        y: torch.Tensor | None = None,
    ) -> torch.Tensor:
        """Compute the posterior effective sample size 1 / sum_i w_i^2."""
        terms = self._compute_posterior_terms(z_t, t, label_filter=y)
        w = terms["w"]
        return 1.0 / (w.pow(2).sum(dim=1).clamp_min(1e-12))

    def posterior_effective_sample_size_derivative(
        self,
        z_t: torch.Tensor,
        t: torch.Tensor,
        y: torch.Tensor | None = None,
    ) -> torch.Tensor:
        """Compute d/dt ESS(z_t, t) exactly from posterior component derivatives.

        If w_i(t) are the posterior component weights and
            ESS(t) = 1 / sum_i w_i(t)^2,
        then
            dESS/dt = -2 ESS(t)^2 sum_i w_i(t) dw_i/dt,
        with
            dw_i/dt = w_i (g_i - sum_j w_j g_j),
            g_i = d/dt log unnormalized_weight_i.
        """
        terms = self._compute_posterior_terms(z_t, t, label_filter=y)
        w = terms["w"]
        z_flat = terms["z_flat"]
        ...
        t_scalar = t[0]
        alpha, sigma = self._get_alpha_sigma(t_scalar)
        alpha_dot, sigma_dot = self._get_alpha_sigma_derivatives(t_scalar)
        alpha = alpha.to(z_t.device)
        sigma = sigma.to(z_t.device)
        alpha_dot = alpha_dot.to(z_t.device)
        sigma_dot = sigma_dot.to(z_t.device)

        mu_dot = alpha_dot * self.all_mu_flat
        var_dot = 2.0 * alpha * alpha_dot * self.all_var_flat + 2.0 * sigma * sigma_dot

        inv_var = one_over_var
        inv_var_sq = inv_var * inv_var

        a_term = mu_dot * inv_var
        b_term = (mu_t * mu_dot * inv_var).sum(dim=1).unsqueeze(0)

        c_term = var_dot * inv_var_sq
        d_term = var_dot * mu_t * inv_var_sq
        e_term = (var_dot * mu_t * mu_t * inv_var_sq).sum(dim=1).unsqueeze(0)
        f_term = (var_dot * inv_var).sum(dim=1).unsqueeze(0)
        ...
        g_bar = (w * g).sum(dim=1)
        w_sq = w * w
        s2 = w_sq.sum(dim=1).clamp_min(1e-12)
        ess = 1.0 / s2
        d_s2_dt = 2.0 * ((w_sq * g).sum(dim=1) - s2 * g_bar)
        return -d_s2_dt * (ess * ess)
...
def evaluate_current_state(
    ...
    mean_estimation_mse_uncond = 0.0
    mean_estimation_t_centres = np.zeros(30, dtype=np.float64)
    mean_estimation_by_t_bin = np.zeros(30, dtype=np.float64)
    ...
    # -----------------------------------------------------------------------
    # Shared oracle-init banks for reverse-trajectory visualizations
    # -----------------------------------------------------------------------
    oracle_z0_bank = real_latents_A
    oracle_noise_bank = fixed_noise_bank[sample_indices] if fixed_noise_bank is not None else None
    oracle_label_bank = real_labels
    ...
    mean_estimation_plot_path = None
    ...
    # -----------------------------------------------------------------------
    # Viz IIIb: Mean estimation variance breakdown by t (log-log)
    # -----------------------------------------------------------------------
    if eval_dir is not None:
        print("  [Viz IIIb] Plotting Bayes mean-estimation MSE by t ...")
        mean_estimation_mse_uncond, mean_estimation_t_values, mean_estimation_mse_values = compute_mean_estimation_mse_by_t(
            oracle_model,
            encoder_mus, encoder_logvars,
            cfg, device,
            labels=None,
        ...
        mean_estimation_plot_path = os.path.join(
            eval_dir, f"{prefix}_mean_estimation_mse_by_t_ep{epoch_idx}.png"
        )
        plot_mean_estimation_mse_by_t(
            mean_estimation_t_values, mean_estimation_mse_values,
            save_path=mean_estimation_plot_path,
            cache_key=prefix,
        )
    ...
    if eval_dir is not None:
        ...
        plot_decoder_output_grid(
            ...
            num_rows=6, num_cols=8, t_upper=2.00, t_values = [.01, .05, .15, .4, .7, 1.0, 1.5, 1.99],
            unet=unet,
            real_labels=real_labels,
    ...
    # -----------------------------------------------------------------------
    # Viz VI: VCSEM breakdown by t (log-log, eps-space)
    # -----------------------------------------------------------------------
    if eval_dir is not None:
        print("  [Viz VI] Plotting VCSEM by t ...")
        _, vcsem_t_values, vcsem_values = compute_vcsem_by_t(
            oracle_model,
            encoder_mus, encoder_logvars,
            cfg, device,
            labels=None,
        ...
        plot_vcsem_by_t(
            vcsem_t_values, vcsem_values,
            save_path=os.path.join(eval_dir, f"{prefix}_vcsem_by_t_ep{epoch_idx}.png"),
            cache_key=prefix,
        )

    # -----------------------------------------------------------------------
    # Viz VII: Mean ESS derivative breakdown by t
    # -----------------------------------------------------------------------
    if eval_dir is not None:
        print("  [Viz VII] Plotting mean d/dt ESS by t ...")
        _, ess_t_values, ess_derivative_values = compute_mean_ess_derivative_by_t(
            oracle_model,
            encoder_mus, encoder_logvars,
            cfg, device,
            labels=None,
        ...
        plot_mean_ess_derivative_by_t(
            ess_t_values, ess_derivative_values,
            save_path=os.path.join(eval_dir, f"{prefix}_ess_derivative_by_t_ep{epoch_idx}.png"),
            cache_key=prefix,
        )

    # -----------------------------------------------------------------------
    # Viz VIII: Frontier smoothed log-reconstruction derivative
    # -----------------------------------------------------------------------
    if eval_dir is not None:
        print("  [Viz VIII] Plotting smoothed log-reconstruction derivative by t ...")
        plot_frontier_log_recon_derivative(
            frontier_tracker,
            save_path=os.path.join(eval_dir, f"{prefix}_frontier_log_recon_derivative_ep{epoch_idx}.png"),
            epoch=epoch_idx,
            cache_key=prefix,
        )
            ...
            if method == "VAE_Rec_eps":
                ...
                for i in range(0, target_count, bs):
                    ...
                    if sampler_eval_settings["init_mode"] == "oracle":
                        oracle_y_batch = real_labels[i:i + batch_sz].to(device=device, dtype=torch.long)

                    y_batch = None
                    if oracle_y_batch is not None:
                        y_batch = oracle_y_batch
                    elif use_rand_token and rand_token_bank_all is not None:
    ...
    output_dict["mean_estimation_plot_uncond"] = mean_estimation_plot_path
    ...
    output_dict["mean_estimation_mse_uncond"] = mean_estimation_mse_uncond
    for r in results:
        config = r["config"]
...
def train_vae_cotrained_cond(cfg):
    ...
    time_cond_decode = bool(cfg.get("time_cond_decode", cfg.get("time_cond_decoder", False)))
    requested_adaptive_time = bool(cfg.get("adaptive_time", False))
    use_adaptive_time = requested_adaptive_time and time_cond_decode
    frontier_tracker = None
    if requested_adaptive_time and not time_cond_decode:
        print("--> Adaptive frontier time sampling requested, but time_cond_decode=False; falling back to base time_schedule sampling.")
    ...
    vae = VAE(
                  ...
                  multiscale_decoder=cfg.get("multiscale_decoder", False),
                  decoder_num_bands=int(cfg.get("decoder_num_bands", 3)),
                  decoder_band_premix=cfg.get("decoder_band_premix", True),
    ...
    if div_mode not in ("head_ratio", "tweedie", "direct"):
        raise ValueError(
            f"Unknown div_mode={div_mode!r}. Expected 'head_ratio', 'tweedie', or 'direct'."
        )
    if div_mode == "direct":
        dit_kwargs_div = dict(dit_kwargs)
        dit_kwargs_div["factored_head"] = False
        unet_diversity = UNetModel(**dit_kwargs_div).to(device)
    else:
        unet_diversity = None
    ...
    if cfg.get("load_from_checkpoint", False):
        ...
        if unet_diversity is not None:
            try:
                unet_diversity.load_state_dict(torch.load(_resolve_ckpt_path("unet_diversity.pt"), map_location=device), strict=False)
                print("    Loaded UNet Diversity.")
            except Exception as e:
                print(f"    Warning: Could not load UNet Diversity ({e})")
    ...
    temp_w = float(cfg.get("temp_w", 0.0))
    project_div = bool(cfg.get("project_div", False))
    freeze_score_in_cotrain = cfg.get("freeze_score_in_cotrain", False)
    need_div_consensus = (div_w != 0.0) or (project_div and temp_w != 0.0)
    if need_div_consensus and div_mode in ("head_ratio", "tweedie") and not bool(getattr(unet_lsi, "factored_head", False)):
        raise ValueError(
            "Diversity losses with div_mode in {'head_ratio', 'tweedie'} require factored_head=True "
    ...
    opt_diversity = None
    if unet_diversity is not None:
        opt_diversity = optim.AdamW(unet_diversity.parameters(), lr=cfg["lr_ldm"], weight_decay=1e-4, betas=(0.9, float(cfg.get("adam_beta2", 0.95))))
    ...
    sched_diversity = CosineAnnealingLR(opt_diversity, T_max=cfg["epochs_vae"], eta_min=1e-6) if opt_diversity is not None else None
    ...
    use_tdd_global = bool(cfg.get("time_cond_decode", cfg.get("time_cond_decoder", False)))
    ...
    for ep in range(cfg["epochs_vae"]):
        ...
        metrics = {k: 0.0 for k in ["loss", "recon", "kl", "score_lsi", "score_control", "aux_lam", "aux_nu", "div", "temp_div", "perc", "stiff", "gan_d", "gan_g", "tdd"]}
        mu_stats = []
        logvar_stats = []
        ...
        for x, y in tqdm(train_l, desc=f"Ep {ep+1}", leave=False):
            ...
            use_cond_enc = bool(cfg.get("use_cond_encoder", False))
            use_tdd = bool(cfg.get("time_cond_decode", cfg.get("time_cond_decoder", False)))
            ...
            temp_div_loss = torch.tensor(0.0, device=device)
            div_head_loss = torch.tensor(0.0, device=device)
            ...
            need_div_direction = project_div and (temp_w != 0.0)
            need_lsi_div_outputs = need_div_consensus and (div_mode in ("head_ratio", "tweedie"))
            ...
            if freeze_score_in_cotrain:
                ...
                if use_factored:
                    ...
                    if need_div_consensus and div_mode in ("head_ratio", "tweedie"):
                        if div_mode == "head_ratio":
                            eps_safe = 1e-3
            ...
            if need_div_consensus:
                ...
                if mu_consensus is not None:
                    ...
                    if project_div and temp_w != 0.0:
                        temp_div_loss = compute_projected_diversity_loss(
                            mu_t,
                            mu_consensus,
                            eps_hat=eps_for_div,
                        )
            ...
            #
            #   time_cond_decode=True  → existing TDD behavior:
            #       recon / LPIPS / GAN are evaluated on the diffused decode path.
            #   time_cond_decode=False → classic co-training behavior:
            #       score loss stays at (z_t, t), but recon / LPIPS / GAN are all
            #       evaluated on the clean latent z_0 using a fixed clean decode time.
            if use_tdd:
                ...
                clean_t = None
                if getattr(vae, "time_cond_decoder", False):
                    clean_t_value = float(cfg.get("decode_time", cfg.get("t_min", 1e-4)))
                    clean_t = torch.full((B,), clean_t_value, device=device, dtype=t.dtype)
                x_rec = vae.decode(z0, t=clean_t, y=y_in)
                x_rec_mse = x_rec
            ...
            if use_tdd and mse_mode == "score" and not freeze_score_in_cotrain:
            ...
            # This tracker is only active when decoder-side losses are applied at z_t.
            ...
            if freeze_score_in_cotrain:
                ...
                loss_joint = recon + cfg["perc_w"]*perc + cfg["kl_w"]*kl + div_w*div_loss + temp_w*temp_div_loss + gan_w_eff*g_loss
            else:
                # Co-training mode: include score loss
                score_w_vae = cfg.get("score_w_vae", cfg["score_w"])
                if cotrain_head == "lsi":
                    loss_joint = recon + cfg["perc_w"]*perc + cfg["kl_w"]*kl + score_w_vae*score_loss_lsi + div_w*div_loss + temp_w*temp_div_loss + stiff_w*stiff_pen + gan_w_eff*g_loss
                else:  # cotrain_head == "control"
                    loss_joint = recon + cfg["perc_w"]*perc + cfg["kl_w"]*kl + score_w_vae*score_loss_control + div_w*div_loss + temp_w*temp_div_loss + stiff_w*stiff_pen + gan_w_eff*g_loss
            ...
            if opt_diversity is not None:
                opt_diversity.zero_grad()
                div_head_loss.backward()
                nn.utils.clip_grad_norm_(unet_diversity.parameters(), 1.0)
                opt_diversity.step()
            ...
            metrics["temp_div"] += temp_div_loss.item()
        ...
        epoch_metrics = {
            ...
            "temp_div": metrics["temp_div"] / n_batches,
        ...
        if sched_tracking is not None:
            sched_tracking.step()
        if sched_diversity is not None:
            sched_diversity.step()
        ...
        if should_eval_cotrain:
            ...
            if unet_diversity is not None:
                save_checkpoint(unet_diversity.state_dict(), os.path.join(cfg["ckpt_dir"], "unet_diversity.pt"))


    if results_dir is not None and not freeze_score_in_cotrain:
        make_eval_plot_videos(
            os.path.join(results_dir, "evals"),
            prefix="LSI_Diff",
    ...
    if epochs_refine > 0:
        ...
        use_tdd_refine = bool(cfg.get("time_cond_decode", cfg.get("time_cond_decoder", False)))
        use_tdd_gan_refine = use_tdd_refine and bool(cfg.get("time_dependent_gan", True))

        decoder_params = []
        for name, p in vae.named_parameters():
            is_decoder_param = name.startswith("dec_")
            p.requires_grad = is_decoder_param
            if is_decoder_param:
        ...
        opt_dec_refine = optim.AdamW(decoder_params, lr=lr_refine, weight_decay=1e-4, betas=(0.9, float(cfg.get("adam_beta2", 0.95)))) if len(decoder_params) > 0 else None
        ...
        sched_dec_refine = CosineAnnealingLR(opt_dec_refine, T_max=epochs_refine, eta_min=1e-7) if opt_dec_refine is not None else None

        for ep in range(epochs_refine):
            vae.train(); unet_lsi.train(); unet_control.train()
            if disc is not None:
                disc.train()
            metrics_refine = {k: 0.0 for k in ["loss", "recon", "perc", "gan_d", "gan_g", "score_lsi", "score_control", "aux_lam", "aux_nu"]}
            ...
            for x, y in tqdm(train_l, desc=f"Refine Ep {ep+1}", leave=False):
                ...
                # --- Oracle TDD for frontier tracker (refine: encoder frozen, decoder trainable) ---
                ...
                # --- Decoder refinement (decoder-only; encoder frozen; no score↔decoder gradients) ---
                if opt_dec_refine is not None:
                    if use_tdd_refine:
                        z_decode = z_t.detach()
                        t_decode = t
                    else:
                        ...
                        if getattr(vae, "time_cond_decoder", False):
                            clean_t_value = float(cfg.get("decode_time", cfg.get("t_min", 1e-4)))
                            t_decode = torch.full((B,), clean_t_value, device=device, dtype=t.dtype)
                        else:
                            t_decode = None

                    x_rec = vae.decode(z_decode, t=t_decode, y=y_in)
                    recon = decode_w * F.mse_loss(x_rec, x)
                    ...
                    if LPIPS_AVAILABLE:
                        ...
                        lpips_per = lpips_fn(x_rec_3c, x_3c)
                        lpips_per = lpips_per.view(B, -1).mean(dim=1) if lpips_per.dim() > 1 else lpips_per.view(B)
                        if not use_tdd_refine:
                            perc = lpips_per.mean()
                        elif lpips_mode == "uniform":
                            perc = lpips_per.mean()
                        elif lpips_mode == "snr":
                            snr_weight = (alpha.view(B, 1, 1, 1) ** 2).mean()
                            perc = snr_weight * lpips_per.mean()
                        elif lpips_mode == "frontier":
                            R_cutoff = float(cfg.get("frontier_R_cutoff", 0.05))
                            R_tau = float(cfg.get("frontier_R_tau", 1.0))
                    ...
                    use_gan_refine = gan_w_eff > 0.0 and disc is not None and (ep + 1) >= disc_start_epoch
                    if use_gan_refine:
                        if use_tdd_gan_refine:
                            with torch.no_grad():
                                x_real = wiener_reference_x0(
                                    x,
                                    alpha,
                        ...
                        gan_time_mode = str(cfg.get("gan_time_weight", "uniform")).lower()
                        if gan_time_mode != "uniform" and use_tdd_refine:
                            if gan_time_mode == "frontier":
                                R_cutoff = float(cfg.get("frontier_R_cutoff", 0.05))
                                R_tau = float(cfg.get("frontier_R_tau", 1.0))
                                if frontier_tracker is not None and frontier_tracker.is_active:
                    ...
                    d_loss = torch.tensor(0.0, device=device)
                    g_loss = torch.tensor(0.0, device=device)
                    loss_dec_refine = torch.tensor(0.0, device=device)

                metrics_refine["loss"] += loss_dec_refine.item()
            ...
            epoch_metrics = {
                "epoch": global_epoch,
                "stage": "refine",
                "loss": metrics_refine["loss"] / n_batches,
            ...
            print(f"Refine Ep {ep+1} | LSI: {epoch_metrics['score_lsi']:.4f} | "
                  ...
                  f"Rec: {epoch_metrics['recon']:.4f} | Perc: {epoch_metrics['perc']:.4f} | GAN_d: {epoch_metrics['gan_d']:.4f} | GAN_g: {epoch_metrics['gan_g']:.4f}")

            # --- Step cosine LR schedulers ---
            sched_lsi_refine.step()
            sched_control_refine.step()
            if sched_dec_refine is not None:
...
def main():
    ...
    cfg_shared = {
        ...
        "multiscale_decoder": True,      # Laplacian bottleneck side-bands in decoder
        "decoder_num_bands": 3,          # coarse / mid / fine
        "decoder_band_premix": True,     # learned 1x1 premix before band split
    ...
    cfg_indep.update({
        ...
        "epochs_refine": 240,       # LDM training on frozen VAE
        "lr_refine": 5e-4,
        "cfg_label_dropout": 0.1,
        "t_min": 3e-4,
