...
# ── Decoder-specific time embedding helper ─────────────────────────────

def decoder_time_embedding(t: torch.Tensor, dim: int, schedule_type: str = "log_t") -> torch.Tensor:
    """Compute a time embedding for decoder-side modules using log-SNR when appropriate."""
...
class VAE(nn.Module):
    ...
    Recommended CIFAR call (with all improvements):
        VAE(latent_channels=8, base_ch=64, img_size=32, img_channels=3,
            ...
            class_decoder=True, decoder_num_classes=10)
    """
    ...
    def __init__(
        ...
        encoder_attn_half: bool = False,
        decoder_extra_block: bool = False,
        conv3x3_proj: bool = False,
        use_tanh_out: bool = True,
        clamp_logvar: bool = False,
        attn_zero_init: bool = True,
        # --- Time-dependent decoder (TDD) ---
        time_cond_decoder: bool = False,
        dec_time_emb_dim: int = 128,
        ...
        #  DECODER
        # ================================================================
        dec_in_ks = 3 if self.conv3x3_proj else 1
        if latent_proj_depth >= 2:
        ...
        dec_nrb = num_res_blocks + (1 if decoder_extra_block else 0)
    ...
    def decode(self, z, t=None, y: torch.Tensor | None = None):
        ...
        elif self.time_cond_decoder and t is None:
            import warnings
        ...
        elif y_ids is not None and self.dec_y_emb is not None and self.dec_label_proj is not None:
            h = h + self.dec_label_proj(self.dec_y_emb(y_ids)).view(B, h.shape[1], 1, 1)

        for i, block in enumerate(self.dec_blocks):
            h = block(h)
            if t_emb is not None:
                scale_shift = self.dec_film_layers[i](t_emb)
