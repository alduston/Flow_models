"""
Latent Diffusion Model for CIFAR-10
===================================
A complete, self-contained implementation in Stable Diffusion style:
- Diagonal Gaussian VAE with small KL regularization
- LPIPS perceptual loss
- Optional PatchGAN discriminator (adversarial loss)
- UNet for latent diffusion
- DDPM/DDIM samplers
- FID evaluation

Designed for easy modification to add:
- BatchNorm to VAE encoder
- LSI loss for co-training
- Joint VAE-diffusion optimization

Author: Implementation synthesized from CompVis/latent-diffusion, stanipov/pytorch-diffusion
Target: Single-digit FID on CIFAR-10
"""
...
class UNet(nn.Module):
    ...
    def forward(self, x, t, y=None, drop_class_prob=0.0):
        ...
        if y is not None:
            ...
            # Unconditional
            c_emb = self.class_emb(torch.full((x.shape[0],), self.null_class, device=x.device))
            t_emb = t_emb + c_emb
...
def evaluate_fid(ldm, dataloader, num_samples=50000, batch_size=128, device='cuda'):
    ...
    fake_images = torch.cat(fake_images)[:num_samples].to(device)
