...
def evaluate_current_state(
    ...
    # -----------------------------------------------------------------------
    # Transform results list into flat dict for DataFrame logging
    # -----------------------------------------------------------------------
    output_dict = {}
    ...
    for r in results:
        ...
        if "VAE_Rec_eps" in config:
            ...
            steps = config.split("@")[1] if "@" in config else "10"
            output_dict[f"fid_rk4_{steps}"] = r["fid"]
            output_dict[f"kid_rk4_{steps}"] = r["kid"]
            output_dict[f"sw2_rk4_{steps}"] = r["w2"]
            output_dict[f"div_rk4_{steps}"] = r["div"]
            output_dict[f"lsi_gap_rk4_{steps}"] = r["lsi_gap"]
        elif "heun" in config.lower():
            steps = config.split("@")[1] if "@" in config else "20"
            output_dict[f"fid_heun_{steps}"] = r["fid"]
            output_dict[f"kid_heun_{steps}"] = r["kid"]
            output_dict[f"sw2_heun_{steps}"] = r["w2"]
            output_dict[f"div_heun_{steps}"] = r["div"]
            output_dict[f"lsi_gap_heun_{steps}"] = r["lsi_gap"]
...
def train_vae_cotrained(cfg):
    ...
    for ep in range(cfg["epochs_vae"]):
        ...
        for x, _ in tqdm(train_l, desc=f"Ep {ep+1}", leave=False):
            ...
            if cotrain_head == "lsi":
                # Control tracks
                eps_pred_control_tracking = unet_control(z_t_detached, t)
                tracking_loss = cfg["score_w"] * F.mse_loss(eps_pred_control_tracking, noise)
            else:
                # LSI tracks
                eps_pred_lsi_tracking = unet_lsi(z_t_detached, t)
