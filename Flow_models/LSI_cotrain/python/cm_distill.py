...
Place this cell directly below lsi_cotrain.py in a Google Colab notebook.
All imports and definitions from lsi_cotrain.py are assumed to be available.
...
def evaluate_consistency_mnist(
    ...
    Uses the same FID computation approach as evaluate_current_state from lsi_cotrain.py:
...
def train_consistency_distillation_mnist(
    ...
    Args:
        ...
        vae: Pre-trained VAE for encoding/decoding (frozen)
    ...
    for ep in range(epochs_cm):
        ...
        if (ep + 1) % eval_every == 0:
            ...
            # Save checkpoint
            ckpt_path = os.path.join(cfg["results_dir"], "checkpoints", f"{exp_name}_ep{ep+1}.pt")
...
def run_consistency_experiment_mnist(dataset_key="MNIST"):
    """
    ...
    vae_cotrained = VAE(latent_channels=cfg["latent_channels"]).to(device)
    ...
    vae_indep = VAE(latent_channels=cfg["latent_channels"]).to(device)
...
if __name__ == "__main__":
    # Run for MNIST by default; change to "FMNIST" for FashionMNIST
    run_consistency_experiment_mnist(dataset_key="FMNIST")
