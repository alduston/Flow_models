...
def compute_lsi_gap(
    ...
    - For flow-matching (time_schedule='flow'): assumes the score_net predicts velocity v, and converts
      v -> eps via eps = z_t - t * v (since z_t = eps + t*v).
    """
        ...
        with torch.no_grad():
            for i in range(0, num_samples, batch_size):
                ...
                for t_val in t_grid:
                    ...
                    # model predicts velocity; convert to eps
                    v_pred = score_net(z_t, t, y_batch)
...
class DiTModel(nn.Module):
    """Diffusion Transformer (DiT) configured for **flow matching** (velocity prediction).
    ...
    - outputs a *velocity field* v(z_t, t)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, y: torch.Tensor | None = None) -> torch.Tensor:
        ...
        # final adaLN modulation + projection
...
class UniversalSampler:
    ...
    def get_ode_derivative(self, x, t, unet, y=None, cfg_scale=None):
        ...
        if self.schedule_type in ("flow", "flow_matching"):
            v_pred = self._predict_eps(unet, x, t_vec, y=y, cfg_scale=cfg_scale)
            return v_pred
...
def train_vae_cotrained_cond(cfg):
    ...
    for ep in range(cfg["epochs_vae"]):
        ...
        for x, y in tqdm(train_l, desc=f"Ep {ep+1}", leave=False):
            ...
            z0_hat = mu + (alpha * var_0) * resid  # E[z0 | z_t, x]
            v_target_lsi = z0_hat - eps_target_lsi  # E[v=z0-eps | z_t, x]

            if cfg.get("train_on_mu", False):
                v_target_control = mu - noise
            else:
                v_target_control = z0 - noise

            if freeze_score_in_cotrain:
                score_loss_lsi = torch.tensor(0.0, device=device)
                score_loss_control = torch.tensor(0.0, device=device)
            else:
                v_pred_lsi = unet_lsi(z_t, t, y_in)
                loss_mse_lsi = F.mse_loss(v_pred_lsi, v_target_lsi)
                loss_cos_lsi = (1.0 - F.cosine_similarity(v_pred_lsi.flatten(1), v_target_lsi.flatten(1), dim=1)).mean()
                score_loss_lsi = loss_mse_lsi + cos_w * loss_cos_lsi

                if cfg.get("train_on_mu", False):
                    v_pred_control = unet_control(z_mu_t, t, y_in)
                else:
                    v_pred_control = unet_control(z_t, t, y_in)
                loss_mse_ctrl = F.mse_loss(v_pred_control, v_target_control)
                loss_cos_ctrl = (1.0 - F.cosine_similarity(v_pred_control.flatten(1), v_target_control.flatten(1), dim=1)).mean()
            ...
            if not freeze_score_in_cotrain:
                ...
                v_target_control_det = v_target_control.detach()
                v_target_lsi_det = v_target_lsi.detach()

                if cotrain_head == "lsi":
                    # Control head tracks the velocity baseline
                    x_in = z_mu_t_detached if cfg.get("train_on_mu", False) else z_t_detached
                    v_pred_control_tracking = unet_control(x_in, t, y_in)
                    loss_mse = F.mse_loss(v_pred_control_tracking, v_target_control_det)
                    loss_cos = (1.0 - F.cosine_similarity(v_pred_control_tracking.flatten(1), v_target_control_det.flatten(1), dim=1)).mean()
                    tracking_loss = cfg["score_w"] * (loss_mse + cos_w * loss_cos)
                else:
                    # LSI head tracks the CSEM velocity target
                    v_pred_lsi_tracking = unet_lsi(z_t_detached, t, y_in)
                    loss_mse = F.mse_loss(v_pred_lsi_tracking, v_target_lsi_det)
                    loss_cos = (1.0 - F.cosine_similarity(v_pred_lsi_tracking.flatten(1), v_target_lsi_det.flatten(1), dim=1)).mean()
                    tracking_loss = cfg["score_w"] * (loss_mse + cos_w * loss_cos)
    ...
    if epochs_refine > 0:
        ...
        for ep in range(epochs_refine):
            ...
            for x, y in tqdm(train_l, desc=f"Refine Ep {ep+1}", leave=False):
                ...
                resid = (z_t - mu_t) / (var_t + 1e-8)
                eps_target_lsi = sigma * resid
                z0_hat = mu + (alpha * var_0) * resid
                v_target_lsi = z0_hat - eps_target_lsi
