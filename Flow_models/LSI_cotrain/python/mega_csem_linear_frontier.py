...
def sample_uniform_times(B: int, t_min: float, t_max: float, device: torch.device) -> torch.Tensor:
    u = torch.rand(B, device=device)
...
def make_schedule(cfg: Dict[str, Any], device: torch.device) -> Dict[str, torch.Tensor]:
    ...
    raise ValueError(f"Unknown time_schedule: {stype!r}. Expected 'flow', 'linear', 'log_t', 'log_snr', or 'cosine'.")
...
# the region in t where |dR / d(log t)| is largest.
#
# R(t) = E[ ||D(z_t, t) - D(z_0, None)||^2 ]  (oracle TDD error)
# Adaptive weight:  w(t) ~ |dR / d(log t)| + floor
#
...
class ReconFrontierTracker:
    ...
    def _compute_weights(self):
        ...
        R_smooth = self._gaussian_smooth_1d(self.R_ema.clamp(min=1e-12), self.smooth_sigma)
        log_t = torch.log(self.bin_centers)
        dR = R_smooth[1:] - R_smooth[:-1]
        d_logt = log_t[1:] - log_t[:-1]
        grad = (dR / (d_logt + 1e-12)).abs()
    ...
    def make_adaptive_time_grid(self, N: int, device: torch.device,
        ...
        |dR / d(log t)| is large — matching training to inference.
...
def plot_lpips_decay_vs_t(
    ...
    """Viz: LPIPS decay E[lpips(x_0, D(z_t,t))] vs t (log-log).
    ...
    with torch.no_grad():
        for tv in t_values:
            ...
            for j in range(0, num_samples, batch_size):
                z_batch = z_t[j:j + batch_size]
                x_batch = imgs_sub[j:j + batch_size]
                bsz_j = z_batch.shape[0]
                t_vec_j = torch.full((bsz_j,), tv, device=device)
                y_j = labels_sub[j:j + batch_size] if labels_sub is not None else None
                ...
                if x_batch.shape[1] == 1:
                    x_lp = x_batch.repeat(1, 3, 1, 1)
                    recon_lp = recon.repeat(1, 3, 1, 1)
                else:
                    x_lp = x_batch
                    recon_lp = recon

                lp_batch = lpips_fn(x_lp, recon_lp)
...
def train_vae_cotrained_cond(cfg):
    ...
    for ep in range(cfg["epochs_vae"]):
        ...
        for x, y in tqdm(train_l, desc=f"Ep {ep+1}", leave=False):
            ...
            schedule_type = str(cfg.get("time_schedule", "")).lower()
            if frontier_tracker is not None and frontier_tracker.is_active:
                ...
                if use_ddim_times:
                    ...
                    alpha = extract_schedule(ou_sched["alpha"], t_idx, z0.shape)
                    sigma = extract_schedule(ou_sched["sigma"], t_idx, z0.shape)
                elif schedule_type in ("flow", "flow_matching"):
                ...
                if schedule_type in ("flow", "flow_matching"):
                    ...
                    t = sample_uniform_times(B, cfg["t_min"], cfg["t_max"], device)
                    alpha, sigma = get_ou_params(t.view(B, 1, 1, 1))
                else:
            ...
            if frontier_tracker is not None and frontier_tracker.is_active:
                if bool(cfg.get("frontier_correct_score", False)):
                    ref = "uniform" if schedule_type in ("flow", "flow_matching", "linear") else "log_uniform"
...
def main():
    ...
    cfg_shared = {
        ...
        # --- Diffusion Settings ---
        "time_schedule": "log_t",     # "flow", "linear", "log_t", "log_snr", or "cosine"
        "use_ddim_times": True,
        "t_min": 1e-4,
        "t_max": 2.0,
    ...
    cfg_cotrain.update({
        ...
        "decode_time": 3e-4,             # Decode at this t; defaults to t_min if None
        #"snr_downweight": True,
