...
def train_vae_cotrained_cond(cfg):
    ...
    for ep in range(cfg["epochs_vae"]):
        ...
        for x, y in tqdm(train_l, desc=f"Ep {ep+1}", leave=False):
            ...
            # --- Perceptual loss (LPIPS) ---
            # If align_lpips=True, we compute:
            #   perc = L_clean(y0) + w(t) * gate(||y_t-y0||) * L_align(y0, y_t)
            # where L_clean(y0) = LPIPS(y0, x0) and
            #       L_align(y0,y_t) = ( <sg(∇_{y0} L_clean), y_t - y0> / ||sg(∇_{y0} L_clean)|| )^2.
            # This constrains the time-conditioned decode y_t to drift in perceptually-tangent directions
            # near y0, without directly optimizing LPIPS at large t.
            lpips_mode = str(cfg.get("lpips_mode", "uniform")).lower()
            align_lpips = bool(cfg.get("align_lpips", False))
            if LPIPS_AVAILABLE:
                x_3c = x.repeat(1, 3, 1, 1) if x.shape[1] == 1 else x
                x_rec_3c = x_rec.repeat(1, 3, 1, 1) if x_rec.shape[1] == 1 else x_rec

                if not align_lpips:
                    # Legacy paired LPIPS on the time-conditioned reconstruction y_t.
                    ...
                    # Alignment LPIPS: LPIPS only anchors the clean decode y0; y_t is constrained
                    # by a first-order (local) alignment penalty.
                    # Clean decode y0 = D(z0) (FiLM bypassed) — corresponds to the sampling-time decoder.
                    y0 = vae.decode(z0)
                    y0_3c = y0.repeat(1, 3, 1, 1) if y0.shape[1] == 1 else y0

                    # L_clean(y0): paired LPIPS on clean decode.
                    lpips_clean = lpips_fn(y0_3c, x_3c)
                    lpips_clean = lpips_clean.view(B, -1).mean(dim=1) if lpips_clean.dim() > 1 else lpips_clean.view(B)
                    L_clean = lpips_clean.mean()

                    # g0 = ∇_{y0} L_clean(y0) (stop-grad to avoid 2nd-order effects)
                    g0 = torch.autograd.grad(L_clean, y0_3c, retain_graph=True, create_graph=False)[0].detach()
                    delta = (x_rec_3c - y0_3c.detach())
                    dot = (g0 * delta).flatten(1).sum(dim=1)
                    gnorm = g0.flatten(1).norm(dim=1).clamp_min(1e-12)
                    align_per = (dot / (gnorm + float(cfg.get("align_lpips_eps", 1e-6)))) ** 2

                    # Soft trust-region gate on ||delta|| (detach so the model cannot game the gate).
                    # Uses RMS(delta) in pixel space.
                    delta_rms = delta.pow(2).flatten(1).mean(dim=1).sqrt()
                    rho = float(cfg.get("align_lpips_rho", 0.12))
                    tau = float(cfg.get("align_lpips_tau", 0.02))
                    gate = torch.sigmoid((rho - delta_rms) / max(tau, 1e-8)).detach()

                    # Time weighting w(t): reuse lpips_mode as the temporal weight on the alignment term.
                    if lpips_mode == "uniform":
                        w_t = torch.ones_like(gate)
                    elif lpips_mode == "snr":
                        w_t = torch.full_like(gate, float(snr_weight.detach().item()) if torch.is_tensor(snr_weight) else float(snr_weight))
                    elif lpips_mode == "frontier":
                        R_cutoff = float(cfg.get("frontier_R_cutoff", 0.05))
                        R_tau = float(cfg.get("frontier_R_tau", 1.0))
                        if frontier_tracker is not None and frontier_tracker.is_active:
                            w_t = frontier_tracker.perceptual_weights(t, R_cutoff=R_cutoff, temperature=R_tau)
                        else:
                            gamma = (alpha ** 2) / ((alpha ** 2) + (sigma ** 2) + 1e-12)
                            w_t = gamma.view(B, -1).mean(dim=1).detach()
                    else:
                        gamma = (alpha ** 2) / ((alpha ** 2) + (sigma ** 2) + 1e-12)
                        gamma = gamma.view(B, -1).mean(dim=1)
                        if lpips_mode == "gamma":
                            ...
                            if (not freeze_score_in_cotrain) and use_factored and (lam_pred is not None):
                                ...
                                lam_s = torch.log(lam_s.clamp_min(1e-8))
                                lam_bn = (lam_s - lam_s.mean()) / (lam_s.std(unbiased=False) + 1e-6)
                                prec_gate = torch.sigmoid(lam_bn).detach()
                                w_t = (gamma * prec_gate).detach()
                            else:
                                w_t = gamma.detach()
                        else:
                            raise ValueError(f"Unknown lpips_mode: {lpips_mode!r}. Expected 'uniform', 'frontier', 'prec_mask', 'gamma', or 'snr'.")
                    ...
                    L_align = (w_t * gate * align_per).mean()
                    perc = L_clean + align_w * L_align
            else:
                perc = torch.tensor(0.0, device=device)
...
def main():
    ...
    cfg_shared = {
        ...
        # --- LPIPS alignment mode (experimental) ---
        # If align_lpips=True, LPIPS is applied only to the clean decode y0 = D(z0), and an additional
        # alignment penalty constrains the time-conditioned decode y_t = D(·, t) to drift in locally
        # LPIPS-tangent directions near y0. The lpips_mode value is re-used as a *temporal weight* on
        # the alignment term (in addition to the soft trust-region gate below).
        "align_lpips": True,
        "align_w": 1.0,
        "align_lpips_rho": 0.12,   # RMS(y_t - y0) threshold where the soft gate is ~0.5
        "align_lpips_tau": 0.02,   # gate temperature (smaller => sharper)
        "align_lpips_eps": 1e-6,   # numerical stabilizer in ||g0|| normalization
    ...
    cfg_cotrain.update({
        ...
        "eval_freq_cotrain": 33,    # Eval every 10 epochs during cotrain
