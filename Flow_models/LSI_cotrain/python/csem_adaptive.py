...
# ---------------------------------------------------------------------------
# Concentrates score-training budget at the "information frontier" of the
# decoder: the region in t where |d_t log R(t)| is largest.
#
# R(t) = E[ ||D(z_t, t) - D(z_0, None)||^2 ]  (oracle TDD error)
# Adaptive weight:  w(t) ~ |d(log R) / d(log t)| + floor
#
# Feedback loop: CONVERGENT (negative). Improvement at frontier reduces
# weight there; frontier walks outward to next unconquered region.
# ---------------------------------------------------------------------------
...
def train_vae_cotrained_cond(cfg):
    ...
    for ep in range(cfg["epochs_vae"]):
        ...
        for x, y in tqdm(train_l, desc=f"Ep {ep+1}", leave=False):
            ...
            if frontier_tracker is not None:
                with torch.no_grad():
                    x_clean = vae.decode(z0)  # D(z_0, None) — FiLM bypassed
                    oracle_tdd_mse = (x_rec.detach() - x_clean).pow(2).flatten(1).mean(1)  # [B]
        ...
        if frontier_tracker is not None:
            ...
            # Plot frontier curves periodically
    ...
    if epochs_refine > 0:
        ...
        # Re-initialize frontier tracker for refinement (VAE is now frozen,
        # reconstruction frontier may have shifted)
        if frontier_tracker is not None:
            frontier_tracker = ReconFrontierTracker(
                t_min=float(cfg["t_min"]),
                t_max=float(cfg["t_max"]),
        ...
        for ep in range(epochs_refine):
            ...
            for x, y in tqdm(train_l, desc=f"Refine Ep {ep+1}", leave=False):
                ...
                # Control head: same IW treatment (refine)
                if frontier_iw_refine is not None:
                    per_sample_mse_ctrl = (eps_pred_control - eps_target_control).pow(2).flatten(1).mean(1)
...
def main():
    ...
    cfg_cotrain.update({
        ...
        "stiff_w": 1e-9,
        "score_w": 1.0,
        "score_w_decode": 0.25,          # Gradient scale: score head ← MSE recon loss
