        ...
        for tv in t_values:
            ...
            for j in range(0, num_samples, batch_size):
                ...
                if getattr(vae, 'time_cond_decoder', False):
                    ...
                    t_dec = None
                recon_list.append(vae.decode(z_dec, t=t_dec))
    ...
    with torch.no_grad():
        ...
        for j in range(0, num_samples, batch_size):
            ...
            if getattr(vae, 'time_cond_decoder', False):
                ...
                t_ref_dec = None
            ref_recon_chunks.append(vae.decode(z0_batch, t=t_ref_dec))
        ...
        for tv in t_values:
            ...
            for j in range(0, num_samples, batch_size):
                ...
                if getattr(vae, 'time_cond_decoder', False):
                    ...
                    t_dec = None
                recon = vae.decode(z_dec, t=t_dec)
    ...
    with torch.no_grad():
        for tv in t_values:
            ...
            if getattr(vae, 'time_cond_decoder', False):
                ...
                t_dec = None
            decoded = vae.decode(z_dec, t=t_dec).cpu()
    ...
    if unet is not None:
         configs.extend([
            ...
            {"method": "rk4_ode",  "steps": 25, "desc": "RandToken (RK4)", "use_rand_token": True,"time_schedule": "log_t",
                 "init_mode": "oracle", "t_max": .25, "t_min": 1e-3, "cfg_level": 3.0, "readout_mode": "direct"},
    ...
    for ep in range(cfg["epochs_vae"]):
        ...
        for x, y in tqdm(train_l, desc=f"Ep {ep+1}", leave=False):
            ...
            if freeze_score_in_cotrain:
                ...
                score_w_vae = cfg.get("score_w_vae", cfg["score_w"])
                if cotrain_head == "lsi":
                    loss_joint = recon + cfg["perc_w"]*perc + cfg["kl_w"]*kl + score_w_vae*score_loss_lsi + stiff_w*stiff_pen + gan_w_eff*g_loss
                else:  # cotrain_head == "control"
                    loss_joint = recon + cfg["perc_w"]*perc + cfg["kl_w"]*kl + score_w_vae*score_loss_control + stiff_w*stiff_pen + gan_w_eff*g_loss
    ...
    if epochs_refine > 0:
        ...
        for ep in range(epochs_refine):
            ...
            for x, y in tqdm(train_l, desc=f"Refine Ep {ep+1}", leave=False):
                ...
                if frontier_tracker is not None:
                    with torch.no_grad():
                        x_rec_tdd = vae.decode(z_t, t)
                        x_clean_tdd = vae.decode(z0)
