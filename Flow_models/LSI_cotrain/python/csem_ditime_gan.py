    ...
    for ep in range(cfg["epochs_vae"]):
        ...
        for x, y in tqdm(train_l, desc=f"Ep {ep+1}", leave=False):
            ...
            if freeze_score_in_cotrain:
                ...
                loss_joint = recon + cfg["perc_w"]*snr_weight*perc + cfg["kl_w"]*kl + gan_w_eff*g_loss
            else:
                # Co-training mode: include score loss
                score_w_vae = cfg.get("score_w_vae", cfg["score_w"])
                if cotrain_head == "lsi":
                    loss_joint = recon + cfg["perc_w"]*snr_weight*perc + cfg["kl_w"]*kl + score_w_vae*score_loss_lsi + stiff_w*stiff_pen + gan_w_eff*g_loss
                else:  # cotrain_head == "control"
                    loss_joint = recon + cfg["perc_w"]*snr_weight*perc + cfg["kl_w"]*kl + score_w_vae*score_loss_control + stiff_w*stiff_pen + gan_w_eff*g_loss
