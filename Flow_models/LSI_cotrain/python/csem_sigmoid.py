...
Original file is located at
    https://colab.research.google.com/drive/1l0RQic_watih09_tD0vFjd80D2FD6st4
"""
...
def sample_sigmoid_times(
    B: int, t_min: float, t_max: float, device: torch.device,
    concentration: float = 3.0,
) -> torch.Tensor:
    """Sample OU times with U-shaped concentration at both t_min and t_max.

    Uses a Beta(beta, beta) distribution with beta < 1 to create
    a U-shaped density concentrating mass near both endpoints.
    concentration > 1 controls how peaked the edges are (beta = 1/concentration).
    """
    beta_param = 1.0 / max(concentration, 1.01)  # ensure < 1 for U-shape
    dist = torch.distributions.Beta(beta_param, beta_param)
    u = dist.sample((B,)).to(device)
...
def compute_temporal_derivative_loss(
    ...
    """Compute ||d/dtau eps_CSEM||^2 where tau is OU diffusion time.

    Fully analytic and element-wise (diagonal covariance).
    No network evaluations or spatial derivatives required.

    The temporal derivative of the CSEM epsilon target exposes the
    score-field stiffness at late diffusion times.  Under latent scale
    blowup the score transitions sharply near tau ~ T, producing a
    large ||d/dtau eps_CSEM||^2 that this loss penalises.

    Args:
        mu:     [B, C, H, W] encoder mean
        logvar: [B, C, H, W] encoder log-variance
        z_t:    [B, C, H, W] diffused latent (detached: position held fixed)
        t_ou:   [B] OU diffusion times

    Returns:
        Scalar mean ||d/dtau eps_CSEM||^2
    """
    t = t_ou.view(-1, 1, 1, 1)

    alpha = torch.exp(-t)
    alpha_sq = alpha ** 2
    sigma_sq = (1.0 - alpha_sq).clamp_min(1e-8)
    sigma = torch.sqrt(sigma_sq)

    var_0 = torch.exp(logvar)                             # encoder Sigma_phi

    # Diffused covariance diagonal: Sigma_tau = alpha^2 Sigma_phi + sigma^2 I
    var_t = alpha_sq * var_0 + sigma_sq
    inv_var_t = 1.0 / (var_t + 1e-8)

    mu_t = alpha * mu                                     # diffused mean
    r = z_t.detach() - mu_t                               # residual (z held fixed)

    # --- OU schedule time derivatives ---
    # d(alpha)/dtau = -alpha,  d(alpha^2)/dtau = -2*alpha^2
    # d(sigma^2)/dtau = 2*alpha^2,  d(sigma)/dtau = alpha^2 / sigma
    dsigma_dt = alpha_sq / (sigma + 1e-8)                 # d sigma / d tau

    # d r / d tau = -d(alpha)/dtau * mu = alpha * mu  (z held fixed)
    dr_dt = alpha * mu

    # d var_t / d tau = d(alpha^2 * var_0 + sigma^2)/dtau
    #                 = -2*alpha^2 * var_0 + 2*alpha^2 = 2*alpha^2*(1 - var_0)
    dvar_t_dt = 2.0 * alpha_sq * (1.0 - var_0)

    # d/dtau [sigma * r / var_t]  (product + quotient rule, element-wise)
    term1 = (dsigma_dt * r + sigma * dr_dt) * inv_var_t
    term2 = sigma * r * dvar_t_dt * (inv_var_t ** 2)
    deps_dt = term1 - term2

    return (deps_dt ** 2).flatten(1).mean()
...
def make_schedule(cfg: Dict[str, Any], device: torch.device) -> Dict[str, torch.Tensor]:
    ...
    if stype == "sigmoid":
        t_min = float(cfg.get("t_min", 2e-5))
        t_max = float(cfg.get("t_max", 2.0))
        concentration = float(cfg.get("sigmoid_concentration", 3.0))

        # Build grid with U-shaped concentration at both endpoints via Beta quantiles
        beta_param = 1.0 / max(concentration, 1.01)
        from scipy import stats as _scipy_stats
        u_grid = _scipy_stats.beta.ppf(
            np.linspace(1e-4, 1.0 - 1e-4, T), beta_param, beta_param
        )
        times = torch.tensor(
            t_min + (t_max - t_min) * u_grid,
            device=device, dtype=torch.float32,
        )
        times = times.clamp(t_min, t_max)
    ...
    raise ValueError(f"Unknown time_schedule: {stype!r}. Expected 'flow', 'log_t', 'log_snr', 'cosine', or 'sigmoid'.")
...
def train_vae_cotrained_cond(cfg):
    ...
    for ep in range(cfg["epochs_vae"]):
        ...
        metrics = {k: 0.0 for k in ["loss", "recon", "kl", "score_lsi", "score_control", "aux_lam", "aux_nu", "perc", "stiff", "temporal", "gan_d", "gan_g", "tdd"]}
        mu_stats = []
        ...
        for x, y in tqdm(train_l, desc=f"Ep {ep+1}", leave=False):
            ...
            elif str(cfg.get("time_schedule", "")).lower() == "sigmoid":
                t = sample_sigmoid_times(B, cfg["t_min"], cfg["t_max"], device,
                                         cfg.get("sigmoid_concentration", 3.0))
            ...
            # --- Temporal derivative loss (scale-blowup counter-pressure) ---
            temporal_w = float(cfg.get("temporal_w", 0.0))
            if temporal_w > 0.0 and not freeze_score_in_cotrain:
                temporal_pen = compute_temporal_derivative_loss(mu, logvar, z_t, t)
            else:
                temporal_pen = torch.tensor(0.0, device=device)
            ...
            if freeze_score_in_cotrain:
                ...
                score_w_vae = cfg.get("score_w_vae", cfg["score_w"])
                if cotrain_head == "lsi":
                    loss_joint = recon + cfg["perc_w"]*perc + cfg["kl_w"]*kl + score_w_vae*score_loss_lsi + stiff_w*stiff_pen + temporal_w*temporal_pen + gan_w*g_loss
                else:  # cotrain_head == "control"
                    loss_joint = recon + cfg["perc_w"]*perc + cfg["kl_w"]*kl + score_w_vae*score_loss_control + stiff_w*stiff_pen + temporal_w*temporal_pen + gan_w*g_loss
            ...
            metrics["temporal"] += temporal_pen.item()
        ...
        epoch_metrics = {
            ...
            "temporal": metrics["temporal"] / n_batches,
        ...
        print(f"Ep {ep+1} | LSI: {epoch_metrics['score_lsi']:.4f} | Ctrl: {epoch_metrics['score_control']:.4f} | AuxLam: {epoch_metrics['aux_lam']:.4f} | AuxNu: {epoch_metrics['aux_nu']:.4f} | "
              ...
              f"Stiff: {epoch_metrics['stiff']:.4f} | Temp: {epoch_metrics['temporal']:.4f} | GAN_d: {epoch_metrics['gan_d']:.4f} | GAN_g: {epoch_metrics['gan_g']:.4f} | TDD: {epoch_metrics['tdd']:.4f}")
...
def main():
    ...
    cfg_cotrain.update({
        ...
        # Temporal derivative loss (scale-blowup counter-pressure)
        "temporal_w": 1e-4,
        "sigmoid_concentration": 3.0,
        "time_schedule": "sigmoid",
