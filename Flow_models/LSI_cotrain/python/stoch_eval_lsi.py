...
# ------------------------------------------------------------
# New evaluation function using heun_pc_stoch at 40/80/120 steps
# (does NOT overwrite your existing evaluate_current_state)
# Adapted for MNIST/FMNIST (grayscale, LeNet/Inception features)
# ------------------------------------------------------------
def evaluate_current_state_stoch(
    ...
    fid_model=None,             # LeNet model for non-FMNIST datasets
    use_lenet_fid=False,        # True for non-FMNIST datasets
):
    """Full test-set evaluation with stochastic Heun-PC sampler at multiple step counts.

    Adapted for MNIST/FMNIST/EMNIST/KMNIST (grayscale images, 1 channel).
    Supports both LeNet features (for MNIST/EMNIST/KMNIST) and Inception features (for FMNIST).
    """
    ...
    # Feature extraction: use LeNet or Inception based on dataset
    ...
    # configs: always include recon; if unet provided include stochastic sampler at 40/80/120
    configs = [("VAE_Rec_eps", 0, "Recon (posterior z)")]
    if unet is not None:
        configs.extend([
             ("rk4_ode", 40, "RK4 ODE (10)"),
             ("rk4_ode", 20, "RK4 ODE (10)"),
            #("heun_pc_stoch", 40, "Stoch Heun-PC (40)"),
            #("heun_pc_stoch", 60, "Stoch Heun-PC (60)"),
    ...
    for method, steps, desc in configs:
        ...
        # Feature extraction for fake images
        if use_lenet_fid and fid_model is not None:
...
# Adapted for MNIST/FMNIST (grayscale, flexible dataset selection)
# ------------------------------------------------------------
...
def run_stoch_eval_from_checkpoints(
    ...
    """
    Run stochastic evaluation from saved checkpoints.

    Args:
        ckpt_dir: Directory containing checkpoint files
        epoch_label: Label for filenames/logs
        results_dir: Directory to save results
        also_run_baselines: Whether to also run rk4_ode@10
        cfg: Configuration dictionary
        train_mode: 'cotrained' or 'indep' (determines which VAE to load)
        dataset_key: One of "MNIST", "FMNIST", "EMNIST", "KMNIST"
    """
    ...
    # data - make_dataloaders returns (train_l, test_l, num_classes) for MNIST/FMNIST
    _, test_l, num_classes = make_dataloaders(cfg["batch_size"], cfg.get("num_workers", 2), dataset_key=dataset_key)

    # Get FID model (Inception for FMNIST, LeNet for others)
    # For evaluation from checkpoints, we need to potentially train or load the LeNet classifier
    if dataset_key == "FMNIST":
        fid_model = None
        use_lenet_fid = False
        print("--> Using Inception features for FID (FMNIST)")
    else:
        print(f"--> Training/Loading LeNet classifier for FID ({dataset_key})")
        # Need training loader for LeNet
        train_l, _, _ = make_dataloaders(cfg["batch_size"], cfg.get("num_workers", 2), dataset_key=dataset_key)
        checkpoint_path = os.path.join(ckpt_dir, f"fid_classifier_{dataset_key.lower()}.pt")
        fid_model = train_fid_classifier(train_l, num_classes, device, epochs=10, checkpoint_path=checkpoint_path)
        use_lenet_fid = True

    # models - Note: MNIST/FMNIST VAE uses in_channels=1 (grayscale)
    use_latent_norm = cfg.get("use_latent_norm", False)
    vae = VAE(latent_channels=cfg["latent_channels"], use_norm=use_latent_norm).to(device)
    ...
    # 2) LSI prior eval (stoch sampler @ 40/80/120)
    ...
    # 3) Control (Tweedie) prior eval (stoch sampler @ 40/80/120)
...
def main():
    """
    Example main function for running stochastic evaluation on MNIST/FMNIST checkpoints.

    Adjust cfg and paths as needed for your setup.
    """
    cfg = {
        "dataset": "FMNIST",
        "batch_size": 128,
    ...
    # --------- run it ----------
    # Assumes your checkpoints are in ./run_results/checkpoints/{vae_cotrained.pt, unet_lsi.pt, unet_control.pt}
    stoch_results = run_stoch_eval_from_checkpoints(
        ...
        dataset_key="FMNIST",           # "MNIST", "FMNIST", "EMNIST", or "KMNIST"
    )
