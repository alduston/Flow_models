...
def train_vae_ldm_indep_cond(cfg):
    """
    Two-stage independent training with conditional (CFG) support for baseline comparison:
    - Stage 1: Train VAE (recon + standard KL + perceptual loss)
    - Stage 2: Freeze VAE, train LSI and Tweedie score networks independently with class conditioning

    This is a conditional variant of train_vae_ldms_indep that uses the same pipeline
    as train_vae_cotrained_cond (same eval functions, same net classes, CFG support).
    ...
    # VAE does NOT use latent norm for independent training (standard VAE)
    vae = VAE(latent_channels=cfg["latent_channels"], use_norm=False).to(device)
    eval_freq = cfg.get("eval_freq", 10)

    # --- Online Models (with class conditioning) ---
    ...
    # ===================================================================
    print("=" * 60)
    print("TWO-STAGE INDEPENDENT TRAINING (CONDITIONAL / CFG)")
    print("Stage 1: Train VAE (recon + standard KL + perceptual)")
    print("Stage 2: Freeze VAE, train score networks (LSI and Tweedie) with CFG")
    print("=" * 60)
    ...
    # ===================================================================
    # STAGE 2: Freeze VAE, train score networks with CFG
    # ===================================================================
    ...
    print("    VAE is FROZEN. Training LSI and Tweedie score heads with CFG.")
    ...
    for ep in range(epochs_score):
        ...
        for x, y in tqdm(train_l, desc=f"Score Ep {ep+1}", leave=False):
            x = x.to(device)
