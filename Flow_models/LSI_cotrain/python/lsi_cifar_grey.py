...
def compute_diversity(imgs: torch.Tensor, lpips_fn: Any) -> float:
    ...
    # CIFAR grayscale is 1 channel, repeat for LPIPS
    if set1.shape[1] == 1:
...
# ===========================================================================
# CIFAR-10 VAE: 1 input channel (grayscale), 32x32 -> 8x8 latent
# ===========================================================================
...
class VAE(nn.Module):
    ...
    """
    def __init__(self, latent_channels: int = 4, base_ch: int = 32, in_channels: int = 1, use_norm: bool = False):
        super().__init__()
...
def make_dataloaders(batch_size, num_workers):
    """CIFAR-10 dataloaders. Images are converted to 32x32x1 grayscale, normalized to [-1,1]."""
...
def train_vae_cotrained(cfg):
    ...
    for ep in range(cfg["epochs_vae"]):
        ...
        for x, _ in tqdm(train_l, desc=f"Ep {ep+1}", leave=False):
            ...
            # CIFAR grayscale is 1 channel, LPIPS will handle channel repeat internally
            if LPIPS_AVAILABLE:
...
def main():
    # Grayscale CIFAR-10 Config - testing grayscale version
    cfg = {
        "batch_size": 128,
        "num_workers": 2,
    ...
    print("=== Dual Co-Training: LSI vs Control (Tweedie) - Grayscale CIFAR-10 ===")
